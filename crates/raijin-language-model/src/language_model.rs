...
use raijin_anthropic::{AnthropicError, parse_prompt_too_long};
use anyhow::{Result, anyhow};
use raijin_client::Client;
use raijin_client::UserStore;
use raijin_cloud_llm_client::CompletionRequestStatus;
use futures::FutureExt;
use futures::{StreamExt, future::BoxFuture, stream::BoxStream};
use inazuma::{AnyView, App, AsyncApp, Entity, SharedString, Task, Window};
use raijin_http_client::{StatusCode, http};
use inazuma_icons::IconName;
use raijin_open_router::OpenRouterError;
...
pub use raijin_env_vars::{EnvVar, env_var};
...
impl From<raijin_anthropic::ApiError> for LanguageModelCompletionError {
    fn from(error: raijin_anthropic::ApiError) -> Self {
        use raijin_anthropic::ApiErrorCode::*;
        let provider = ANTHROPIC_PROVIDER_NAME;
        match error.code() {
...
impl From<raijin_open_router::ApiError> for LanguageModelCompletionError {
    fn from(error: raijin_open_router::ApiError) -> Self {
        use raijin_open_router::ApiErrorCode::*;
        let provider = LanguageModelProviderName::new("OpenRouter");
        match error.code {
            InvalidRequestError => Self::BadRequestFormat {
                provider,
