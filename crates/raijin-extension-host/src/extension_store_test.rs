...
use raijin_fs::{FakeFs, Fs, RealFs};
...
async fn test_extension_store(cx: &mut TestAppContext) {
    ...
    let mut expected_index = ExtensionIndex {
        extensions: [
            ...
            (
                ...
                ExtensionIndexEntry {
                    manifest: Arc::new(ExtensionManifest {
                        id: "raijin-monokai".into(),
                        name: "Raijin Monokai".into(),
                        version: "2.0.0".into(),
                        schema_version: SchemaVersion::ZERO,
                        description: None,
                        authors: vec![],
    ...
    fs.insert_tree(
        "/the-extension-dir/installed/raijin-gruvbox",
        json!({
            "extension.json": r#"{
                "id": "raijin-gruvbox",
                "name": "Raijin Gruvbox",
    ...
    expected_index.extensions.insert(
        ...
        ExtensionIndexEntry {
            manifest: Arc::new(ExtensionManifest {
                id: "raijin-gruvbox".into(),
                name: "Raijin Gruvbox".into(),
                version: "1.0.0".into(),
                schema_version: SchemaVersion::ZERO,
                description: None,
                authors: vec![],
    ...
    // Create new extension store, as if Raijin were restarting.
    drop(store);
    let store = cx.new(|cx| {
        ExtensionStore::new(
            PathBuf::from("/the-extension-dir"),
    ...
    store.update(cx, |store, cx| {
        store
            .uninstall_extension("raijin-ruby".into(), cx)
            .detach_and_log_err(cx);
    });

    cx.executor().advance_clock(RELOAD_DEBOUNCE_DURATION);
    expected_index.extensions.remove("raijin-ruby");
