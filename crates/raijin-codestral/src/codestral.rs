use anyhow::Result;
use raijin_edit_prediction::cursor_excerpt;
...
use raijin_language_model::{ApiKeyState, AuthenticateError, EnvVar, env_var};
...
impl CodestralEditPredictionDelegate {
    ...
    async fn fetch_completion(
        ...
        let http_request = raijin_http_client::Request::builder()
            .method(raijin_http_client::Method::POST)
            .uri(format!("{}/v1/fim/completions", api_url))
            .header("Content-Type", "application/json")
            .header("Authorization", format!("Bearer {}", api_key))
            .body(raijin_http_client::AsyncBody::from(request_body))?;
