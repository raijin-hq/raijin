...
use raijin_ui::{ButtonLike, ElevationIndex, KeyBinding, prelude::*};
use inazuma_util::ResultExt as _;
use raijin_workspace::item::ItemEvent;
use raijin_workspace::{Workspace, item::Item};
...
impl Render for ReplSessionsPage {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // When there are no kernel specifications, show a link to the Raijin docs explaining how to
                    ...
                    h_flex().w_full().p_4().justify_center().gap_2().child(
                        ButtonLike::new("install-kernels")
                            ...
                            .on_click(move |_, _, cx| {
                                cx.open_url(
                                    "https://raijin.dev/docs/repl#language-specific-instructions",
                                )
                            }),
                    ),
                );
