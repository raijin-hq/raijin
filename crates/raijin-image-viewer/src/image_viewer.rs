...
use raijin_editor::{EditorSettings, items::entry_git_aware_label_color};
...
use raijin_language::File as _;
use persistence::ImageViewerDb;
use raijin_project::{ImageItem, Project, ProjectPath, image_store::ImageItemEvent};
    ...
    fn to_item_events(event: &Self::Event, f: &mut dyn FnMut(raijin_workspace::item::ItemEvent)) {
        match event {
            ImageViewEvent::TitleChanged => {
                f(raijin_workspace::item::ItemEvent::UpdateTab);
                f(raijin_workspace::item::ItemEvent::UpdateBreadcrumbs);
            }
        }
    }
...
pub fn init(cx: &mut App) {
    raijin_workspace::register_project_item::<ImageView>(cx);
    raijin_workspace::register_serializable_item::<ImageView>(cx);
}
...
mod persistence {
    ...
    raijin_db::static_connection!(ImageViewerDb, [WorkspaceDb]);
