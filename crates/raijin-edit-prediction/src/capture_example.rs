...
pub fn capture_example(
    ...
    cursor_anchor: raijin_language::Anchor,
    ...
    Some(cx.spawn(async move |mut cx| {
        ...
        events.retain(|stored_event| {
            let raijin_zeta_prompt::Event::BufferChange { path, .. } = stored_event.event.as_ref();
...
fn write_event_with_relative_paths(
    output: &mut String,
    event: &raijin_zeta_prompt::Event,
    root_name: &str,
) {
...
fn compute_cursor_excerpt(
    ...
    let (_, context_range) = raijin_zeta_prompt::compute_editable_and_context_ranges(
        &excerpt_text,
        cursor_offset_in_excerpt,
        &syntax_ranges,
        100,
...
async fn collect_snapshots(
    project: &Entity<Project>,
    git_store: &Entity<raijin_project::git_store::GitStore>,
...
fn compute_uncommitted_diff(
    ...
    for (relative_path, (before_text, diff_snapshot)) in snapshots_by_path {
        if let Some(head_text) = &diff_snapshot.base_text_string() {
            let file_diff = raijin_language::unified_diff(head_text, &before_text.text());
...
mod tests {
    ...
    use indoc::indoc;
    use raijin_language::{Anchor, Point};
        ...
        assert!(
            matches!(
                events
                    ...
                    .event
                    .as_ref(),
                raijin_zeta_prompt::Event::BufferChange { path, .. } if path.as_ref() == "/external/external.rs"
            ),
