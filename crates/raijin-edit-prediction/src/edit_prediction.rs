...
use raijin_language::language_settings::all_language_settings;
use raijin_language::{Anchor, Buffer, File, Point, TextBufferSnapshot, ToOffset, ToPoint};
use raijin_language::{BufferSnapshot, OffsetRangeExt};
use raijin_language_model::{LlmApiToken, NeedsLlmTokenRefresh};
use raijin_project::{DisableAiSettings, Project, ProjectPath, WorktreeId};
...
fn currently_following(_project: &Entity<Project>, _cx: &App) -> bool {
    // Raijin has a single window and no collaboration — never following anyone.
    false
}
