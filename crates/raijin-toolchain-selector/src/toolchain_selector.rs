...
use raijin_language::{Language, LanguageName, Toolchain, ToolchainScope};
use raijin_open_path_prompt::OpenPathDelegate;
use inazuma_picker::{Picker, PickerDelegate};
use raijin_project::{DirectoryLister, Project, ProjectPath, Toolchains, WorktreeId};
use std::{
    borrow::Cow,
    path::{Path, PathBuf},
    sync::Arc,
    ...
    fn confirm(&mut self, _: bool, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        if let Some(string_match) = self.matches.get(self.selected_index) {
                ...
                .flatten()
            {
                ...
                let db = raijin_workspace::WorkspaceDb::global(cx);
