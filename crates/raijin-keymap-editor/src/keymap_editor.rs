...
use raijin_editor::{CompletionProvider, Editor, EditorEvent, EditorMode, SizingBehavior};
use raijin_fs::Fs;
...
use raijin_language::{Language, LanguageConfig, ToOffset as _};
...
use raijin_actions::{ChangeKeybinding, OpenKeymap};
...
impl KeymapEditor {
    fn new(workspace: WeakEntity<Workspace>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        cx.spawn({
            ...
            async move |this, cx| {
                let temp_dir = tempfile::tempdir_in(raijin_paths::temp_dir())?;
    ...
    async fn update_matches(
        ...
        let action_query = raijin_command_palette::normalize_action_query(&action_query);
    ...
    fn copy_context_to_clipboard(
        ...
        raijin_telemetry::event!("Keybinding Context Copied", context = context);
    ...
    fn copy_action_to_clipboard(
        ...
        raijin_telemetry::event!("Keybinding Action Copied", action = action);
...
impl HumanizedActionNameCache {
    fn new(cx: &App) -> Self {
        let cache = HashMap::from_iter(cx.all_action_names().iter().map(|&action_name| {
            (
                action_name,
                raijin_command_palette::humanize_action_name(action_name).into(),
            )
        }));
        Self { cache }
    }
...
impl KeybindingEditorModal {
    pub fn new(
        ...
        let (action_editor, action_name_to_static) = if has_action_editor {
            ...
            let humanized_names: HashMap<&'static str, SharedString> = actions
                .iter()
                .map(|&name| (name, raijin_command_palette::humanize_action_name(name).into()))
                .collect();
    ...
    fn validate_context(&self, cx: &App) -> anyhow::Result<Option<String>> {
        ...
        inazuma::KeyBindingContextPredicate::parse(&context).context("Failed to parse key context")?;
    ...
    fn new(
        ...
        cx.spawn_in(window, async move |this, cx| {
            let result = async {
                ...
                let file_name = raijin_json_schema_store::normalized_action_file_name(action_name);
        ...
        let (temp_file_path, temp_dir) = {
            ...
            async move {
                let temp_dir_backup = match temp_dir.as_ref() {
                    Some(_) => None,
                    None => {
                        let temp_dir = raijin_paths::temp_dir();
...
async fn load_keybind_context_language(
    ...
    let language_task = workspace
        .read_with(cx, |workspace, cx| {
            workspace
                ...
                .read(cx)
                .languages()
                .language_for_name("Raijin Keybind Context")
        })
        .context("Failed to load Raijin Keybind Context language")
        .log_err();
    ...
    language.unwrap_or_else(|| {
        Arc::new(Language::new(
            LanguageConfig {
                name: "Raijin Keybind Context".into(),
                ..Default::default()
            },
...
impl SerializableItem for KeymapEditor {
    ...
    fn cleanup(
        ...
        alive_items: Vec<raijin_workspace::ItemId>,
        _window: &mut Window,
        cx: &mut App,
    ) -> inazuma::Task<inazuma::Result<()>> {
        let db = KeybindingEditorDb::global(cx);
        raijin_workspace::delete_unloaded_items(alive_items, workspace_id, "keybinding_editors", &db, cx)
    }
...
mod persistence {
    use raijin_db::{query, sqlez::domain::Domain, sqlez_macros::sql};
    use raijin_workspace::WorkspaceDb;

    pub struct KeybindingEditorDb(raijin_db::sqlez::thread_safe_connection::ThreadSafeConnection);
    ...
    raijin_db::static_connection!(KeybindingEditorDb, [WorkspaceDb]);
    ...
    impl KeybindingEditorDb {
        ...
        query! {
            pub fn get_keybinding_editor(
                item_id: raijin_workspace::ItemId,
                workspace_id: raijin_workspace::WorkspaceId
            ) -> Result<Option<raijin_workspace::ItemId>> {
                SELECT item_id
