...
impl GitRepository for FakeGitRepository {
    ...
    fn load_blob_content(&self, oid: raijin_git::Oid) -> BoxFuture<'_, Result<String>> {
    ...
    fn load_commit(
        ...
        _commit: String,
        _cx: AsyncApp,
    ) -> BoxFuture<'_, Result<raijin_git::repository::CommitDiff>> {
        unimplemented!()
    }
    ...
    fn stash_entries(&self) -> BoxFuture<'_, Result<raijin_git::stash::GitStash>> {
        async { Ok(raijin_git::stash::GitStash::default()) }.boxed()
    }
    ...
    ) -> BoxFuture<'_, Result<raijin_git::blame::Blame>> {
        self.with_state_async(false, move |state| {
            state
                .blames
                .get(&path)
    ...
    fn file_history(&self, path: RepoPath) -> BoxFuture<'_, Result<raijin_git::repository::FileHistory>> {
    ...
    fn file_history_paginated(
        ...
        _skip: usize,
        _limit: Option<usize>,
    ) -> BoxFuture<'_, Result<raijin_git::repository::FileHistory>> {
        async move {
    ...
    fn push(
        ...
        _env: Arc<HashMap<String, String>>,
        _cx: AsyncApp,
    ) -> BoxFuture<'_, Result<raijin_git::repository::RemoteCommandOutput>> {
        unimplemented!()
    }
    ...
    fn diff(&self, _diff: raijin_git::repository::DiffType) -> BoxFuture<'_, Result<String>> {
    ...
    fn diff_stat(
    ...
    ) -> BoxFuture<'_, Result<raijin_git::status::GitDiffStat>> {
        ...
        self.with_state_async(false, move |state| {
            ...
            Ok(raijin_git::status::GitDiffStat {
                entries: entries.into(),
            })
        })
        .boxed()
    ...
    fn checkpoint(&self) -> BoxFuture<'static, Result<GitRepositoryCheckpoint>> {
        ...
        async move {
            executor.simulate_random_delay().await;
            let oid = raijin_git::Oid::random(&mut *executor.rng().lock());
