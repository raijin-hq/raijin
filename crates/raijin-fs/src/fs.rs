...
use raijin_git::repository::{GitRepository, RealGitRepository};
...
impl From<raijin_proto::Timestamp> for MTime {
    fn from(timestamp: raijin_proto::Timestamp) -> Self {
        MTime(timestamp.into())
    }
}

impl From<MTime> for raijin_proto::Timestamp {
...
impl Fs for RealFs {
    ...
    async fn atomic_write(&self, path: PathBuf, data: String) -> Result<()> {
        smol::unblock(move || {
            ...
            // See https://github.com/raijin-industries/raijin/pull/8437 for more details.
            let mut tmp_file =
                tempfile::NamedTempFile::new_in(path.parent().unwrap_or(raijin_paths::temp_dir()))?;
    ...
    async fn atomic_write(&self, path: PathBuf, data: String) -> Result<()> {
        smol::unblock(move || {
            ...
            // https://github.com/raijin-industries/raijin/issues/16571
            let temp_dir = TempDir::new_in(path.parent().unwrap_or(raijin_paths::temp_dir()))?;
            let temp_file = {
...
impl FakeFs {
    ...
    pub fn set_blame_for_repo(&self, dot_git: &Path, blames: Vec<(RepoPath, raijin_git::blame::Blame)>) {
        self.with_git_state(dot_git, true, |state| {
            state.blames.clear();
