...
use raijin_agent::{NativeAgent, NativeAgentConnection, Templates, ThreadStore};
...
#[command(
    name = "eval-cli",
    about = "Run Raijin's agent headlessly in evaluation/benchmark environments"
)]
struct Args {
...
async fn run_agent(
    ...
    cx: &mut AsyncApp,
) -> (Result<AgentOutcome>, Option<raijin_language_model::TokenUsage>) {
    let setup_result: Result<()> = cx.update(|cx| {
        ...
        registry.update(cx, |registry, cx| {
            registry.set_default_model(
                Some(raijin_language_model::ConfiguredModel {
                    provider: registry
    ...
    let acp_usage = cx.update(|cx| {
        acp_thread
            .read(cx)
            .token_usage()
            .map(|usage| raijin_language_model::TokenUsage {
