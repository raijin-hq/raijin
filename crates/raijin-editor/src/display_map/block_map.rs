...
use raijin_language::{Patch, Point};
...
use raijin_ui::{ElementId, IntoElement};
...
impl BlockMap {
    ...
    #[raijin_tracing::instrument(skip_all, fields(edits = ?edits))]
    fn sync(
        &self,
        wrap_snapshot: &WrapSnapshot,
        mut edits: WrapPatch,
        ...
        while let Some(edit) = edits.next() {
            let span = raijin_tracing::debug_span!("while edits", edit = ?edit);
            ...
            loop {
                let span = raijin_tracing::debug_span!("decide where edit ends loop");
                let _enter = span.enter();
            ...
            for (block_placement, block) in blocks_in_edit.drain(..) {
                let span =
                    raijin_tracing::debug_span!("for block in edits", block_height = block.height());
                let _enter = span.enter();
...
#[raijin_tracing::instrument(skip(tree, wrap_snapshot))]
...
mod tests {
    ...
    use raijin_language::{Buffer, Capability, Point};
