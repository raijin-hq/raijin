...
use raijin_language::{Edit, HighlightId, Point};
...
use raijin_ui::IntoElement as _;
...
impl FoldMapWriter<'_> {
    ...
    pub(crate) fn fold<T: ToOffset>(
        ...
        let buffer = &snapshot.buffer;
        folds.sort_unstable_by(|a, b| inazuma_sum_tree::SeekTarget::cmp(&a.range, &b.range, buffer));
...
pub struct Chunk<'a> {
    ...
    pub diagnostic_severity: Option<raijin_lsp::DiagnosticSeverity>,
