...
use raijin_language::{Bias, ToOffset};
use linkify::{LinkFinder, LinkKind};
use raijin_lsp::LanguageServerId;
use raijin_project::{InlayId, LocationLink, Project, ResolvedPath};
use regex::Regex;
...
pub enum HoverLink {
    ...
    File(ResolvedPath),
    Text(LocationLink),
    InlayHint(raijin_lsp::Location, LanguageServerId),
}
...
pub enum TriggerPoint {
    Text(Anchor),
    InlayHint(InlayHighlight, raijin_lsp::Location, LanguageServerId),
}
...
mod tests {
    ...
    use indoc::indoc;
    use raijin_lsp::request::{GotoDefinition, GotoTypeDefinition};
    ...
    use raijin_workspace::item::Item;
    ...
    async fn test_hover_type_links(cx: &mut inazuma::TestAppContext) {
        ...
        let mut requests =
            cx.set_request_handler::<GotoTypeDefinition, _, _>(move |url, _, _| async move {
                Ok(Some(raijin_lsp::GotoTypeDefinitionResponse::Link(vec![
                    raijin_lsp::LocationLink {
    ...
    async fn test_hover_links(cx: &mut inazuma::TestAppContext) {
        ...
        let mut requests =
            cx.set_request_handler::<GotoDefinition, _, _>(move |url, _, _| async move {
                Ok(Some(raijin_lsp::GotoDefinitionResponse::Link(vec![
                    raijin_lsp::LocationLink {
                        origin_selection_range: Some(symbol_range),
                        target_uri: url.clone(),
                        target_range,
