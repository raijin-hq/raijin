...
use raijin_language::{Buffer, OutlineItem};
...
use raijin_theme::{ActiveTheme as _, SyntaxTheme};
...
mod tests {
    ...
    fn lsp_range(start_line: u32, start_char: u32, end_line: u32, end_char: u32) -> raijin_lsp::Range {
        raijin_lsp::Range {
            start: raijin_lsp::Position::new(start_line, start_char),
            end: raijin_lsp::Position::new(end_line, end_char),
        }
    }

    fn nested_symbol(
        name: &str,
        kind: raijin_lsp::SymbolKind,
        range: raijin_lsp::Range,
        selection_range: raijin_lsp::Range,
        children: Vec<raijin_lsp::DocumentSymbol>,
            ...
            .set_request_handler::<raijin_lsp::request::DocumentSymbolRequest, _, _>(
                move |_, _, _| async move {
                    Ok(Some(raijin_lsp::DocumentSymbolResponse::Nested(vec![
                        nested_symbol(
                            "main",
                            raijin_lsp::SymbolKind::FUNCTION,
                            lsp_range(0, 0, 2, 1),
                            lsp_range(0, 3, 0, 7),
                            Vec::new(),
                        ),
    ...
    async fn test_lsp_document_symbols_caches_results(cx: &mut TestAppContext) {
        ...
        let mut symbol_request = cx
            .set_request_handler::<raijin_lsp::request::DocumentSymbolRequest, _, _>(move |_, _, _| {
    ...
    async fn test_lsp_document_symbols_flat_response(cx: &mut TestAppContext) {
        ...
        let mut symbol_request = cx
            .set_request_handler::<raijin_lsp::request::DocumentSymbolRequest, _, _>(
                move |_, _, _| async move {
                    #[allow(deprecated)]
                    Ok(Some(raijin_lsp::DocumentSymbolResponse::Flat(vec![
                        raijin_lsp::SymbolInformation {
                            ...
                            location: raijin_lsp::Location {
                                uri: raijin_lsp::Uri::from_file_path(path!("/a/main.rs")).unwrap(),
    ...
    async fn test_lsp_document_symbols_empty_response(cx: &mut TestAppContext) {
        ...
        let mut symbol_request = cx
            .set_request_handler::<raijin_lsp::request::DocumentSymbolRequest, _, _>(
                move |_, _, _| async move {
                    Ok(Some(raijin_lsp::DocumentSymbolResponse::Nested(Vec::new())))
                },
            );
    ...
    async fn test_lsp_document_symbols_disabled_by_default(cx: &mut TestAppContext) {
        ...
        let request_count_clone = request_count.clone();
        let _symbol_request =
            cx.set_request_handler::<raijin_lsp::request::DocumentSymbolRequest, _, _>(move |_, _, _| {
                request_count_clone.fetch_add(1, atomic::Ordering::AcqRel);
                async move {
                    Ok(Some(raijin_lsp::DocumentSymbolResponse::Nested(vec![
                        nested_symbol(
