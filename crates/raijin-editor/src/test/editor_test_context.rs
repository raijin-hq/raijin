...
use raijin_git::repository::RepoPath;
use inazuma::{
...
use raijin_language::{Buffer, BufferSnapshot, LanguageRegistry};
use raijin_multi_buffer::{Anchor, ExcerptRange, MultiBufferOffset, MultiBufferRow, PathKey};
...
impl EditorTestContext {
    ...
    pub async fn for_editor_in(editor: Entity<Editor>, cx: &mut inazuma::VisualTestContext) -> Self {
        cx.focus(&editor);
        Self {
            window: cx.windows()[0],
            cx: cx.clone(),
    ...
    pub fn text_anchor_range(&mut self, marked_text: &str) -> Range<raijin_language::Anchor> {
        let ranges = self.ranges(marked_text);
...
struct FormatMultiBufferAsMarkedText {
    ...
    excerpts: Vec<(ExcerptId, BufferSnapshot, ExcerptRange<inazuma_text::Anchor>, bool)>,
}
