...
use raijin_ui::ActiveTheme as _;
...
fn buffer_into_editor_highlights<'a, 'b>(
    ...
    multi_buffer_snapshot: &'a raijin_multi_buffer::MultiBufferSnapshot,
...
mod tests {
    ...
    use crate::{
        ...
        test::{build_editor_with_project, editor_lsp_test_coninazuma_text::EditorLspTestContext},
    };
    ...
    async fn lsp_semantic_tokens_full_capability(cx: &mut TestAppContext) {
            ...
            raijin_lsp::ServerCapabilities {
                semantic_tokens_provider: Some(
                    raijin_lsp::SemanticTokensServerCapabilities::SemanticTokensOptions(
                        raijin_lsp::SemanticTokensOptions {
                            legend: raijin_lsp::SemanticTokensLegend {
                                token_types: vec!["function".into()],
                                token_modifiers: Vec::new(),
                            },
                            full: Some(raijin_lsp::SemanticTokensFullOptions::Delta { delta: None }),
                            ..raijin_lsp::SemanticTokensOptions::default()
                        },
                    ),
                ),
        ...
        let mut full_request = cx
            .set_request_handler::<raijin_lsp::request::SemanticTokensFullRequest, _, _>(
                move |_, _, _| {
                    full_counter_clone.fetch_add(1, atomic::Ordering::Release);
                    async move {
                        Ok(Some(raijin_lsp::SemanticTokensResult::Tokens(
                            raijin_lsp::SemanticTokens {
                                data: vec![
                                    0, // delta_line
                                    3, // delta_start
                                    4, // length
                ...
                semantic_tokens_provider: Some(
                    raijin_lsp::SemanticTokensServerCapabilities::SemanticTokensOptions(
                        raijin_lsp::SemanticTokensOptions {
                            ...
                            full: Some(raijin_lsp::SemanticTokensFullOptions::Delta { delta: Some(true) }),
                            ..raijin_lsp::SemanticTokensOptions::default()
                        },
                    ),
                ),
    ...
    async fn lsp_semantic_tokens_delta(cx: &mut TestAppContext) {
        ...
        let mut delta_request = cx
            .set_request_handler::<raijin_lsp::request::SemanticTokensFullDeltaRequest, _, _>(
                move |_, params, _| {
                    delta_counter_clone.fetch_add(1, atomic::Ordering::Release);
                    assert_eq!(params.previous_result_id, "a");
                    async move {
                        Ok(Some(raijin_lsp::SemanticTokensFullDeltaResult::TokensDelta(
                            raijin_lsp::SemanticTokensDelta {
                                edits: Vec::new(),
    ...
    async fn lsp_semantic_tokens_multiserver_full(cx: &mut TestAppContext) {
        ...
        let toml_legend_1 = raijin_lsp::SemanticTokensLegend {
            token_types: vec!["property".into()],
            token_modifiers: Vec::new(),
        };
        let toml_legend_2 = raijin_lsp::SemanticTokensLegend {
            token_types: vec!["number".into()],
            token_modifiers: Vec::new(),
        };

        let app_state = cx.update(raijin_workspace::AppState::test);
    ...
    async fn lsp_semantic_tokens_multibuffer_part(cx: &mut TestAppContext) {
        ...
        let toml_legend = raijin_lsp::SemanticTokensLegend {
            token_types: vec!["property".into()],
            token_modifiers: Vec::new(),
        };
        let rust_legend = raijin_lsp::SemanticTokensLegend {
