...
use raijin_language::{Buffer, BufferRow, Runnable};
...
use raijin_task::{ResolvedTask, RunnableTag, TaskContext, TaskTemplate, TaskVariables, VariableName};
use inazuma_text::{BufferId, OffsetRangeExt as _, ToOffset as _, ToPoint as _};
use raijin_ui::{Clickable as _, Color, IconButton, IconSize, Toggleable as _};
...
pub struct RunnableTasks {
    ...
    pub offset: raijin_multi_buffer::Anchor,
...
impl Editor {
    ...
    pub fn render_run_indicator(
        ...
        IconButton::new(
            ...
            raijin_ui::IconName::PlayOutlined,
        )
    ...
    fn runnable_rows(
        ...
        prefer_lsp: bool,
        runnable_ranges: Vec<(Range<Anchor>, raijin_language::RunnableRange)>,
...
mod tests {
    ...
    use indoc::indoc;
    use raijin_language::{ContextProvider, FakeLspAdapter};
    ...
    use serde_json::json;
    use raijin_task::{TaskTemplate, TaskTemplates};
    ...
    impl ContextProvider for TestRustContextProvider {
        fn associated_tasks(
            &self,
            _: Option<Entity<raijin_language::Buffer>>,
            _: &inazuma::App,
    ...
    fn rust_lang_with_task_context() -> Arc<raijin_language::Language> {
        Arc::new(
    ...
    fn rust_lang_with_lsp_task_context() -> Arc<raijin_language::Language> {
        Arc::new(
            Arc::try_unwrap(rust_lang())
                .unwrap()
    ...
    fn collect_runnable_labels(
        editor: &Editor,
    ) -> Vec<(inazuma_text::BufferId, raijin_language::BufferRow, Vec<String>)> {
    ...
    async fn test_multi_buffer_runnables_on_scroll(cx: &mut TestAppContext) {
        ...
        let multi_buffer = cx.new(|cx| {
            let mut multi_buffer = MultiBuffer::new(raijin_language::Capability::ReadWrite);
    ...
    async fn test_lsp_runnables_removed_after_edit(cx: &mut TestAppContext) {
        ...
        use raijin_project::lsp_store::lsp_ext_command::Runnables;
        fake_server.set_request_handler::<Runnables, _, _>(move |params, _| async move {
            ...
            if text.contains("main.rs") {
                let uri = raijin_lsp::Uri::from_file_path(path!("/project/main.rs")).expect("valid uri");
                Ok(vec![Runnable {
                    label: "LSP test_one".into(),
                    location: Some(raijin_lsp::LocationLink {
                        origin_selection_range: None,
                        target_uri: uri,
                        target_range: raijin_lsp::Range::new(
                            raijin_lsp::Position::new(0, 0),
                            raijin_lsp::Position::new(3, 1),
                        ),
                        target_selection_range: raijin_lsp::Range::new(
                            raijin_lsp::Position::new(0, 0),
                            raijin_lsp::Position::new(3, 1),
                        ),
                    }),
        ...
        assert_eq!(
            labels,
            Vec::<(inazuma_text::BufferId, raijin_language::BufferRow, Vec<String>)>::new(),
