...
use raijin_language::{Capability, Language, proto::serialize_anchor};
        ...
        let location_links = if let Some((client, project_id)) = upstream_client {
            ...
            project
                .update(cx, |project, cx| {
                    project.request_lsp(
                        buffer,
                        raijin_project::LanguageServerToQuery::Other(server_to_query),
                        raijin_project::lsp_store::lsp_ext_command::GoToParentModule { position },
                        cx,
                    )
                })
                .await
    ...
    cx.spawn_in(window, async move |_editor, cx| {
            ...
            project
                .update(cx, |project, cx| {
                    project.request_lsp(
                        buffer,
                        raijin_project::LanguageServerToQuery::Other(server_to_query),
                        raijin_project::lsp_store::lsp_ext_command::OpenDocs { position },
                        cx,
                    )
                })
                .await
        ...
        workspace.update(cx, |_workspace, cx| {
            ...
            if let Some(local_url) = docs_urls.local
                && raijin_fs::metadata(Path::new(&local_url[8..])).is_ok()
            {
