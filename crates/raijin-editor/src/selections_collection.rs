...
use raijin_language::{Bias, Point, PointUtf16, Selection, SelectionGoal};
use raijin_multi_buffer::{MultiBufferDimension, MultiBufferOffset};
...
impl SelectionsCollection {
    ...
    fn build_columnar_selection_from_utf16_columns(
        ...
        let is_empty = positions.start == positions.end;
        let line_len_utf16 = snapshot.line_len_utf16(raijin_multi_buffer::MultiBufferRow(buffer_row));
...
impl<'snap, 'a> MutableSelectionsCollection<'snap, 'a> {
    ...
    pub fn remove_selections_from_buffer(&mut self, buffer_id: raijin_language::BufferId) {
        let mut changed = false;
...
/// but don't share any positions remain separate, see: https://github.com/raijin-industries/raijin/issues/24748
