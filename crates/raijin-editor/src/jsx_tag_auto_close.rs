...
use raijin_multi_buffer::{BufferOffset, MultiBuffer, ToOffset};
use std::ops::Range;
use inazuma_util::ResultExt as _;

use raijin_language::{BufferSnapshot, JsxTagAutoCloseConfig, Node, language_settings::LanguageSettings};
...
pub(crate) type InitialBufferVersionsMap = HashMap<raijin_language::BufferId, inazuma_clock::Global>;
...
pub(crate) fn handle_from(
    ...
    struct JsxAutoCloseEditContext {
        ...
        config: raijin_language::JsxTagAutoCloseConfig,
        edits: Vec<Range<Anchor>>,
    }

    let mut edit_contexts =
        HashMap::<(raijin_language::BufferId, raijin_language::LanguageId), JsxAutoCloseEditContext>::default();
    ...
    for ((buffer_id, _), auto_close_context) in edit_contexts {
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            if buffer_parse_status == raijin_language::ParseStatus::Parsing {
                let Some(raijin_language::ParseStatus::Idle) = buffer_parse_status_rx.recv().await.ok()
                else {
                    return Some(());
                };
            }
...
mod jsx_tag_autoclose_tests {
    use crate::{
        ...
        test::{build_editor, editor_test_coninazuma_text::EditorTestContext},
    };

    use super::*;
    use inazuma::{AppContext as _, TestAppContext};
    use languages::language;
    use raijin_multi_buffer::{MultiBufferOffset, PathKey};
    ...
    async fn test_multibuffer(cx: &mut TestAppContext) {
        ...
        let buffer_a = cx.new(|cx| {
            let mut buf = raijin_language::Buffer::local("<div", cx);
            buf.set_language(
        ...
        let buffer_b = cx.new(|cx| {
            let mut buf = raijin_language::Buffer::local("<pre", cx);
            buf.set_language(
                Some(language("tsx", tree_sitter_typescript::LANGUAGE_TSX.into())),
                cx,
            );
        ...
        let buffer_c = cx.new(|cx| raijin_language::Buffer::local("<span", cx));
        let buffer = cx.new(|cx| {
            let mut buf = MultiBuffer::new(raijin_language::Capability::ReadWrite);
            let range_a = raijin_language::Point::zero()..buffer_a.read(cx).max_point();
            let range_b = raijin_language::Point::zero()..buffer_b.read(cx).max_point();
            let range_c = raijin_language::Point::zero()..buffer_c.read(cx).max_point();
