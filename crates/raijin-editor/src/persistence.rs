...
pub struct EditorDb(raijin_db::sqlez::thread_safe_connection::ThreadSafeConnection);
...
raijin_db::static_connection!(EditorDb, [WorkspaceDb]);
...
mod tests {
    ...
    async fn test_save_and_get_serialized_editor(cx: &mut inazuma::TestAppContext) {
        let db = cx.update(|cx| raijin_workspace::WorkspaceDb::global(cx));
    ...
    // 1. Open a file, fold some sections, close Raijin
    // 2. Add text at the START of the file externally (shifts all offsets)
    // 3. Reopen Raijin - folds should be restored at their NEW correct positions
