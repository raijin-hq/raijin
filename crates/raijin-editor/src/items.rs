...
use raijin_rpc::proto::{self, update_view};
...
use raijin_ui::{IconDecorationKind, prelude::*};
use inazuma_util::{ResultExt, TryFutureExt, paths::PathExt};
use raijin_workspace::item::{Dedup, ItemSettings, SerializableItem, TabContentParams};
...
impl FollowableItem for Editor {
    ...
    fn to_follow_event(event: &EditorEvent) -> Option<raijin_workspace::item::FollowEvent> {
        match event {
    ...
    fn update_agent_location(
        &mut self,
        location: raijin_language::Anchor,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let buffer = self.buffer.read(cx);
...
) -> Option<(ExcerptId, ExcerptRange<raijin_language::Anchor>)> {
    let context = {
        let start = raijin_language::proto::deserialize_anchor(excerpt.context_start?)?;
        let end = raijin_language::proto::deserialize_anchor(excerpt.context_end?)?;
        start..end
    };
    let primary = excerpt
        .primary_start
        ...
        .and_then(|(start, end)| {
            let start = raijin_language::proto::deserialize_anchor(start)?;
            let end = raijin_language::proto::deserialize_anchor(end)?;
            Some(start..end)
        })
...
fn deserialize_anchor(anchor: proto::EditorAnchor) -> Option<Anchor> {
    ...
    Some(Anchor::in_buffer(
        excerpt_id,
        raijin_language::proto::deserialize_anchor(anchor.anchor?)?,
    ))
}
...
impl Item for Editor {
    ...
    fn tab_content(&self, params: TabContentParams, _: &Window, cx: &App) -> AnyElement {
        ...
        let description = params.detail.and_then(|detail| {
            ...
            Some(inazuma_util::truncate_and_trailoff(description, MAX_TAB_TITLE_LEN))
        });
    ...
    fn added_to_workspace(
        ...
        if let Some(workspace_entity) = &workspace.weak_handle().upgrade() {
            cx.subscribe(
                workspace_entity,
                |editor, _, event: &raijin_workspace::Event, _cx| {
                    if let raijin_workspace::Event::ModalOpened = event {
        ...
        if !has_folds {
            if let Some(workspace_id) = workspace.database_id()
                && let Some(file_path) = self.buffer().read(cx).as_singleton().and_then(|buffer| {
                    raijin_project::File::from_dyn(buffer.read(cx).file()).map(|file| file.abs_path(cx))
                })
            {
...
impl SerializableItem for Editor {
    ...
    fn cleanup(
        ...
        raijin_workspace::delete_unloaded_items(
            alive_items,
            workspace_id,
            "editors",
            &EditorDb::global(cx),
    ...
    fn for_project_item(
        ...
        if let Some((excerpt_id, _, snapshot)) =
            ...
            && let Some(restoration_data) = Self::project_item_kind()
                ...
                .and_then(|data| {
                    let file = raijin_project::File::from_dyn(buffer.read(cx).file())?;
    ...
    pub fn update_restoration_data(
        ...
        cx.defer(move |cx| {
            editor.update(cx, |editor, cx| {
                ...
                let file_abs_path = raijin_project::File::from_dyn(buffer.read(cx).file())?.abs_path(cx);
...
fn path_for_file<'a>(
    file: &'a Arc<dyn raijin_language::File>,
    mut height: usize,
    include_filename: bool,
    cx: &'a App,
) -> Option<Cow<'a, str>> {
    if raijin_project::File::from_dyn(Some(file)).is_none() {
        return None;
    }
...
mod tests {
    ...
    fn test_path_for_file(cx: &mut App) {
        let file: Arc<dyn raijin_language::File> = Arc::new(TestFile {
    ...
    // Regression test for https://github.com/raijin-industries/raijin/issues/35947
