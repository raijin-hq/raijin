...
//! [`TextSummary`]: raijin_multi_buffer::MBTextSummary
...
use raijin_project::project_settings::DiagnosticSeverity;
use raijin_project::{InlayId, lsp_store::LspFoldingRange, lsp_store::TokenType};
...
use raijin_ui::{SharedString, px};
use unicode_segmentation::UnicodeSegmentation;
use raijin_tracing::instrument;
...
pub struct SemanticTokenHighlight {
    ...
    pub server_id: raijin_lsp::LanguageServerId,
}
...
impl DisplayMap {
    ...
    pub fn fold_buffers(
        &mut self,
        buffer_ids: impl IntoIterator<Item = raijin_language::BufferId>,
        cx: &mut App,
    ) {
    ...
    #[instrument(skip_all)]
    pub(crate) fn is_buffer_folded(&self, buffer_id: raijin_language::BufferId) -> bool {
...
impl DisplaySnapshot {
            ...
            let diagnostic_highlight = chunk
                ...
                .map(|severity| HighlightStyle {
                    ...
                    underline: (chunk.underline
                        ...
                        && !(chunk.is_unnecessary && severity > raijin_lsp::DiagnosticSeverity::WARNING))
                        .then(|| {
    ...
    pub(super) fn combined_highlights(
        ...
        syntax_theme: &raijin_theme::SyntaxTheme,
...
pub mod tests {
    ...
    use raijin_theme::{LoadThemes, SyntaxTheme};
        ...
        _ = cx.update_window(window, |_, window, cx| {
            ...
            assert_eq!(
                movement::up(
                    ...
                    raijin_language::SelectionGoal::None,
                    false,
                    &text_layout_details,
                ),
                (
                    DisplayPoint::new(DisplayRow(0), 7),
                    raijin_language::SelectionGoal::HorizontalPosition(f64::from(x))
                )
            );
            assert_eq!(
                movement::down(
    ...
    async fn test_chunks(cx: &mut inazuma::TestAppContext) {
        ...
        let theme =
            SyntaxTheme::new_test(vec![("mod.body", Oklch::red()), ("fn.name", Oklch::blue())]);
        ...
        assert_eq!(
            ...
            vec![
                ("fn ".to_string(), None),
                ("outer".to_string(), Some(Oklch::blue())),
                ("() {}\n\nmod module ".to_string(), None),
                ("{\n    fn ".to_string(), Some(Oklch::red())),
                ("inner".to_string(), Some(Oklch::blue())),
                ("() {}\n}".to_string(), Some(Oklch::red())),
            ]
        );
        assert_eq!(
    ...
    async fn test_chunks_with_syntax_highlighting_across_blocks(cx: &mut inazuma::TestAppContext) {
        ...
        pretty_assertions::assert_eq!(
            ...
            [
                ("const".into(), Some(Oklch::green())),
                (" A".into(), None),
                (":".into(), Some(Oklch::blue())),
                (" &str = ".into(), None),
                ("\"\n    one\n".into(), Some(Oklch::red())),
                ("\n".into(), None),
                ("    two\n    three\n\"".into(), Some(Oklch::red())),
    ...
    async fn test_chunks_with_diagnostics_across_blocks(cx: &mut inazuma::TestAppContext) {
        ...
        let snapshot = map.update(cx, |map, cx| map.snapshot(cx));
        let mut chunks = Vec::<(String, Option<raijin_lsp::DiagnosticSeverity>, Rgba)>::new();
        ...
        assert_eq!(
            ...
            [
                ...
                ("\n".into(), None, black),
                ("}".into(), Some(raijin_lsp::DiagnosticSeverity::ERROR), black),
    ...
    async fn test_chunks_with_soft_wrapping(cx: &mut inazuma::TestAppContext) {
        ...
        assert_eq!(
            ...
            [
                ("fn \n".to_string(), None),
                ("oute".to_string(), Some(Oklch::blue())),
                ("\n".to_string(), None),
                ("r".to_string(), Some(Oklch::blue())),
                ("() \n{}\n\n".to_string(), None),
            ]
        );
        assert_eq!(
    ...
    async fn test_chunks_with_text_highlights(cx: &mut inazuma::TestAppContext) {
        ...
        let theme =
            SyntaxTheme::new_test(vec![("operator", Oklch::red()), ("string", Oklch::green())]);
        let language = Arc::new(
            Language::new(
                LanguageConfig {
                    name: "Test".into(),
        ...
        assert_eq!(
            ...
            [
                ...
                ("a".to_string(), None, Some(Oklch::blue())),
                (":".to_string(), Some(Oklch::red()), Some(Oklch::blue())),
                (" B = ".to_string(), None, None),
                ("\"c ".to_string(), Some(Oklch::green()), None),
                ("d".to_string(), Some(Oklch::green()), Some(Oklch::blue())),
                ("\"".to_string(), Some(Oklch::green()), None),
            ]
        );
    }
    ...
    fn init_test(cx: &mut App, f: &dyn Fn(&mut SettingsContent)) {
        ...
        crate::init(cx);
        raijin_theme_settings::init(LoadThemes::JustBase, cx);
        cx.update_global::<SettingsStore, _>(|store, cx| {
