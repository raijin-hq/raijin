...
use raijin_buffer_diff::{BufferDiff, BufferDiffSnapshot};
...
use raijin_language::{Buffer, Capability, HighlightedText};
...
where
    F: Fn(&BufferDiffSnapshot, RangeInclusive<Point>, &inazuma_text::BufferSnapshot) -> Patch<Point>,
{
...
impl SplittableEditor {
    ...
    fn debug_print(&self, cx: &mut App) {
        ...
        use crate::display_map::Block;
        use raijin_buffer_diff::DiffHunkStatusKind;
        ...
        fn format_row(
            ...
            row_infos: &[raijin_multi_buffer::RowInfo],
    ...
    fn check_excerpt_mapping_invariants(&self, cx: &inazuma::App) {
        use raijin_multi_buffer::{ExcerptId, PathKey};
...
impl Item for SplittableEditor {
    ...
    fn tab_content_text(&self, detail: usize, cx: &App) -> raijin_ui::SharedString {
    ...
    fn tab_icon(&self, window: &Window, cx: &App) -> Option<raijin_ui::Icon> {
        self.rhs_editor.read(cx).tab_icon(window, cx)
    }

    fn tab_content(&self, params: TabContentParams, window: &Window, cx: &App) -> inazuma::AnyElement {
    ...
    fn capability(&self, cx: &App) -> raijin_language::Capability {
    ...
    fn save_as(
        &mut self,
        project: Entity<Project>,
        path: raijin_project::ProjectPath,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> inazuma::Task<anyhow::Result<()>> {
        self.rhs_editor
...
impl SearchableItem for SplittableEditor {
    ...
    fn active_match_index(
        &mut self,
        direction: raijin_workspace::searchable::Direction,
        matches: &[Self::Match],
        token: SearchToken,
        window: &mut Window,
        cx: &mut Context<Self>,
...
//     fn as_searchable(&self, _this: &Entity<Self>, cx: &App) -> Option<Box<dyn raijin_workspace::searchable::SearchableItemHandle>> {
...
mod tests {
    ...
    use raijin_language::language_settings::SoftWrap;
    use raijin_language::{Buffer, Capability};
    use raijin_multi_buffer::{MultiBuffer, PathKey};
    ...
    async fn test_random_split_editor(mut rng: StdRng, cx: &mut inazuma::TestAppContext) {
        use raijin_multi_buffer::ExpandExcerptDirection;
        use rand::prelude::*;
    ...
    async fn test_expand_excerpt_with_hunk_before_excerpt_start(cx: &mut inazuma::TestAppContext) {
        ...
        editor.update(cx, |editor, cx| {
            editor.expand_excerpts(
                ...
                2,
                raijin_multi_buffer::ExpandExcerptDirection::UpAndDown,
                cx,
            );
        });
    }
    ...
    #[inazuma::test]
    async fn test_edit_line_before_soft_wrapped_line_preceding_hunk(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    async fn test_two_path_keys_for_one_buffer(cx: &mut inazuma::TestAppContext) {
        use raijin_multi_buffer::PathKey;
        use inazuma_rope::Point;
        use unindent::Unindent as _;
