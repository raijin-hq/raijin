...
use inazuma::{
    ...
    FontWeight, GlobalElementId, Hitbox, HitboxBehavior, Oklch, InteractiveElement, IntoElement,
...
use raijin_language::{HighlightedText, IndentGuideSettings, language_settings::ShowWhitespaceSetting};
...
impl EditorElement {
    ...
    pub(crate) fn mouse_moved(
            ...
            && split_side != Some(SplitSide::Left)
        {
            ...
            {
                let as_point = inazuma_text::ToPoint::to_point(&buffer_anchor.text_anchor, buffer_snapshot);
    ...
    fn layout_inline_diagnostics(
        ...
        let severity_to_color = |sev: &raijin_lsp::DiagnosticSeverity| match sev {
            &raijin_lsp::DiagnosticSeverity::ERROR => Color::Error,
            &raijin_lsp::DiagnosticSeverity::WARNING => Color::Warning,
            &raijin_lsp::DiagnosticSeverity::INFORMATION => Color::Info,
            &raijin_lsp::DiagnosticSeverity::HINT => Color::Hint,
            _ => Color::Error,
        };
    ...
    fn layout_inline_code_actions(
        ...
        let icon_size = raijin_ui::IconSize::XSmall;
    ...
    fn layout_blame_popover(
        ...
        let cursor_point = self
            ...
            .read(cx)
            .selections
            .newest::<raijin_language::Point>(&editor_snapshot.display_snapshot)
            .head();
    ...
    fn layout_line_numbers(
        ...
        let segments = buffer_rows.iter().enumerate().flat_map(|(ix, row_info)| {
            ...
            if !(snapshot.number_deleted_lines || relative_line_numbers_enabled && relative_number.is_some())
                && row_info
                    .diff_status
                    .is_some_and(|status| status.is_deleted())
            {
                            ...
                            if scrollbar_settings.diagnostics != ScrollbarDiagnostics::None {
                                    ...
                                    .filter(|diagnostic| {
                                        match (
                                            ...
                                            (
                                                ...
                                                raijin_lsp::DiagnosticSeverity::ERROR,
                                            ) => true,
                                            (
                                                ScrollbarDiagnostics::Warning,
                                                raijin_lsp::DiagnosticSeverity::ERROR
                                                | raijin_lsp::DiagnosticSeverity::WARNING,
                                            ) => true,
                                            (
                                                ScrollbarDiagnostics::Information,
                                                raijin_lsp::DiagnosticSeverity::ERROR
                                                | raijin_lsp::DiagnosticSeverity::WARNING
                                                | raijin_lsp::DiagnosticSeverity::INFORMATION,
                                            ) => true,
                                            (_, _) => false,
                                        }
                                    })
                                ...
                                let marker_row_ranges = diagnostics.into_iter().map(|diagnostic| {
                                    ...
                                    let color = match diagnostic.diagnostic.severity {
                                        raijin_lsp::DiagnosticSeverity::ERROR => theme.status().error.color,
                                        raijin_lsp::DiagnosticSeverity::WARNING => theme.status().warning.color,
                                        raijin_lsp::DiagnosticSeverity::INFORMATION => theme.status().info.color,
...
pub fn render_breadcrumb_text(
    ...
    let highlighted_segments = segments.into_iter().enumerate().map(|(index, segment)| {
        ...
        if index == 0
            && !raijin_workspace::TabBarSettings::get_global(cx).show
                            ...
                            v_flex()
                                ...
                                .child(
                                    h_flex()
                                        ...
                                        .child(raijin_ui::KeyBinding::for_action_in(
                                            &raijin_actions::outline::ToggleOutline,
                                            &focus_handle,
                                            cx,
                                        )),
                                )
                        ...
                        .on_click({
                            ...
                            move |_, window, cx| {
                                if let Some((editor, callback)) = editor
                                    .upgrade()
                                    .zip(raijin_actions::outline::TOGGLE_OUTLINE.get())
                                {
...
struct JumpTargetInExcerptInput<'a> {
    id: ExcerptId,
    buffer: &'a raijin_language::BufferSnapshot,
...
fn header_jump_data_inner(
    ...
    let jump_position = raijin_language::ToPoint::to_point(&for_excerpt.jump_anchor, buffer);
    let excerpt_start = for_excerpt.excerpt_start_anchor;
    let rows_from_excerpt_start = if for_excerpt.jump_anchor == excerpt_start {
        0
    } else {
        let excerpt_start_point = raijin_language::ToPoint::to_point(&excerpt_start, buffer);
                            ...
                            |path_header| {
                                ...
                                path_header
                                    ...
                                    .when_some(breadcrumbs, |then, breadcrumbs| {
                                        let font = raijin_theme_settings::ThemeSettings::get_global(cx)
                                            .buffer_font
                                            .clone();
                ...
                match buffer_font_size {
                    AbsoluteLength::Pixels(pixels) => {
                        let rem_size_scale = {
                            ...
                            let default_font_size_scale = 14. / raijin_ui::BASE_REM_SIZE_IN_PX;
                    ...
                    AbsoluteLength::Rems(rems) => {
                        Some(rems.to_pixels(raijin_ui::BASE_REM_SIZE_IN_PX.into()))
                    }
                }
            }
...
mod tests {
    ...
    use raijin_language::{Buffer, language_settings, tree_sitter_python};
    use log::info;
                        ...
                        .map(|row| RowInfo {
                            ...
                            diff_status: (row == DELETED_LINE).then(|| {
                                DiffHunkStatus::deleted(
                                    raijin_buffer_diff::DiffHunkSecondaryStatus::NoSecondaryHunk,
                                )
                            }),
                            ..Default::default()
                        })
