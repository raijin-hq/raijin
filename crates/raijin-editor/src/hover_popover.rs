...
use inazuma::{
    AnyElement, App, AsyncWindowContext, Bounds, Context, Entity, Focusable as _, FontWeight, Oklch,
...
use raijin_language::{DiagnosticEntry, Language, LanguageRegistry};
use raijin_lsp::DiagnosticSeverity;
use raijin_markdown::{Markdown, MarkdownElement, MarkdownStyle};
use raijin_multi_buffer::{MultiBufferOffset, ToOffset, ToPoint};
use raijin_project::{HoverBlock, HoverBlockKind, InlayHintLabelPart};
...
async fn parse_blocks(
    ...
    let combined_text = blocks
        ...
        .map(|block| match &block.kind {
            raijin_project::HoverBlockKind::PlainText | raijin_project::HoverBlockKind::Markdown => {
                Cow::Borrowed(block.text.trim())
            }
            raijin_project::HoverBlockKind::Code { language } => {
                ...
                editor.update_in(cx, |editor, window, cx| {
                    editor.change_selections(Default::default(), window, cx, |selections| {
                        selections.select_ranges([
                            inazuma_text::Point::new(accum - 1, 0)..inazuma_text::Point::new(accum - 1, 0)
                        ]);
                    });
                })
            })
                    ...
                    div()
                        ...
                        .child(
                            MarkdownElement::new(markdown, hover_markdown_style(window, cx))
                                .code_block_renderer(raijin_markdown::CodeBlockRenderer::Default {
                                    copy_button: false,
...
mod tests {
    ...
    use crate::{
        ...
        test::editor_lsp_test_coninazuma_text::EditorLspTestContext,
    };
    use inazuma_collections::BTreeSet;
    use inazuma::App;
    use indoc::indoc;
    use raijin_markdown::parser::MarkdownEvent;
    use raijin_project::InlayId;
    ...
    async fn test_mouse_hover_info_popover_with_autocomplete_popover(
        ...
        let mut request = cx
            .lsp
            .set_request_handler::<raijin_lsp::request::HoverRequest, _, _>(
        ...
        cx.set_request_handler::<raijin_lsp::request::HoverRequest, _, _>(move |_, _, _| async move {
            Ok(Some(raijin_lsp::Hover {
                contents: raijin_lsp::HoverContents::Array(vec![
                    raijin_lsp::MarkedString::String("regular text for hover to show".to_string()),
                    raijin_lsp::MarkedString::String("".to_string()),
                    raijin_lsp::MarkedString::LanguageString(raijin_lsp::LanguageString {
    ...
    async fn test_line_ends_trimmed(cx: &mut inazuma::TestAppContext) {
        ...
        cx.set_request_handler::<raijin_lsp::request::HoverRequest, _, _>(move |_, _, _| {
    ...
    async fn test_hover_inlay_label_parts(cx: &mut inazuma::TestAppContext) {
            ...
            .set_request_handler::<raijin_lsp::request::InlayHintRequest, _, _>(move |params, _| {
                ...
                async move {
                    ...
                    Ok(Some(vec![raijin_lsp::InlayHint {
                        ...
                        label: raijin_lsp::InlayHintLabel::LabelParts(vec![raijin_lsp::InlayHintLabelPart {
        ...
        cx.lsp
            .set_request_handler::<raijin_lsp::request::InlayHintResolveRequest, _, _>(
                move |mut hint_to_resolve, _| {
                    ...
                    async move {
                        ...
                        hint_to_resolve.label = raijin_lsp::InlayHintLabel::LabelParts(vec![
                            raijin_lsp::InlayHintLabelPart {
                                value: ": ".to_string(),
                                ..Default::default()
                            },
                            raijin_lsp::InlayHintLabelPart {
                                value: new_type_label.to_string(),
                                location: Some(raijin_lsp::Location {
                                    uri: task_uri.clone(),
                                    range: new_type_target_range,
                                }),
                                tooltip: Some(raijin_lsp::InlayHintLabelPartTooltip::String(format!(
                            ...
                            raijin_lsp::InlayHintLabelPart {
                                ...
                                tooltip: Some(raijin_lsp::InlayHintLabelPartTooltip::MarkupContent(
    ...
    fn test_find_hovered_hint_part_with_multibyte_characters() {
        ...
        use raijin_multi_buffer::MultiBufferOffset;
        use raijin_project::InlayHintLabelPart;
