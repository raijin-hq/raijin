...
use raijin_language::point_from_lsp;
use raijin_multi_buffer::Anchor;
use raijin_project::{DocumentColor, InlayId};
use inazuma_settings_framework::Settings as _;
use inazuma_text::{Bias, BufferId, OffsetRangeExt as _};
use raijin_ui::{App, Context, Window};
...
mod tests {
    ...
    async fn test_document_colors(cx: &mut TestAppContext) {
        ...
        let mut fake_servers = language_registry.register_fake_lsp(
            ...
            FakeLspAdapter {
                capabilities: raijin_lsp::ServerCapabilities {
                    color_provider: Some(raijin_lsp::ColorProviderCapability::Simple(true)),
        ...
        let mut fake_servers_without_capabilities = language_registry.register_fake_lsp(
            ...
            FakeLspAdapter {
                capabilities: raijin_lsp::ServerCapabilities {
                    color_provider: Some(raijin_lsp::ColorProviderCapability::Simple(false)),
                    ..raijin_lsp::ServerCapabilities::default()
                },
        ...
        let mut color_request_handle = fake_language_server
            .set_request_handler::<raijin_lsp::request::DocumentColor, _, _>(move |params, _| {
                let requests_made = Arc::clone(&closure_requests_made);
                async move {
                    assert_eq!(
                        params.text_document.uri,
                        raijin_lsp::Uri::from_file_path(path!("/a/first.rs")).unwrap()
                    );
                    requests_made.fetch_add(1, atomic::Ordering::Release);
                    Ok(vec![
                        raijin_lsp::ColorInformation {
        ...
        let _handle = fake_language_server_without_capabilities
            .set_request_handler::<raijin_lsp::request::DocumentColor, _, _>(move |_, _| async move {
        ...
        workspace.update_in(cx, |workspace, window, cx| {
            ...
            workspace.move_item_to_pane_in_direction(
                &MoveItemToPaneInDirection {
                    direction: raijin_workspace::SplitDirection::Right,
                    focus: false,
                    clone: true,
                },
                window,
        ...
        workspace.update_in(cx, |workspace, window, cx| {
            workspace.active_pane().update(cx, |pane, cx| {
                pane.navigate_backward(&raijin_workspace::GoBack, window, cx);
            })
        });
        cx.executor().advance_clock(LSP_REQUEST_DEBOUNCE_TIMEOUT);
        cx.run_until_parked();
