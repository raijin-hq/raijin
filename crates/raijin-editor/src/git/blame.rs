...
use raijin_language::{Bias, BufferSnapshot, Edit};
use raijin_markdown::Markdown;
use raijin_multi_buffer::{MultiBuffer, RowInfo};
use raijin_project::{
    ...
    pub fn new(
        ...
        let multi_buffer_subscription = cx.subscribe(
            ...
            |git_blame, multi_buffer, event, cx| match event {
                raijin_multi_buffer::Event::DirtyChanged => {
                    if !multi_buffer.read(cx).is_dirty(cx) {
                        git_blame.generate(cx);
                    }
                }
                raijin_multi_buffer::Event::ExcerptsAdded { .. }
                | raijin_multi_buffer::Event::ExcerptsEdited { .. } => git_blame.regenerate_on_edit(cx),
                _ => {}
            },
        );
        ...
        let project_subscription = cx.subscribe(&project, {
            ...
            move |git_blame, _, event, cx| {
                if let raijin_project::Event::WorktreeUpdatedEntries(_, updated) = event {
    ...
    fn generate(&mut self, cx: &mut Context<Self>) {
        ...
        self.task = cx.spawn(async move |this, cx| {
            ...
            for buffers in buffers_to_blame.chunks(4) {
                let span = raijin_tracing::debug_span!("for each chunk of buffers");
...
mod tests {
    ...
    use inazuma::Context;
    use raijin_language::{Point, Rope};
