...
use indoc::indoc;
use raijin_language::EditPredictionsMode;
use raijin_language::{Buffer, CodeLabel};
use raijin_multi_buffer::{Anchor, ExcerptId, MultiBufferSnapshot, ToPoint};
use raijin_project::{Completion, CompletionResponse, CompletionSource};
use std::{
    ops::Range,
    rc::Rc,
    sync::{
...
use crate::{
    ...
    test::editor_test_coninazuma_text::EditorTestContext,
};
use raijin_rpc::proto::PeerId;
use raijin_workspace::CollaboratorId;
...
async fn propose_edits_with_preview<T: ToOffset + Clone>(
    ...
    cx.update(|_, cx| {
        provider.update(cx, |provider, _| {
            provider.set_edit_prediction(Some(raijin_edit_prediction_types::EditPrediction::Local {
                id: None,
...
impl EditPredictionDelegate for FakeEditPredictionDelegate {
    ...
    fn is_enabled(
        &self,
        _buffer: &inazuma::Entity<raijin_language::Buffer>,
    ...
    fn discard(
        &mut self,
        _reason: raijin_edit_prediction_types::EditPredictionDiscardReason,
        _cx: &mut inazuma::Context<Self>,
    ) {
    }
