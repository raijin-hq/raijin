...
use raijin_project::{LspStore, lsp_store::LocalLspAdapterDelegate};
...
const SCHEMA_URI_PREFIX: &str = "raijin://schemas/";
...
static TASKS_SCHEMA: LazyLock<String> = LazyLock::new(|| {
    serde_json::to_string(&raijin_task::TaskTemplates::generate_json_schema())
...
static KEYMAP_SCHEMA: LazyLock<String> = LazyLock::new(|| {
    serde_json::to_string(&inazuma_settings_framework::KeymapFile::generate_json_schema_from_inventory())
...
pub fn init(cx: &mut App) {
    ...
    raijin_project::lsp_store::json_language_server_ext::register_schema_handler(
    ...
    if let Some(extension_events) = raijin_extension::ExtensionEvents::try_global(cx) {
        cx.subscribe(&extension_events, move |_, evt, cx| match evt {
            ...
            | raijin_extension::Event::ConfigureExtensionRequested(_) => {}
        })
        .detach();
    }

    cx.observe_global::<raijin_dap::DapRegistry>(move |cx| {
        cx.update_global::<SchemaStore, _>(|schema_store, cx| {
...
impl SchemaStore {
    fn notify_schema_changed(&mut self, changed_schemas: ChangedSchemas, cx: &mut App) {
        ...
        self.lsp_stores.retain(|lsp_store| {
            ...
            raijin_project::lsp_store::json_language_server_ext::notify_schemas_changed(
                lsp_store,
                &uris_to_invalidate,
                cx,
            );
...
fn resolve_static_schema(path: &str) -> Option<String> {
    ...
    match schema_name {
        ...
        "action" => {
            ...
            let schema =
                inazuma_settings_framework::KeymapFile::get_action_schema_by_name(&action_name, &mut generator);
    ...
    let schema = match schema_name {
        ...
        "settings" => {
            ...
            cx.update(|cx| {
                ...
                if let Some(registry) = raijin_theme::ThemeRegistry::try_global(cx) {
                ...
                inazuma_settings_framework::SettingsStore::json_schema(&inazuma_settings_framework::SettingsJsonSchemaParams {
                    language_names,
                    font_names,
                    theme_names,
                    icon_theme_names,
        ...
        "project_settings" => {
            ...
            inazuma_settings_framework::SettingsStore::project_json_schema(&inazuma_settings_framework::SettingsJsonSchemaParams {
                language_names,
                lsp_adapter_names: &lsp_adapter_names,
        ...
        "debug_tasks" => {
            let adapter_schemas = cx.read_global::<raijin_dap::DapRegistry, _>(|dap_registry, _| {
                dap_registry.adapters_schema()
            });
            raijin_task::DebugTaskFile::generate_json_schema(&adapter_schemas)
        }
        "keymap" => cx.update(inazuma_settings_framework::KeymapFile::generate_json_schema_for_registered_actions),
        "action" => {
        ...
        "tasks" => raijin_task::TaskTemplates::generate_json_schema(),
        _ => {
...
pub fn all_schema_file_associations(
    ...
    let mut file_associations = serde_json::json!([
        {
            "fileMatch": [
                schema_file_match(raijin_paths::settings_file()),
            ],
            "url": format!("{SCHEMA_URI_PREFIX}settings"),
        },
        {
            "fileMatch": [
            raijin_paths::local_settings_file_relative_path()],
            "url": format!("{SCHEMA_URI_PREFIX}project_settings"),
        },
        {
            "fileMatch": [schema_file_match(raijin_paths::keymap_file())],
            "url": format!("{SCHEMA_URI_PREFIX}keymap"),
        },
        {
            "fileMatch": [
                schema_file_match(raijin_paths::tasks_file()),
                raijin_paths::local_tasks_file_relative_path()
            ],
            "url": format!("{SCHEMA_URI_PREFIX}tasks"),
        },
        {
            "fileMatch": [
                schema_file_match(raijin_paths::debug_scenarios_file()),
                raijin_paths::local_debug_file_relative_path()
            ],
