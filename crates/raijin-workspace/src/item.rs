...
use anyhow::Result;
use raijin_client::{Client, proto};
use inazuma::{
...
use raijin_language::Capability;
pub use raijin_language::HighlightedText;
use raijin_project::{Project, ProjectEntryId, ProjectPath};
...
use raijin_ui::{Color, Icon, IntoElement, Label, LabelCommon};
...
pub trait Item: Focusable + EventEmitter<Self::Event> + Render + Sized {
    ...
    fn for_each_project_item(
        &self,
        _: &App,
        _: &mut dyn FnMut(EntityId, &dyn raijin_project::ProjectItem),
    ) {
    }
            ...
            cx.on_focus_out(
                ...
                move |workspace, _event, window, cx| {
                        ...
                        && item.workspace_settings(cx).autosave == AutosaveSetting::OnFocusChange
                    {
                        ...
                        let vim_mode = raijin_vim_mode_setting::VimModeSetting::is_enabled(cx);
                        let helix_mode = raijin_vim_mode_setting::HelixModeSetting::is_enabled(cx);
...
pub trait ProjectItem: Item {
    type Item: raijin_project::ProjectItem;
    ...
    /// A fallback handler, which will be called after [`raijin_project::ProjectItem::try_open`] fails,
...
pub trait FollowableItem: Item {
    ...
    fn update_agent_location(
        &mut self,
        _location: raijin_language::Anchor,
        _window: &mut Window,
        _cx: &mut Context<Self>,
    ) {
    }
...
pub trait FollowableItemHandle: ItemHandle {
    ...
    fn update_agent_location(&self, location: raijin_language::Anchor, window: &mut Window, cx: &mut App);
}
...
pub mod test {
    ...
    use raijin_project::{Project, ProjectEntryId, ProjectPath, WorktreeId};
    ...
    impl raijin_project::ProjectItem for TestProjectItem {
        fn try_open(
            _project: &Entity<Project>,
            _path: &ProjectPath,
            _cx: &mut App,
