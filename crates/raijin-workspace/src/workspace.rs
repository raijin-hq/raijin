...
use raijin_db::smol::future::yield_now;
...
use inazuma::{
    ...
    Axis, Bounds, Context, CursorStyle, Decorations, DragMoveEvent, Entity, EntityId,
    EventEmitter, FocusHandle, Focusable, Global, HitboxBehavior, Oklch, KeyContext, Keystroke,
    ...
    WindowId, WindowOptions, actions, canvas, point, relative, size,
    transparent_black,
};
...
use raijin_language::{Buffer, LanguageRegistry, Rope, language_settings::all_language_settings};
pub use modal_layer::*;
...
pub use persistence::{
    ...
    model::{
        DockStructure, ItemId, SerializedWorkspace,
...
use raijin_task::{DebugScenario, SharedTaskContext, SpawnInTerminal};
use raijin_theme::{ActiveTheme, SystemAppearance};
...
use raijin_actions::{Spawn, theme::ToggleMode};
...
/// function in `raijin_shell`.  The `replace_window` and `env` fields are only
...
// that live in raijin-shell (to avoid circular dependencies).
// ---------------------------------------------------------------------------

/// Trait that raijin-shell implements to provide window-opening functions
/// to raijin-workspace code.  Registered as a global via [`Global`].
...
/// Register the workspace opener (called by raijin-shell during init).
...
impl WorkspaceRegistry {
    ...
    pub fn register(window_id: WindowId, workspace: WeakEntity<Workspace>, cx: &mut App) {
        Self::ensure(cx);
        cx.global_mut::<Self>().workspaces.insert(window_id, workspace);
    }
        ...
        cx.spawn(async move |cx| {
            ...
            cx.update(|cx| {
                for window in cx.windows() {
                    let ok = window.update(cx, |_, window, cx| {
                        if let Some(ws) = Workspace::for_window(window, cx) {
                            if ws.entity_id() == workspace.entity_id() {
                                window.activate_window();
                                workspace.update(cx, |workspace, cx| {
                                    prompt_for_open_path_and_open(workspace, app_state.clone(), options.clone(), true, window, cx);
                                });
                                return true;
                            }
                        }
...
impl ProjectItemRegistry {
    fn register<T: ProjectItem>(&mut self) {
        ...
        self.build_project_item_for_path_fns
            .push(|project, project_path, window, cx| {
                ...
                let project_item =
                    <T::Item as raijin_project::ProjectItem>::try_open(project, &project_path, cx)?;
                    ...
                    match project_item.await.with_context(|| {
                        ...
                        Ok(project_item) => {
                            ...
                            let project_entry_id: Option<ProjectEntryId> =
                                project_item.read_with(cx, raijin_project::ProjectItem::entry_id);
    ...
    fn build_item<T: raijin_project::ProjectItem>(
        &self,
        item: Entity<T>,
        project: Entity<Project>,
        pane: Option<&Pane>,
...
impl Workspace {
    pub fn new(
        ...
        cx.subscribe_in(&project, window, move |this, _, event, window, cx| {
            match event {
                ...
                raijin_project::Event::CollaboratorLeft(peer_id) => {
                    this.collaborator_left(*peer_id, window, cx);
                }

                &raijin_project::Event::WorktreeRemoved(_) => {
                    this.update_window_title(window, cx);
                    this.serialize_workspace(window, cx);
                    this.update_history(cx);
                }

                &raijin_project::Event::WorktreeAdded(id) => {
                    this.update_window_title(window, cx);
                    if this
                        .project()
                        .read(cx)
                ...
                raijin_project::Event::WorktreeUpdatedEntries(..) => {
                    this.update_window_title(window, cx);
                    this.serialize_workspace(window, cx);
                }
                ...
                raijin_project::Event::Closed => {
                    window.remove_window();
                }

                raijin_project::Event::DeletedEntry(_, entry_id) => {
                ...
                raijin_project::Event::HideToast { notification_id } => {
                    this.dismiss_notification(&NotificationId::named(notification_id.clone()), cx)
                }

                raijin_project::Event::LanguageServerPrompt(request) => {
                ...
                raijin_project::Event::AgentLocationChanged => {
        ...
        let status_bar = cx.new(|cx| {
            let mut status_bar =
                StatusBar::new(&center_pane.clone(), window, cx);
                ...
                'emit_winner: {
                    ...
                    this.update(cx, |workspace, cx| {
                        workspace.project.update(cx, |_, cx| {
                            cx.emit(raijin_project::Event::ActiveEntryChanged(Some(entry_id)));
                        });
                    })
                    .ok();
                }
    ...
    where
        T: ProjectItem,
    {
        use raijin_project::ProjectItem as _;
    ...
    pub fn follow(
        ...
        if let CollaboratorId::PeerId(peer_id) = leader_id {
            ...
            if let Some(project_id) = other_project_id {
                let app_state = self.app_state.clone();
                if let Some(opener) = workspace_opener(cx) {
                    opener.join_in_room_project(project_id, remote_participant.user.id, app_state, cx)
                        .detach_and_log_err(cx);
                }
            }
        }
    ...
    pub fn actions(&self, div: Div, window: &mut Window, cx: &mut Context<Self>) -> Div {
        self.add_workspace_actions_listeners(div, window, cx)
            .on_action(cx.listener(
                |_workspace, action_sequence: &inazuma_settings_framework::ActionSequence, window, cx| {
    ...
    pub fn cancel(&mut self, _: &inazuma_menu::Cancel, window: &mut Window, cx: &mut Context<Self>) {
    ...
    fn toggle_theme_mode(&mut self, _: &ToggleMode, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        let next_mode = match current_mode {
            Some(raijin_theme_settings::ThemeAppearanceMode::Light) => {
                raijin_theme_settings::ThemeAppearanceMode::Dark
            }
            Some(raijin_theme_settings::ThemeAppearanceMode::Dark) => {
                raijin_theme_settings::ThemeAppearanceMode::Light
            }
            Some(raijin_theme_settings::ThemeAppearanceMode::System) | None => {
                match cx.theme().appearance() {
                    raijin_theme::Appearance::Light => raijin_theme_settings::ThemeAppearanceMode::Dark,
                    raijin_theme::Appearance::Dark => raijin_theme_settings::ThemeAppearanceMode::Light,
                }
            }
        };
        ...
        inazuma_settings_framework::update_settings_file(fs, cx, move |settings, _cx| {
            raijin_theme_settings::set_mode(settings, next_mode);
        });
    }
...
async fn join_channel_internal(
    ...
    // this loop will terminate within raijin_client::CONNECTION_TIMEOUT seconds.
    'outer: loop {
...
pub fn join_channel(
    ...
    cx.spawn(async move |cx| {
        ...
        let mut active_window = requesting_window.or_else(|| {
...
pub fn open_remote_project_with_new_connection(
    ...
    cx.spawn(async move |cx| {
        ...
        let session = match cx
            .update(|cx| {
                raijin_remote::RemoteClient::new(
...
async fn open_remote_project_inner(
    ...
    let workspace = window.update(cx, |_, window, cx| {
        raijin_telemetry::event!("SSH Project Opened");
...
pub fn client_side_decorations(
    ...
    match decorations {
        Decorations::Client { .. } => window.set_client_inset(raijin_theme::CLIENT_SIDE_DECORATION_SHADOW),
        ...
        .map(|div| match decorations {
            ...
            Decorations::Client { .. } => div
                .when(
                    ...
                    |div| div.rounded_tr(raijin_theme::CLIENT_SIDE_DECORATION_ROUNDING),
                )
                .when(
                    !(tiling.top
                        || tiling.left
                    ...
                    |div| div.rounded_tl(raijin_theme::CLIENT_SIDE_DECORATION_ROUNDING),
                )
                .when(
                    !(tiling.bottom
                        || tiling.right
                        ...
                        || border_radius_tiling.right),
                    |div| div.rounded_br(raijin_theme::CLIENT_SIDE_DECORATION_ROUNDING),
                )
                .when(
                    !(tiling.bottom
                        || tiling.left
                        || border_radius_tiling.bottom
                        || border_radius_tiling.left),
                    |div| div.rounded_bl(raijin_theme::CLIENT_SIDE_DECORATION_ROUNDING),
                )
                .when(!tiling.top, |div| {
                    div.pt(raijin_theme::CLIENT_SIDE_DECORATION_SHADOW)
                })
                .when(!tiling.bottom, |div| {
                    div.pb(raijin_theme::CLIENT_SIDE_DECORATION_SHADOW)
                })
                .when(!tiling.left, |div| {
                    div.pl(raijin_theme::CLIENT_SIDE_DECORATION_SHADOW)
                })
                .when(!tiling.right, |div| {
                    div.pr(raijin_theme::CLIENT_SIDE_DECORATION_SHADOW)
                })
                .on_mouse_move(move |e, window, cx| {
                    ...
                    let new_edge =
                        resize_edge(pos, raijin_theme::CLIENT_SIDE_DECORATION_SHADOW, size, tiling);
                ...
                .on_mouse_down(MouseButton::Left, move |e, window, _| {
                    ...
                    let edge = match resize_edge(
                        pos,
                        raijin_theme::CLIENT_SIDE_DECORATION_SHADOW,
                        size,
                        tiling,
                    ) {
                        Some(value) => value,
                    ...
                    Decorations::Client { .. } => div
                        ...
                        .when(!tiling.is_tiled(), |div| {
                            div.shadow(vec![inazuma::BoxShadow {
                                ...
                                blur_radius: raijin_theme::CLIENT_SIDE_DECORATION_SHADOW / 2.,
                                spread_radius: px(0.),
            ...
            Decorations::Client { tiling, .. } => div.child(
                canvas(
                    ...
                    move |_bounds, hitbox, window, cx| {
                        ...
                        let size = window.window_bounds().get_bounds().size;
                        let Some(edge) =
                            resize_edge(mouse, raijin_theme::CLIENT_SIDE_DECORATION_SHADOW, size, tiling)
                        else {
                            return;
                        };
...
mod tests {
    ...
    use raijin_project::{Project, ProjectEntryId};
    use serde_json::json;
    use inazuma_settings_framework::SettingsStore;
    ...
    #[inazuma::test]
    async fn test_pane_navigation(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_navigation_history_deduplication(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_join_pane_into_next(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_join_all_panes(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
        let project = Project::test(fs, None, cx).await;
        let (workspace, cx) =
    ...
    mod register_project_item_tests {
        ...
        impl raijin_project::ProjectItem for TestPngItem {
            fn try_open(
        ...
        impl raijin_project::ProjectItem for TestIpynbItem {
            fn try_open(
                _project: &Entity<Project>,
                path: &ProjectPath,
                cx: &mut App,
    ...
    pub fn init_test(cx: &mut TestAppContext) {
        cx.update(|cx| {
            let settings_store = SettingsStore::test(cx);
            cx.set_global(settings_store);
            cx.set_global(raijin_db::AppDatabase::test_new());
            raijin_theme_settings::init(raijin_theme::LoadThemes::JustBase, cx);
        });
    }
    ...
    async fn test_toggle_theme_mode_persists_and_updates_active_theme(cx: &mut TestAppContext) {
        use inazuma_settings_framework::{ThemeName, ThemeSelection};
        use raijin_theme::SystemAppearance;
        use raijin_actions::theme::ToggleMode;

        init_test(cx);

        let fs = FakeFs::new(cx.executor());
        let settings_fs: Arc<dyn raijin_fs::Fs> = fs.clone();
        ...
        workspace.update_in(cx, |_workspace, _window, cx| {
            *SystemAppearance::global_mut(cx) = SystemAppearance(raijin_theme::Appearance::Light);
            inazuma_settings_framework::update_settings_file(settings_fs.clone(), cx, |settings, _cx| {
        ...
        let settings_text = SettingsStore::load_settings(&settings_fs).await.unwrap();
        let parsed: serde_json::Value = inazuma_settings_framework::parse_json_with_comments(&settings_text).unwrap();
        assert_eq!(
            parsed["theme"],
            serde_json::json!({
                "mode": "system",
