...
impl ModalView for SecurityModal {
    ...
    fn on_before_dismiss(&mut self, _: &mut Window, _: &mut Context<Self>) -> DismissDecision {
        match self.trusted {
            Some(false) => raijin_telemetry::event!("Open in Restricted", source = "Worktree Trust Modal"),
            Some(true) => raijin_telemetry::event!("Trust and Continue", source = "Worktree Trust Modal"),
            None => raijin_telemetry::event!("Dismissed", source = "Worktree Trust Modal"),
        }
                ...
                v_flex()
                    ...
                    .child(
                        v_flex()
                            ...
                            .child(
                                Label::new(
                                    "Review .raijin/settings.json for any extensions or commands configured by this project.",
                                )
                                .color(Color::Muted),
                            ),
                    )
