...
use raijin_project::{TaskSourceKind, WorktreeId};
use raijin_remote::ConnectionState;
use raijin_task::{
    DebugScenario, ResolvedTask, SaveStrategy, SharedTaskContext, SpawnInTerminal, TaskContext,
    TaskTemplate,
};
use raijin_ui::Window;
...
mod tests {
    ...
    use raijin_project::{FakeFs, Project, TaskSourceKind};
    use serde_json::json;
    use std::sync::Arc;
    ...
    async fn create_fixture(
        ...
        let task = template
            .resolve_task("test", &raijin_task::TaskContext::default())
            .unwrap();
    ...
    impl TerminalProvider for TestTerminalProvider {
        fn spawn(
            &self,
            _task: raijin_task::SpawnInTerminal,
