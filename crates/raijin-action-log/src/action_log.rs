...
use raijin_language::{Anchor, Buffer, BufferEvent, Point, ToOffset, ToPoint};
use raijin_project::{Project, ProjectItem, lsp_store::OpenLspBufferHandle};
use std::{
    cmp,
    ops::Range,
    path::{Path, PathBuf},
...
impl ActionLog {
    ...
    async fn maintain_diff(
        ...
        let (mut git_diff_updates_tx, mut git_diff_updates_rx) = raijin_watch::channel(());
            ...
            if let Some((git_diff, (buffer_repo, _))) = git_diff.as_ref().zip(buffer_repo) {
                cx.update(|cx| {
                    ...
                    Some(cx.subscribe(git_diff, move |_, event, cx| {
                        if let raijin_buffer_diff::BufferDiffEvent::DiffChanged { .. } = event {
        ...
        let (new_base_text, new_diff_base) = this
            .read_with(cx, |this, cx| {
                ...
                anyhow::Ok(cx.background_spawn(async move {
                    ...
                    let committed_edits = raijin_language::line_diff(
    ...
    pub fn keep_edits_in_range(
        &mut self,
        buffer: Entity<Buffer>,
        buffer_range: Range<impl raijin_language::ToPoint>,
    ...
    pub fn reject_edits_in_ranges(
        &mut self,
        buffer: Entity<Buffer>,
        buffer_ranges: Vec<Range<impl raijin_language::ToPoint>>,
        telemetry: Option<ActionLogTelemetry>,
        cx: &mut Context<Self>,
...
mod tests {
    ...
    use raijin_project::{FakeFs, Fs, Project, RemoveOptions};
    use rand::prelude::*;
    use serde_json::json;
