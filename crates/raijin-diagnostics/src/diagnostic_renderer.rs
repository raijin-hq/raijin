...
use raijin_language::{BufferId, Diagnostic, DiagnosticEntryRef, LanguageRegistry};
...
use raijin_ui::{CopyButton, prelude::*};
...
impl raijin_editor::DiagnosticRenderer for DiagnosticRenderer {
    fn render_group(
        &self,
        diagnostic_group: Vec<DiagnosticEntryRef<'_, Point>>,
        buffer_id: BufferId,
...
impl DiagnosticBlock {
    ...
    pub fn open_link(
        ...
        let Some(diagnostic_link) = link.strip_prefix("file://#diagnostic-") else {
            raijin_editor::hover_popover::open_markdown_url(link, window, cx);
            return;
        };
