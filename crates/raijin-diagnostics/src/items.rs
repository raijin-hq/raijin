...
use raijin_editor::{Editor, MultiBufferOffset};
use inazuma::{
    Context, Entity, EventEmitter, IntoElement, ParentElement, Render, Styled, Subscription, Task,
    WeakEntity, Window,
};
use raijin_language::Diagnostic;
use raijin_project::project_settings::{GoToDiagnosticSeverityFilter, ProjectSettings};
use inazuma_settings_framework::Settings;
use raijin_ui::{Button, ButtonLike, Color, Icon, IconName, Label, Tooltip, h_flex, prelude::*};
use inazuma_util::ResultExt;
use raijin_workspace::{StatusItemView, ToolbarItemEvent, Workspace, item::ItemHandle};

use crate::{Deploy, IncludeWarnings, ProjectDiagnosticsEditor};

/// The status bar item that displays diagnostic counts.
pub struct DiagnosticIndicator {
    summary: raijin_project::DiagnosticSummary,
            ...
            Some(
                Button::new("diagnostic_message", SharedString::new(message))
                    ...
                    .tooltip(|_window, cx| {
                        Tooltip::for_action(
                            "Next Diagnostic",
                            &raijin_editor::actions::GoToDiagnostic::default(),
                            cx,
                        )
                    })
                    .on_click(
...
impl DiagnosticIndicator {
    pub fn new(workspace: &Workspace, cx: &mut Context<Self>) -> Self {
        ...
        cx.subscribe(project, |this, project, event, cx| match event {
            raijin_project::Event::DiskBasedDiagnosticsStarted { .. } => {
                cx.notify();
            }

            raijin_project::Event::DiskBasedDiagnosticsFinished { .. }
            | raijin_project::Event::LanguageServerRemoved(_) => {
