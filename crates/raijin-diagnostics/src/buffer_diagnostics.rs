...
use raijin_language::{Buffer, Capability, DiagnosticEntry, DiagnosticEntryRef, Point};
use raijin_project::{
...
use raijin_ui::{Button, ButtonStyle, Icon, IconName, Label, Tooltip, h_flex, prelude::*};
...
impl BufferDiagnosticsEditor {
    ...
    fn update_excerpts(
        ...
        let max_severity = Self::max_diagnostics_severity(self.include_warnings)
            .into_lsp()
            .unwrap_or(raijin_lsp::DiagnosticSeverity::WARNING);
        ...
        cx.spawn_in(window, async move |buffer_diagnostics_editor, mut cx| {
            ...
            buffer_diagnostics_editor.update_in(cx, |buffer_diagnostics_editor, window, cx| {
                ...
                // `raijin_editor::test::editor_content_with_blocks` function can then
                ...
                #[cfg(test)]
                {
                    for (block_id, block) in block_ids.iter().zip(cloned_blocks.iter()) {
                        ...
                        raijin_editor::test::set_block_content_for_tests(
                            ...
                            *block_id,
                            ...
                            move |cx| {
                                markdown::MarkdownElement::rendered_text(
                                    markdown.clone(),
                                    cx,
                                    raijin_editor::hover_popover::diagnostics_markdown_style,
                                )
                            },
                        );
                    }
...
impl Item for BufferDiagnosticsEditor {
    ...
    fn save(
        &mut self,
        options: raijin_workspace::item::SaveOptions,
        project: Entity<Project>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Task<Result<()>> {
