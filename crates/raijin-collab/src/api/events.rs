...
use raijin_telemetry_events::{Event, EventRequestBody};
...
pub struct RaijinChecksumHeader(Vec<u8>);

impl Header for RaijinChecksumHeader {
    fn name() -> &'static HeaderName {
        static RAIJIN_CHECKSUM_HEADER: OnceLock<HeaderName> = OnceLock::new();
        RAIJIN_CHECKSUM_HEADER.get_or_init(|| HeaderName::from_static("x-raijin-checksum"))
    }
...
pub async fn post_events(
    ...
    TypedHeader(RaijinChecksumHeader(checksum)): TypedHeader<RaijinChecksumHeader>,
...
fn for_snowflake(
    ...
    body.events.into_iter().map(move |event| {
        ...
        // dictionary. See https://app.amplitude.com/data/raijin/Raijin/sources/detail/production/falcon%3A159998
