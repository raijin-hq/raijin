...
impl Database {
    ...
    async fn check_user_has_signed_cla(
        ...
        if let Some(channel) = channel {
            let requires_raijin_cla = channel.requires_raijin_cla
                || channel::Entity::find()
                    .filter(
                        channel::Column::Id
                            .is_in(channel.ancestors())
                            .and(channel::Column::RequiresRaijinCla.eq(true)),
                    )
                    .count(tx)
                    .await?
                    > 0;
            if requires_raijin_cla
                && contributor::Entity::find()
                    ...
                    .is_none()
            {
                Err(anyhow!("user has not signed the Raijin CLA"))?;
            }
        }
        Ok(())
    }
