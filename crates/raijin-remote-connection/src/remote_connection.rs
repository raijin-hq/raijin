...
use raijin_auto_update::AutoUpdater;
...
use raijin_remote::{ConnectionIdentifier, RemoteClient, RemoteConnectionOptions, RemotePlatform};
use semver::Version;
...
use raijin_ui_input::{ERASED_EDITOR_FACTORY, ErasedEditor};
            ...
            .child(
                div().w_full().py_1().child(
                    ListItem::new("li-devcontainer-go-back")
                        ...
                        .end_slot(
                            KeyBinding::for_action_in(&inazuma_menu::Cancel, &self.focus_handle(cx), cx)
...
impl raijin_remote::RemoteClientDelegate for RemoteClientDelegate {
    fn ask_password(
        &self,
        prompt: String,
        tx: oneshot::Sender<EncryptedPassword>,
...
pub fn connect(
    ...
    cx.spawn(async move |cx| {
        let connection = raijin_remote::connect(connection_options, delegate.clone(), cx);
        let connection = select! {
            _ = rx => return Ok(None),
            result = connection.fuse() => result,
        }?;

        cx.update(|cx| raijin_remote::RemoteClient::new(unique_identifier, connection, rx, delegate, cx))
            .await
    })
}
