...
use raijin_http_client::github::{AssetKind, GitHubLspBinaryVersion, latest_github_release};
...
use raijin_language::{ContextProvider, LspAdapter, LspAdapterDelegate};
use raijin_language::{LanguageName, ManifestName, ManifestProvider, ManifestQuery};
use raijin_language::{Toolchain, ToolchainList, ToolchainLister, ToolchainMetadata};
use raijin_lsp::{LanguageServerBinary, Uri};
use raijin_lsp::{LanguageServerBinaryOptions, LanguageServerName};
use raijin_node_runtime::{NodeRuntime, VersionStrategy};
use pet_core::Configuration;
use pet_core::os_environment::Environment;
use pet_core::python_environment::{PythonEnvironment, PythonEnvironmentKind};
use pet_virtualenv::is_virtualenv_dir;
use raijin_project::Fs;
use raijin_project::lsp_store::language_server_settings;
use semver::Version;
...
use raijin_http_client::github_download::{GithubBinaryMetadata, download_server_binary};
...
use raijin_task::{ShellKind, TaskTemplate, TaskTemplates, VariableName};
use inazuma_util::{ResultExt, maybe};

pub(crate) fn semantic_token_rules() -> SemanticTokenRules {
    let content = raijin_grammars::get_file("python/semantic_token_rules.json")
...
fn process_pyright_completions(items: &mut [raijin_lsp::CompletionItem]) {
    for item in items {
        ...
        let kind_priority = match item.kind {
            Some(raijin_lsp::CompletionItemKind::KEYWORD) => '0',
            Some(raijin_lsp::CompletionItemKind::ENUM_MEMBER) => '1',
            Some(raijin_lsp::CompletionItemKind::FIELD) => '2',
            Some(raijin_lsp::CompletionItemKind::PROPERTY) => '3',
            Some(raijin_lsp::CompletionItemKind::VARIABLE) => '4',
            Some(raijin_lsp::CompletionItemKind::CONSTANT) => '5',
            Some(raijin_lsp::CompletionItemKind::METHOD) => '6',
            Some(raijin_lsp::CompletionItemKind::FUNCTION) => '6',
            Some(raijin_lsp::CompletionItemKind::CLASS) => '7',
            Some(raijin_lsp::CompletionItemKind::MODULE) => '8',
...
fn label_for_pyright_completion(
    ...
    language: &Arc<raijin_language::Language>,
) -> Option<raijin_language::CodeLabel> {
    let label = &item.label;
    let label_len = label.len();
    let grammar = language.grammar()?;
    let highlight_id = match item.kind? {
        raijin_lsp::CompletionItemKind::METHOD => grammar.highlight_id_for_name("function.method"),
        raijin_lsp::CompletionItemKind::FUNCTION => grammar.highlight_id_for_name("function"),
        raijin_lsp::CompletionItemKind::CLASS => grammar.highlight_id_for_name("type"),
        raijin_lsp::CompletionItemKind::CONSTANT => grammar.highlight_id_for_name("constant"),
        raijin_lsp::CompletionItemKind::VARIABLE => grammar.highlight_id_for_name("variable"),
        _ => {
            return None;
        }
    };
    ...
    Some(raijin_language::CodeLabel::filtered(
        text,
        label_len,
...
fn label_for_python_symbol(
    ...
    let (text, filter_range, display_range) = match symbol.kind {
        raijin_lsp::SymbolKind::METHOD | raijin_lsp::SymbolKind::FUNCTION => {
            let text = format!("def {}():\n", name);
            let filter_range = 4..4 + name.len();
            let display_range = 0..filter_range.end;
            (text, filter_range, display_range)
        }
        raijin_lsp::SymbolKind::CLASS => {
            let text = format!("class {}:", name);
            let filter_range = 6..6 + name.len();
            let display_range = 0..filter_range.end;
            (text, filter_range, display_range)
        }
        raijin_lsp::SymbolKind::CONSTANT => {
    ...
    Some(raijin_language::CodeLabel::new(
...
impl LspAdapter for PyrightLspAdapter {
    ...
    async fn process_completions(&self, items: &mut [raijin_lsp::CompletionItem]) {
...
impl ContextProvider for PythonContextProvider {
    fn build_context(
        &self,
        variables: &raijin_task::TaskVariables,
        location: ContextLocation<'_>,
        _: Option<HashMap<String, String>>,
        toolchains: Arc<dyn LanguageToolchainStore>,
        cx: &mut inazuma::App,
    ) -> Task<Result<raijin_task::TaskVariables>> {
        ...
        cx.spawn(async move |cx| {
            ...
            Ok(raijin_task::TaskVariables::from_iter(
                test_target
                    .into_iter()
...
mod tests {
    ...
    use raijin_language::{AutoindentMode, Buffer};
    ...
    async fn test_conda_activation_script_injection(cx: &mut TestAppContext) {
        use raijin_language::{LanguageName, Toolchain, ToolchainLister};
        use inazuma_settings_framework::{CondaManager, VenvSettings};
        use raijin_task::ShellKind;
