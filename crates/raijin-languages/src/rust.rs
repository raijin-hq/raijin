...
use raijin_http_client::github::AssetKind;
use raijin_http_client::github::{GitHubLspBinaryVersion, latest_github_release};
use raijin_http_client::github_download::{GithubBinaryMetadata, download_server_binary};
pub use raijin_language::*;
use raijin_lsp::{InitializeParams, LanguageServerBinary, LanguageServerBinaryOptions};
use raijin_project::lsp_store::rust_analyzer_ext::CARGO_DIAGNOSTICS_SOURCE_NAME;
...
pub(crate) fn semantic_token_rules() -> SemanticTokenRules {
    let content = raijin_grammars::get_file("rust/semantic_token_rules.json")
...
impl LspAdapter for RustLspAdapter {
    ...
    fn process_diagnostics(&self, params: &mut raijin_lsp::PublishDiagnosticsParams, _: LanguageServerId) {
    ...
    async fn label_for_completion(
        &self,
        completion: &raijin_lsp::CompletionItem,
        ...
        let mut label = match (detail_right, completion.kind) {
            (Some(signature), Some(raijin_lsp::CompletionItemKind::FIELD)) => {
            ...
            (
                Some(signature),
                Some(raijin_lsp::CompletionItemKind::CONSTANT | raijin_lsp::CompletionItemKind::VARIABLE),
            ) if completion.insert_text_format != Some(raijin_lsp::InsertTextFormat::SNIPPET) => {
                let name = &completion.label;
                let text = format!("{name}: {signature}",);
                let prefix = "let ";
                let source = Rope::from_iter([prefix, &text, " = ();"]);
            ...
            (
                function_signature,
                Some(raijin_lsp::CompletionItemKind::FUNCTION | raijin_lsp::CompletionItemKind::METHOD),
            ) => {
            ...
            (_, kind) => {
                ...
                if completion.insert_text_format == Some(raijin_lsp::InsertTextFormat::SNIPPET)
                    && let Some(
                        raijin_lsp::CompletionTextEdit::InsertAndReplace(raijin_lsp::InsertReplaceEdit {
                            new_text,
                            ..
                        })
                        | raijin_lsp::CompletionTextEdit::Edit(raijin_lsp::TextEdit { new_text, .. }),
                    ) = completion.text_edit.as_ref()
                    && let Ok(mut snippet) = raijin_snippet::Snippet::parse(new_text)
                    && snippet.tabstops.len() > 1
                {
                    ...
                    let highlight_name = kind.and_then(|kind| match kind {
                        raijin_lsp::CompletionItemKind::STRUCT
                        | raijin_lsp::CompletionItemKind::INTERFACE
                        | raijin_lsp::CompletionItemKind::ENUM => Some("type"),
                        raijin_lsp::CompletionItemKind::ENUM_MEMBER => Some("variant"),
                        raijin_lsp::CompletionItemKind::KEYWORD => Some("keyword"),
                        raijin_lsp::CompletionItemKind::VALUE | raijin_lsp::CompletionItemKind::CONSTANT => {
                            Some("constant")
                        }
                        _ => None,
                    });
                    ...
                    if let Some(highlight_name) = highlight_name {
                        ...
                        && kind != Some(raijin_lsp::CompletionItemKind::SNIPPET)
                    {
                        return None;
                    }
                }
    ...
    async fn label_for_symbol(
        ...
        let (prefix, suffix) = match symbol.kind {
            raijin_lsp::SymbolKind::METHOD | raijin_lsp::SymbolKind::FUNCTION => ("fn ", "();"),
            raijin_lsp::SymbolKind::STRUCT => ("struct ", ";"),
            raijin_lsp::SymbolKind::ENUM => ("enum ", "{}"),
            raijin_lsp::SymbolKind::INTERFACE => ("trait ", "{}"),
            raijin_lsp::SymbolKind::CONSTANT => ("const ", ":()=();"),
            raijin_lsp::SymbolKind::MODULE => ("mod ", ";"),
            raijin_lsp::SymbolKind::PACKAGE => ("extern crate ", ";"),
            raijin_lsp::SymbolKind::TYPE_PARAMETER => ("type ", "=();"),
            raijin_lsp::SymbolKind::ENUM_MEMBER => {
                let prefix = "enum E {";
...
mod tests {
    ...
    use raijin_lsp::CompletionItemLabelDetails;
    ...
    async fn test_process_rust_diagnostics() {
        let mut params = raijin_lsp::PublishDiagnosticsParams {
            uri: raijin_lsp::Uri::from_file_path(path!("/a")).unwrap(),
            version: None,
            diagnostics: vec![
                // no newlines
    ...
    async fn test_rust_label_for_completion() {
        ...
        assert_eq!(
            adapter
                .label_for_completion(
                    &raijin_lsp::CompletionItem {
                        kind: Some(raijin_lsp::CompletionItemKind::FIELD),
                        label: "len".to_string(),
        ...
        assert_eq!(
            adapter
                .label_for_completion(
                    &raijin_lsp::CompletionItem {
                        kind: Some(raijin_lsp::CompletionItemKind::METHOD),
            ...
            adapter
                .label_for_completion(
                    &raijin_lsp::CompletionItem {
                        ...
                        insert_text_format: Some(raijin_lsp::InsertTextFormat::SNIPPET),
                        text_edit: Some(raijin_lsp::CompletionTextEdit::Edit(raijin_lsp::TextEdit {
                            range: raijin_lsp::Range::default(),
        ...
        let res = adapter
            .label_for_completion(
                &raijin_lsp::CompletionItem {
                    kind: Some(raijin_lsp::CompletionItemKind::STRUCT),
            ...
            adapter
                .label_for_symbol(
                    &raijin_language::Symbol {
                        ...
                        kind: raijin_lsp::SymbolKind::FUNCTION,
                        container_name: None,
                    },
                    &language
                )
            ...
            adapter
                .label_for_symbol(
                    &raijin_language::Symbol {
                        ...
                        kind: raijin_lsp::SymbolKind::TYPE_PARAMETER,
                        container_name: None,
                    },
                    &language
                )
            ...
            adapter
                .label_for_symbol(
                    &raijin_language::Symbol {
                        ...
                        kind: raijin_lsp::SymbolKind::ENUM_MEMBER,
                        container_name: None,
                    },
                    &language
                )
