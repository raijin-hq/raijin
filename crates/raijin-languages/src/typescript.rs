...
use raijin_lsp::{CodeActionKind, LanguageServerBinary, LanguageServerName, Uri};
use raijin_node_runtime::{NodeRuntime, VersionStrategy};
use raijin_project::{Fs, lsp_store::language_server_settings};
use semver::Version;
...
impl ContextProvider for TypeScriptContextProvider {
    fn associated_tasks(
        ...
        let Some(file) = raijin_project::File::from_dyn(file).cloned() else {
    ...
    fn build_context(
        &self,
        current_vars: &raijin_task::TaskVariables,
        ...
        let mut vars = raijin_task::TaskVariables::default();
...
mod tests {
    ...
    async fn test_outline(cx: &mut TestAppContext) {
        ...
        "#
            ...
            let buffer = cx.new(|cx| raijin_language::Buffer::local(text, cx).with_language(language, cx));
