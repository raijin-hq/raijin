...
use raijin_lsp::{InitializeParams, LanguageServerBinary, LanguageServerName};
use raijin_project::lsp_store::clangd_ext;
use serde_json::json;
use smol::fs;
...
impl super::LspAdapter for CLspAdapter {
    ...
    async fn label_for_completion(
        ...
        if !label_detail.is_empty() {
            let should_add_space = match completion.kind {
                Some(raijin_lsp::CompletionItemKind::FUNCTION | raijin_lsp::CompletionItemKind::METHOD) => false,
                _ => true,
            };
        ...
        match completion.kind {
            Some(raijin_lsp::CompletionItemKind::FIELD) if completion.detail.is_some() => {
            ...
            Some(kind) => {
                let highlight_name = match kind {
                    ...
                    | raijin_lsp::CompletionItemKind::CLASS
    ...
    async fn label_for_symbol(
        ...
        let (text, filter_range, display_range) = match symbol.kind {
            ...
            raijin_lsp::SymbolKind::STRUCT => {
                let text = format!("struct {} {{}}", name);
                let filter_range = 7..7 + name.len();
                let display_range = 0..filter_range.end;
                (text, filter_range, display_range)
            }
            raijin_lsp::SymbolKind::ENUM => {
                let text = format!("enum {} {{}}", name);
                let filter_range = 5..5 + name.len();
                let display_range = 0..filter_range.end;
                (text, filter_range, display_range)
            }
            raijin_lsp::SymbolKind::INTERFACE | raijin_lsp::SymbolKind::CLASS => {
            ...
            raijin_lsp::SymbolKind::MODULE => {
                let text = format!("namespace {} {{}}", name);
                let filter_range = 10..10 + name.len();
                let display_range = 0..filter_range.end;
                (text, filter_range, display_range)
            }
            raijin_lsp::SymbolKind::TYPE_PARAMETER => {
    ...
    fn underline_diagnostic(&self, diagnostic: &raijin_lsp::Diagnostic) -> bool {
