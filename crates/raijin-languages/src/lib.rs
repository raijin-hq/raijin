    ...
    std::sync::LazyLock::new(|| {
        Arc::new(Language::new(
            LanguageConfig {
                ...
                soft_wrap: Some(raijin_language::SoftWrap::EditorWidth),
...
pub fn init(languages: Arc<LanguageRegistry>, fs: Arc<dyn Fs>, node: NodeRuntime, cx: &mut App) {
    ...
    languages.register_native_grammars(raijin_grammars::native_grammars());
    ...
    for provider in manifest_providers {
        raijin_project::ManifestProvidersStore::global(cx).register(provider);
    }
}
...
fn register_language(
    ...
    languages.register_language(
        ...
        Arc::new(move || {
            Ok(LoadedLanguage {
                config: config.clone(),
                queries: raijin_grammars::load_queries(name),
...
pub fn language(name: &str, grammar: tree_sitter::Language) -> Arc<Language> {
    Arc::new(
        Language::new(raijin_grammars::load_config(name), Some(grammar))
            .with_queries(raijin_grammars::load_queries(name))
            .unwrap(),
    )
}
...
fn load_config(name: &str) -> LanguageConfig {
    ...
    raijin_grammars::load_config_for_feature(name, grammars_loaded)
}
