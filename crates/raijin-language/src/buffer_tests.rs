...
use raijin_proto::deserialize_operation;
use rand::prelude::*;
...
fn test_select_language(cx: &mut App) {
    ...
    assert_eq!(
        registry
            .language_for_file(&file("raijin/cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("raijin/a.cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("raijin/sumk"), None, cx)
            .map(|l| l.name()),
        None
    );
}
...
fn test_serialization(cx: &mut inazuma::App) {
    ...
    let buffer2 = cx.new(|cx| {
        ...
        buffer.apply_ops(
            ops.into_iter()
                .map(|op| raijin_proto::deserialize_operation(op).unwrap()),
            cx,
        );
        buffer
    });
            ...
            cx.subscribe(&cx.entity(), move |buffer, _, event, _| {
                ...
                } = event
                {
                    network.lock().broadcast(
                        buffer.replica_id(),
                        vec![raijin_proto::serialize_operation(operation)],
                    );
                }
            })
            .detach();
