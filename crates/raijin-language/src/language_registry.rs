...
pub struct FakeLanguageServerEntry {
    ...
    pub tx: futures::channel::mpsc::UnboundedSender<raijin_lsp::FakeLanguageServer>,
    pub _server: Option<raijin_lsp::FakeLanguageServer>,
}
...
impl LanguageRegistry {
    ...
    ) -> futures::channel::mpsc::UnboundedReceiver<raijin_lsp::FakeLanguageServer> {
    ...
    pub fn register_fake_lsp_server(
        ...
        initializer: Option<Box<dyn Fn(&mut raijin_lsp::FakeLanguageServer) + Send + Sync>>,
    ) -> futures::channel::mpsc::UnboundedReceiver<raijin_lsp::FakeLanguageServer> {
    ...
    fn get_or_load_grammar(
        ...
        let span = raijin_tracing::debug_span!("get_or_load_grammar", name = &*name.clone());
    ...
    pub fn create_fake_language_server(
        ...
        name: &LanguageServerName,
        binary: raijin_lsp::LanguageServerBinary,
        cx: &mut inazuma::AsyncApp,
    ) -> Option<raijin_lsp::LanguageServer> {
        let mut state = self.state.write();
        let fake_entry = state.fake_server_entries.get_mut(name)?;

        let (server, mut fake_server) = raijin_lsp::FakeLanguageServer::new(
            server_id,
            binary,
            name.0.to_string(),
