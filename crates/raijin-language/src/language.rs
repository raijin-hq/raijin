//! The `language` crate provides a large chunk of Raijin's language-related
...
pub use raijin_language_core::highlight_map::{HighlightId, HighlightMap};

pub use raijin_language_core::{
...
pub use raijin_lsp::{LanguageServerId, LanguageServerName};
pub use outline::*;
pub use syntax_map::{
...
pub(crate) fn to_settings_soft_wrap(value: raijin_language_core::SoftWrap) -> inazuma_settings_framework::SoftWrap {
    match value {
        raijin_language_core::SoftWrap::None => inazuma_settings_framework::SoftWrap::None,
        raijin_language_core::SoftWrap::PreferLine => inazuma_settings_framework::SoftWrap::PreferLine,
        raijin_language_core::SoftWrap::EditorWidth => inazuma_settings_framework::SoftWrap::EditorWidth,
        raijin_language_core::SoftWrap::PreferredLineLength => inazuma_settings_framework::SoftWrap::PreferredLineLength,
        raijin_language_core::SoftWrap::Bounded => inazuma_settings_framework::SoftWrap::Bounded,
    }
}
...
impl CachedLspAdapter {
    ...
    pub fn process_diagnostics(
        &self,
        params: &mut raijin_lsp::PublishDiagnosticsParams,
    ...
    pub fn underline_diagnostic(&self, diagnostic: &raijin_lsp::Diagnostic) -> bool {
    ...
    pub async fn process_completions(&self, completion_items: &mut [raijin_lsp::CompletionItem]) {
        self.adapter.process_completions(completion_items).await
    }

    pub async fn labels_for_completions(
        &self,
        completion_items: &[raijin_lsp::CompletionItem],
...
pub trait LspAdapter: 'static + Send + Sync + DynLspInstaller {
    ...
    fn process_diagnostics(&self, _: &mut raijin_lsp::PublishDiagnosticsParams, _: LanguageServerId) {}

    /// When processing new `raijin_lsp::PublishDiagnosticsParams` diagnostics, whether to retain previous one(s) or not.
    ...
    fn underline_diagnostic(&self, _diagnostic: &raijin_lsp::Diagnostic) -> bool {
        true
    }

    /// Post-processes completions provided by the language server.
    async fn process_completions(&self, _: &mut [raijin_lsp::CompletionItem]) {}
    ...
    /// Returns initialization options that are going to be sent to a LSP server as a part of [`raijin_lsp::InitializeParams`]
    ...
    /// for settings that should be persisted to Raijin's settings file.
    ...
    LI: LspInstaller<BinaryVersion = BinaryVersion> + LspAdapter,
{
    ...
    fn get_language_server_command(
        ...
        async move {
            ...
            //      worktree 3: no gopls found in PATH -> fallback to Raijin installation
            //
...
pub struct FakeLspAdapter {
    ...
    pub capabilities: raijin_lsp::ServerCapabilities,
    pub initializer: Option<Box<dyn 'static + Send + Sync + Fn(&mut raijin_lsp::FakeLanguageServer)>>,
    pub label_for_completion: Option<
        Box<
            dyn 'static
                + Send
                + Sync
                + Fn(&raijin_lsp::CompletionItem, &Arc<Language>) -> Option<CodeLabel>,
        >,
    >,
}
...
impl LanguageScope {
    ...
    ///
    /// By default, Raijin treats as paragraph and comment prefixes as boundaries.
    ...
    ///
    /// By default, Raijin treats alphanumeric characters (and '_') as word characters for
...
pub trait CodeLabelExt {
    fn fallback_for_completion(
        item: &raijin_lsp::CompletionItem,
...
impl CodeLabelExt for CodeLabel {
    fn fallback_for_completion(
        ...
        let highlight_id = item.kind.and_then(|kind| {
            ...
            use raijin_lsp::CompletionItemKind as Kind;
            match kind {
...
impl Default for FakeLspAdapter {
    fn default() -> Self {
        Self {
            ...
            capabilities: raijin_lsp::LanguageServer::full_capabilities(),
            initializer: None,
...
pub fn point_to_lsp(point: PointUtf16) -> raijin_lsp::Position {
    raijin_lsp::Position::new(point.row, point.column)
}

pub fn point_from_lsp(point: raijin_lsp::Position) -> Unclipped<PointUtf16> {
    Unclipped(PointUtf16::new(point.line, point.character))
}

pub fn range_to_lsp(range: Range<PointUtf16>) -> Result<raijin_lsp::Range> {
    anyhow::ensure!(
...
pub fn range_from_lsp(range: raijin_lsp::Range) -> Range<Unclipped<PointUtf16>> {
    ...
    if start > end {
        ...
        //
        // See issue #36223.
        raijin_log::debug!("range_from_lsp called with inverted range {start:?}-{end:?}");
...
mod tests {
    ...
    async fn test_completion_label_omits_duplicate_data() {
        let regular_completion_item_1 = raijin_lsp::CompletionItem {
            label: "regular1".to_string(),
            detail: Some("detail1".to_string()),
            label_details: Some(raijin_lsp::CompletionItemLabelDetails {
                detail: None,
                description: Some("description 1".to_string()),
            }),
            ..raijin_lsp::CompletionItem::default()
        };

        let regular_completion_item_2 = raijin_lsp::CompletionItem {
        ...
        let completion_item_with_duplicate_detail_and_proper_description = raijin_lsp::CompletionItem {
            detail: Some(regular_completion_item_1.label.clone()),
            ..regular_completion_item_1.clone()
        };

        let completion_item_with_duplicate_detail = raijin_lsp::CompletionItem {
            detail: Some(regular_completion_item_1.label.clone()),
            label_details: None,
            ..regular_completion_item_1.clone()
        };

        let completion_item_with_duplicate_description = raijin_lsp::CompletionItem {
            label_details: Some(raijin_lsp::CompletionItemLabelDetails {
                detail: None,
