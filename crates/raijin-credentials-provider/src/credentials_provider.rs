...
///
/// By default, running Raijin in development uses the development credentials
...
static RAIJIN_DEVELOPMENT_USE_KEYCHAIN: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("RAIJIN_DEVELOPMENT_USE_KEYCHAIN").is_ok_and(|value| !value.is_empty())
});
    ...
    fn new(cx: &App) -> Arc<Self> {
        let use_development_provider = match ReleaseChannel::try_global(cx) {
            Some(ReleaseChannel::Dev) => {
                ...
                //
                // However, if the `RAIJIN_DEVELOPMENT_USE_KEYCHAIN` environment
                // variable is set, we will use the actual keychain.
                !*RAIJIN_DEVELOPMENT_USE_KEYCHAIN
            }
...
/// re-allow access to the system keychain when developing Raijin.
...
impl DevelopmentCredentialsProvider {
    fn new() -> Self {
        let path = raijin_paths::config_dir().join("development_credentials");
