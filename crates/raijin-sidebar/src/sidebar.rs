use raijin_acp_thread::ThreadStatus;
use raijin_action_log::DiffStats;
use raijin_agent_client_protocol::{self as acp};
use raijin_agent_settings::AgentSettings;
use raijin_agent_ui::thread_metadata_store::{SidebarThreadMetadataStore, ThreadMetadata};
use raijin_agent_ui::threads_archive_view::{
...
use raijin_feature_flags::{AgentV2FeatureFlag, FeatureFlagViewExt as _};
use inazuma::{
...
use raijin_project::{Event as ProjectEvent, linked_worktree_short_name};
use raijin_recent_projects::sidebar_recent_projects::SidebarRecentProjects;
use raijin_ui::utils::platform_title_bar_height;
...
impl From<&ActiveThreadInfo> for raijin_acp_thread::AgentSessionInfo {
...
struct ThreadEntry {
    agent: Agent,
    session_info: raijin_acp_thread::AgentSessionInfo,
    icon: IconName,
    icon_from_external_svg: Option<SharedString>,
    status: AgentThreadStatus,
...
fn root_repository_snapshots(
    workspace: &Entity<Workspace>,
    cx: &App,
) -> impl Iterator<Item = raijin_project::git_store::RepositorySnapshot> {
...
impl Sidebar {
    ...
    fn subscribe_to_workspace(
        ...
        cx.subscribe_in(
            &git_store,
            window,
            |this, _, event: &raijin_project::git_store::GitStoreEvent, window, cx| {
                if matches!(
                    event,
                    raijin_project::git_store::GitStoreEvent::RepositoryUpdated(
                        _,
                        raijin_project::git_store::RepositoryEvent::GitWorktreeListChanged,
                        _,
                    )
                ) {
        ...
        cx.subscribe_in(
            workspace,
            window,
            |this, _workspace, event: &raijin_workspace::Event, window, cx| {
                if let raijin_workspace::Event::PanelAdded(view) = event {
                ...
                let menu = ContextMenu::build_persistent(window, cx, move |menu, _window, cx| {
                    ...
                    let menu = if workspace_count > 1 {
                        ...
                        menu.entry(
                            "Move to New Window",
                            Some(Box::new(
                                raijin_actions::agents_sidebar::MoveWorkspaceToNewWindow,
                            )),
                            move |window, cx| {
    ...
    fn focus_sidebar_filter(
        ...
        // immediately.
        if raijin_vim_mode_setting::VimModeSetting::get_global(cx).0 {
    ...
    fn toggle_selected_fold(
        &mut self,
        _: &raijin_editor::actions::ToggleFold,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(ix) = self.selection else { return };
    ...
    fn fold_all(
        &mut self,
        _: &raijin_editor::actions::FoldAll,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn unfold_all(
        &mut self,
        _: &raijin_editor::actions::UnfoldAll,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn render_filter_input(&self, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .flex_1()
            .capture_action(
                cx.listener(|this, _: &raijin_editor::actions::Newline, window, cx| {
        ...
        v_flex()
            ...
            .child(
                Button::new("open_project", "Open Project")
                    .full_width()
                    .key_binding(KeyBinding::for_action(&raijin_workspace::Open::default(), cx))
    ...
    fn render_sidebar_toggle_button(&self, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        sidebar_side_context_menu("sidebar-toggle-menu", _cx)
            .anchor(if on_right {
                ...
                inazuma::Corner::BottomLeft
            })
            .attach(if on_right {
                inazuma::Corner::TopRight
            } else {
                inazuma::Corner::TopLeft
            })
...
mod tests {
    use super::*;
    use raijin_acp_thread::StubAgentConnection;
    use raijin_agent::ThreadStore;
    use raijin_agent_ui::test_support::{active_session_id, open_thread_with_connection, send_message};
    ...
    ) -> Entity<raijin_project::Project> {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
        fs.insert_tree(worktree_path, serde_json::json!({ "src": {} }))
            .await;
        cx.update(|cx| <dyn raijin_fs::Fs>::set_global(fs.clone(), cx));
        raijin_project::Project::test(fs, [worktree_path.as_ref()], cx).await
    }
    ...
    async fn test_background_thread_completion_triggers_notification(cx: &mut TestAppContext) {
        ...
        let project_b = raijin_project::Project::test(fs, [], cx).await;
        multi_workspace.update_in(cx, |mw, window, cx| {
    ...
    async fn test_focused_thread_tracks_user_intent(cx: &mut TestAppContext) {
        ...
        fs.as_fake()
            ...
            .await;
        let project_b = raijin_project::Project::test(fs, ["/project-b".as_ref()], cx).await;
        ...
        sidebar.update_in(cx, |sidebar, window, cx| {
            sidebar.activate_thread(
                Agent::NativeAgent,
                raijin_acp_thread::AgentSessionInfo {
    ...
    async fn test_cmd_n_shows_new_thread_entry_in_absorbed_worktree(cx: &mut TestAppContext) {
        ...
        let main_project = raijin_project::Project::test(fs.clone(), ["/project".as_ref()], cx).await;
        let worktree_project =
            raijin_project::Project::test(fs.clone(), ["/wt-feature-a".as_ref()], cx).await;
    ...
    async fn init_test_project_with_git(
        ...
        .await;
        cx.update(|cx| <dyn raijin_fs::Fs>::set_global(fs.clone(), cx));
        let project = raijin_project::Project::test(fs.clone(), [worktree_path.as_ref()], cx).await;
        (project, fs)
    }
    ...
    async fn test_two_worktree_workspaces_absorbed_when_main_added(cx: &mut TestAppContext) {
        ...
        let project_a = raijin_project::Project::test(fs.clone(), ["/wt-feature-a".as_ref()], cx).await;
        let project_b = raijin_project::Project::test(fs.clone(), ["/wt-feature-b".as_ref()], cx).await;
    ...
    async fn test_activate_archived_thread_with_saved_paths_activates_matching_workspace(
        ...
        let project_a = raijin_project::Project::test(fs.clone(), ["/project-a".as_ref()], cx).await;
        let project_b = raijin_project::Project::test(fs.clone(), ["/project-b".as_ref()], cx).await;
    ...
    async fn test_activate_archived_thread_prefers_current_window_for_matching_paths(
        ...
        let project_b = raijin_project::Project::test(fs.clone(), ["/project-a".as_ref()], cx).await;
        let project_a = raijin_project::Project::test(fs.clone(), ["/project-a".as_ref()], cx).await;
    ...
    async fn test_linked_worktree_threads_not_duplicated_across_groups(cx: &mut TestAppContext) {
        ...
        let project_only = raijin_project::Project::test(fs.clone(), ["/project".as_ref()], cx).await;
        project_only
            .update(cx, |p, cx| p.git_scans_complete(cx))
            .await;

        let multi_root =
            raijin_project::Project::test(fs.clone(), ["/other".as_ref(), "/project".as_ref()], cx).await;
        multi_root
            .update(cx, |p, cx| p.git_scans_complete(cx))
            .await;
