...
use raijin_language::{Buffer, LanguageMatcher, LanguageName, LanguageRegistry};
use raijin_open_path_prompt::file_finder_settings::FileFinderSettings;
...
impl LanguageSelectorDelegate {
    ...
    fn language_icon(&self, matcher: &LanguageMatcher, cx: &App) -> Option<Icon> {
        matcher
            .path_suffixes
            .iter()
            .find_map(|extension| raijin_file_icons::FileIcons::get_icon(Path::new(extension), cx))
            .map(Icon::from_path)
