...
pub struct BufferDiff {
    ...
    inner: BufferDiffInner<Entity<raijin_language::Buffer>>,
    secondary_diff: Option<Entity<BufferDiff>>,
}

#[derive(Clone)]
pub struct BufferDiffSnapshot {
    inner: BufferDiffInner<raijin_language::BufferSnapshot>,
...
impl inazuma_sum_tree::SeekTarget<'_, DiffHunkSummary, DiffHunkSummary> for Anchor {
    fn cmp(&self, cursor_location: &DiffHunkSummary, buffer: &inazuma_text::BufferSnapshot) -> Ordering {
        if self
...
impl inazuma_sum_tree::SeekTarget<'_, DiffHunkSummary, DiffHunkSummary> for usize {
    fn cmp(&self, cursor_location: &DiffHunkSummary, _cx: &inazuma_text::BufferSnapshot) -> Ordering {
...
impl std::fmt::Debug for BufferDiffInner<raijin_language::BufferSnapshot> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BufferDiffSnapshot")
...
impl BufferDiffSnapshot {
    ...
    pub fn base_text(&self) -> &raijin_language::BufferSnapshot {
...
impl BufferDiffInner<Entity<raijin_language::Buffer>> {
...
impl BufferDiffInner<raijin_language::BufferSnapshot> {
...
fn build_diff_options(
    ...
    language_scope: Option<raijin_language::LanguageScope>,
    cx: &App,
) -> Option<DiffOptions> {
...
fn compute_hunks(
    ...
    if let Some((diff_base, diff_base_rope)) = diff_base {
        ...
        // A common case in Raijin is that the empty buffer is represented as just a newline,
...
fn compare_hunks(
    ...
    let extended_range = if has_changes && let Some(changed_range) = changed_range.clone() {
        let extended_start = *last_unchanged_new_hunk_end
            .unwrap_or(inazuma_text::Anchor::min_for_buffer(new_snapshot.remote_id()))
        ...
        let extended_end = *extended_end_candidate
            .unwrap_or(inazuma_text::Anchor::max_for_buffer(new_snapshot.remote_id()))
...
impl BufferDiff {
    pub fn new(buffer: &inazuma_text::BufferSnapshot, cx: &mut App) -> Self {
        let base_text = cx.new(|cx| {
            let mut buffer = raijin_language::Buffer::local("", cx);
    ...
    pub fn new_unchanged(buffer: &inazuma_text::BufferSnapshot, cx: &mut Context<Self>) -> Self {
        ...
        let base_text = cx.new(|cx| {
            let mut buffer = raijin_language::Buffer::local(base_text, cx);
            buffer.set_capability(Capability::ReadOnly, cx);
            buffer
        });
    ...
    pub fn base_text(&self, cx: &App) -> raijin_language::BufferSnapshot {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn recalculate_diff_sync(&mut self, buffer: &inazuma_text::BufferSnapshot, cx: &mut Context<Self>) {
    ...
    pub fn base_text_buffer(&self) -> &Entity<raijin_language::Buffer> {
        &self.inner.base_text
    }
}
...
mod tests {
    ...
    async fn test_stage_all_with_stale_buffer(cx: &mut TestAppContext) {
        // Regression test for RAIJIN-5R2: when the buffer is edited after the diff is
    ...
    async fn test_staging_and_unstaging_hunks(cx: &mut TestAppContext, mut rng: StdRng) {
        ...
        fn uncommitted_diff(
            working_copy: &raijin_language::BufferSnapshot,
            index_text: &Rope,
            head_text: String,
        ...
        let working_copy = cx.new(|cx| {
            raijin_language::Buffer::local_normalized(
    ...
    async fn test_changed_ranges(cx: &mut inazuma::TestAppContext) {
        ...
        "
        .unindent();
        let buffer = cx.new(|cx| raijin_language::Buffer::local(buffer_text, cx));
        let diff = cx.new(|cx| {
