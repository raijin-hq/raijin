...
use raijin_language_model::{LanguageModelEffortLevel, Speed};
use inazuma_settings_framework::update_settings_file;
use raijin_ui::{ButtonLike, SplitButton, SplitButtonStyle, Tab};
        ...
        let (error_kind, acp_error_code, message): (&str, Option<SharedString>, SharedString) =
            match error {
                ThreadError::PaymentRequired => (
                    "payment_required",
                    None,
                    "You reached your free usage limit. Upgrade to Raijin Pro for more prompts."
                        .into(),
                ),
                ThreadError::Refusal => {
                        ...
                        .handler({
                            move |window, cx| {
                                window.dispatch_action(
                                    raijin_actions::agent::AddSelectionToThread.boxed_clone(),
                                    cx,
                                );
                            }
                        }),
...
impl ThreadView {
    ...
    fn render_thread_controls(
            ...
            && self.thread.read(cx).connection().telemetry().is_some()
        {
            ...
            let tooltip_meta = || {
                SharedString::new(
                    "Rating the thread sends all of your current conversation to the Raijin team.",
                )
            };
                ...
                Button::new("open-wsl-modal", "Open in WSL").on_click(cx.listener({
                    move |_, _, _window, cx| {
                        ...
                        _window.dispatch_action(
                            raijin_actions::wsl_actions::OpenWsl::default().boxed_clone(),
                            cx,
                        );
                        cx.notify();
                    }
