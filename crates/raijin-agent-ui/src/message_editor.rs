...
use raijin_acp_thread::MentionUri;
...
use raijin_ui::{ContextMenu, Disclosure, ElevationIndex, prelude::*};
use inazuma_util::paths::PathStyle;
use inazuma_util::{ResultExt, debug_panic};
use raijin_workspace::{CollaboratorId, Workspace};
use raijin_actions::agent::{Chat, PasteRaw};
...
impl MessageEditor {
    ...
    fn insert_crease_impl(
        ...
        use raijin_editor::display_map::{Crease, FoldPlaceholder};
...
mod tests {
    ...
    use raijin_acp_thread::MentionUri;
    use raijin_agent::{ThreadStore, outline};
    use agent_client_protocol as acp;
    ...
    use raijin_language_model::LanguageModelRegistry;
    use raijin_lsp::{CompletionContext, CompletionTriggerKind};
    use parking_lot::RwLock;
    use raijin_project::{CompletionIntent, Project, ProjectPath};
    ...
    fn test_parse_mention_links() {
        // Single file mention
        let text = "[@bundle-mac](file:///Users/test/raijin/script/bundle-mac)";
    ...
    fn write_test_png_file(extension: Option<&str>) -> PathBuf {
        ...
        let file_name = match extension {
            Some(extension) => format!("raijin-agent-ui-test-{}.{}", uuid::Uuid::new_v4(), extension),
            None => format!("raijin-agent-ui-test-{}", uuid::Uuid::new_v4()),
        };
