...
use raijin_feature_flags::{AgentSharingFeatureFlag, AgentV2FeatureFlag, FeatureFlagAppExt as _};
...
use raijin_markdown::{Markdown, MarkdownElement, MarkdownFont, MarkdownStyle};
use parking_lot::RwLock;
use raijin_project::{AgentId, AgentServerStore, Project, ProjectEntryId};
use raijin_prompt_store::{PromptId, PromptStore};
...
impl ConversationView {
    ...
    fn spawn_external_agent_login(
        ...
        window.spawn(cx, async move |cx| {
            ...
            if let Some(cmd) = &task.command {
                // Have "node" command use Raijin's managed Node runtime by default
                if cmd == "node" {
    ...
    fn render_unsupported(
        ...
        let (heading_label, description_label) = (
            format!("Upgrade {} to work with Raijin", self.agent.agent_id()),
            if version.is_empty() {
                format!(
    ...
    fn current_model_name(&self, cx: &App) -> SharedString {
        // For native agent (Raijin Agent), use the specific model name (e.g., "Claude 3.5 Sonnet")
        ...
        fn connect(
            ...
            Task::ready(Err(anyhow!(
                "extracting downloaded asset for \
                 https://github.com/raijin-hq/codex-acp/releases/download/v0.9.4/\
