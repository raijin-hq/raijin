use raijin_auto_update::DismissMessage;
use raijin_editor::Editor;
use raijin_extension_host::{ExtensionOperation, ExtensionStore};
        ...
        let this = cx.new(|cx| {
            ...
            cx.spawn(async move |this, cx| {
                while let Some(job_event) = job_events.next().await {
                    this.update(cx, |this: &mut ActivityIndicator, cx| {
                        match job_event {
                            raijin_fs::JobEvent::Started { info } => {
                                this.fs_jobs.retain(|j| j.id != info.id);
                                this.fs_jobs.push(info);
                            }
                            raijin_fs::JobEvent::Completed { id } => {
            ...
            cx.subscribe(
                ...
                |activity_indicator, _, event, cx| {
                    if let LspStoreEvent::LanguageServerUpdate { name, message, .. } = event {
                        if let raijin_proto::update_language_server::Variant::StatusUpdate(status_update) =
                            message
                        {
                            ...
                            let status = match &status_update.status {
                                Some(raijin_proto::status_update::Status::Binary(binary_status)) => {
                                    if let Some(binary_status) =
                                        raijin_proto::ServerBinaryStatus::from_i32(*binary_status)
                                    {
                                        let binary_status = match binary_status {
                                            raijin_proto::ServerBinaryStatus::None => BinaryStatus::None,
                                            raijin_proto::ServerBinaryStatus::CheckingForUpdate => {
                                                BinaryStatus::CheckingForUpdate
                                            }
                                            raijin_proto::ServerBinaryStatus::Downloading => {
                                                BinaryStatus::Downloading
                                            }
                                            raijin_proto::ServerBinaryStatus::Starting => {
                                                BinaryStatus::Starting
                                            }
                                            raijin_proto::ServerBinaryStatus::Stopping => {
                                                BinaryStatus::Stopping
                                            }
                                            raijin_proto::ServerBinaryStatus::Stopped => {
                                                BinaryStatus::Stopped
                                            }
                                            raijin_proto::ServerBinaryStatus::Failed => {
                                ...
                                Some(raijin_proto::status_update::Status::Health(health_status)) => {
                                    if let Some(health) =
                                        raijin_proto::ServerHealth::from_i32(*health_status)
                                    {
                                        let health = match health {
                                            raijin_proto::ServerHealth::Ok => ServerHealth::Ok,
                                            raijin_proto::ServerHealth::Warning => ServerHealth::Warning,
                                            raijin_proto::ServerHealth::Error => ServerHealth::Error,
                                        };
            ...
            cx.subscribe(
                ...
                |_, _, event: &GitStoreEvent, cx| {
                    if let raijin_project::git_store::GitStoreEvent::JobsUpdated = event {
                        cx.notify()
                    }
                },
            )
        ...
        if let Some(message) = self.pending_environment_error(cx) {
            return Some(Content {
                ...
                on_click: Some(Arc::new(move |this, window, cx| {
                    ...
                    window.dispatch_action(Box::new(raijin_workspace::OpenLog), cx);
                })),
