...
impl PlatformDispatcher for LinuxDispatcher {
    fn get_all_timings(&self) -> Vec<inazuma::ThreadTaskTimings> {
        let global_timings = GLOBAL_THREAD_TIMINGS.lock();
        ThreadTaskTimings::convert(&global_timings)
    }

    fn get_current_thread_timings(&self) -> inazuma::ThreadTaskTimings {
...
impl<T> PriorityQueueCalloopSender<T> {
    ...
    fn send(&self, priority: Priority, item: T) -> Result<(), inazuma::queue::SendError<T>> {
