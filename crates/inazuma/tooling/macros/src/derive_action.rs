...
pub(crate) fn derive_action(input: TokenStream) -> TokenStream {
    ...
    */
    ...
    let build_fn_body = if no_json {
        ...
        quote! { Err(inazuma::private::anyhow::anyhow!(#error_msg)) }
    } else if is_unit_struct {
        quote! { Ok(Box::new(Self)) }
    } else {
        quote! { Ok(Box::new(inazuma::private::serde_json::from_value::<Self>(_value)?)) }
    };

    let json_schema_fn_body = if no_json || is_unit_struct {
        quote! { None }
    } else {
        quote! { Some(<Self as inazuma::private::schemars::JsonSchema>::json_schema(_generator)) }
    };
    ...
    TokenStream::from(quote! {
        ...
        impl inazuma::Action for #struct_name {
            ...
            fn partial_eq(&self, action: &dyn inazuma::Action) -> bool {
                action
                    .as_any()
                    .downcast_ref::<Self>()
            ...
            fn boxed_clone(&self) -> Box<dyn inazuma::Action> {
                Box::new(self.clone())
            }

            fn build(_value: inazuma::private::serde_json::Value) -> inazuma::Result<Box<dyn inazuma::Action>> {
                #build_fn_body
            }

            fn action_json_schema(
                _generator: &mut inazuma::private::schemars::SchemaGenerator,
            ) -> Option<inazuma::private::schemars::Schema> {
                #json_schema_fn_body
            }
