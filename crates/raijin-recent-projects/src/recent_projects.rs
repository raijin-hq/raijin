...
pub use raijin_remote_connection::{RemoteConnectionModal, connect};
...
use raijin_project::{Worktree, git_store::Repository};
    ...
    fn new(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            let workspaces = db
                ...
                .log_err()
                .unwrap_or_default();
            let workspaces = raijin_workspace::resolve_worktree_workspaces(workspaces, fs.as_ref()).await;
        ...
        if popover_style {
            return Some(
                v_flex()
                    ...
                    .border_color(cx.theme().colors().border_variant)
                    .child({
                        let open_action = raijin_workspace::Open {
                                ...
                                Some(ContextMenu::build(window, cx, {
                                    ...
                                    move |menu, _, _| {
                                        menu.context(focus_handle)
                                            .action(
                                                "Open Local Project",
                                                raijin_workspace::Open { create_new_window }.boxed_clone(),
                                            )
                                            .action(
                                                "Open Remote Project",
                                                OpenRemote {
            ...
            self.filtered_entries.get(ix)
        {
            ...
            cx.spawn_in(window, async move |this, cx| {
                ...
                let workspaces = db
                    ...
                    .unwrap_or_default();
                let workspaces =
                    raijin_workspace::resolve_worktree_workspaces(workspaces, fs.as_ref()).await;
                this.update_in(cx, move |picker, window, cx| {
                    ...
                    // But we do not emit a update event when user opens a project, because it's handled in `raijin_workspace::load_workspace`.
...
mod tests {
    ...
    use inazuma::{TestAppContext, UpdateGlobal, WindowHandle};
