//! Provides constructs for the Raijin app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("RAIJIN_RELEASE_CHANNEL")
            .unwrap_or_else(|_| include_str!("../../raijin-app/RELEASE_CHANNEL").trim().to_string())
    } else {
        include_str!("../../raijin-app/RELEASE_CHANNEL").trim().to_string()
    }
});
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Raijin-Dev",
        ReleaseChannel::Nightly => "Raijin-Nightly",
        ReleaseChannel::Preview => "Raijin-Preview",
        ReleaseChannel::Stable => "Raijin-Stable",
    }
}

/// The Git commit SHA that Raijin was built at.
...
impl AppVersion {
    ...
    pub fn load(
        ...
        let mut version: Version = if let Ok(from_env) = env::var("RAIJIN_APP_VERSION") {
            from_env.parse().expect("invalid RAIJIN_APP_VERSION")
        } else {
...
/// A Raijin release channel.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub enum ReleaseChannel {
    /// The development release channel.
    ///
    /// Used for local debug builds of Raijin.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for Raijin on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "dev.nyxb.Raijin-Dev",
            ReleaseChannel::Nightly => "dev.nyxb.Raijin-Nightly",
            ReleaseChannel::Preview => "dev.nyxb.Raijin-Preview",
            ReleaseChannel::Stable => "dev.nyxb.Raijin",
        }
    }
