...
pub static RAIJIN_DISABLE_STAFF: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("RAIJIN_DISABLE_STAFF").is_ok_and(|value| !value.is_empty() && value != "0")
});
...
impl FeatureFlags {
    fn has_flag<T: FeatureFlag>(&self) -> bool {
        ...
        if (cfg!(debug_assertions) || self.staff) && !*RAIJIN_DISABLE_STAFF && T::enabled_for_staff() {
            return true;
        }
...
/// so you can test flags being disabled, set RAIJIN_DISABLE_STAFF=1 in your environment,
/// which will force Raijin to treat the current user as non-staff.
...
impl FeatureFlagAppExt for App {
    ...
    fn has_flag<T: FeatureFlag>(&self) -> bool {
        self.try_global::<FeatureFlags>()
            ...
            .unwrap_or_else(|| {
                (cfg!(debug_assertions) && T::enabled_for_staff() && !*RAIJIN_DISABLE_STAFF)
                    || T::enabled_for_all()
            })
    }
