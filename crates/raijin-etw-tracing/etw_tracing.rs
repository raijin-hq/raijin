...
use raijin_workspace::notifications::simple_message_notification::MessageNotification;
use raijin_workspace::notifications::{NotificationId, show_app_notification};
use wprcontrol::*;
...
struct EtwSessionHandle {
    writer: raijin_net::OwnedWriteHalf,
    _listener: raijin_net::UnixListener,
    socket_path: PathBuf,
}
...
fn start_etw_recording(cx: &mut App, heap_pid: Option<u32>) {
    ...
    let save_dialog = cx.prompt_for_new_path(&PathBuf::default(), Some("raijin-trace.etl"));
...
const INSTANCE_NAME: &str = "Raijin";
...
fn heap_tracing_profile(heap_pid: Option<u32>) -> String {
    let (heap_provider, heap_collector) = match heap_pid {
        Some(pid) => (
            format!(
                r#"
    <HeapEventProvider Id="RaijinHeapProvider">
      ...
      <Collectors Operation="Add">
        <HeapEventCollectorId Value="HeapCollector_WPRHeapCollector">
          <HeapEventProviders Operation="Set">
            <HeapEventProviderId Value="RaijinHeapProvider"/>
          </HeapEventProviders>
...
<WindowsPerformanceRecorder Version="1.0" Author="Raijin HQ">
  <Profiles>
    {heap_provider}

    <Profile Id="RaijinHeap.Verbose.Memory" Base="Heap.Verbose.Memory" Name="RaijinHeap" DetailLevel="Verbose" LoggingMode="Memory" Description="Heap tracing">
      {heap_collector}
    </Profile>
  </Profiles>
...
fn build_profile_collection(heap_pid: Option<u32>) -> Result<IProfileCollection> {
    ...
    unsafe {
        ...
        collection
            ...
            .wpr_context(&collection)
            .context("Add RaijinHeap profile to collection")?;
    }
...
pub fn record_etw_trace(
    ...
    let mut stream = raijin_net::UnixStream::connect(socket_path).context("Connect to parent socket")?;
...
fn record_etw_trace_inner(
    heap_pid: Option<u32>,
    output_path: &Path,
    stream: &mut raijin_net::UnixStream,
) -> Result<()> {
...
fn receive_command(stream: &mut raijin_net::UnixStream) -> Result<(Command, bool)> {
...
pub struct EtwSession {
    output_path: PathBuf,
    stream: BufReader<raijin_net::UnixStream>,
    listener: raijin_net::UnixListener,
    socket_path: PathBuf,
}

pub fn launch_etw_recording(heap_pid: Option<u32>, output_path: &Path) -> Result<EtwSession> {
    let sock_path = std::env::temp_dir().join(format!("raijin-etw-{}.sock", std::process::id()));

    _ = std::fs::remove_file(&sock_path);
    let listener = raijin_net::UnixListener::bind(&sock_path).context("Bind Unix socket for ETW IPC")?;
