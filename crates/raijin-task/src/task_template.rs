...
use crate::{
    ...
    VariableName, RAIJIN_VARIABLE_NAME_PREFIX, serde_helpers::non_empty_string_vec,
};

/// A template definition of a Raijin task to run.
...
/// contains all relevant Raijin state in task variables.
...
impl TaskTemplate {
    ...
    /// Validates that all `$RAIJIN_*` variables used in this template are known
    /// variable names, returning a vector with all of the unique unknown
    /// variables.
    ///
    /// Note that `$RAIJIN_CUSTOM_*` variables are never considered to be invalid
    ...
    fn collect_unknown_variables(template: &str, unknown: &mut HashSet<String>) {
        shellexpand::env_with_context_no_errors(template, |variable| {
            ...
            // separated by a `:`, for example, `${RAIJIN_FILE:default_value} so we
            ...
            if variable_name.starts_with(RAIJIN_VARIABLE_NAME_PREFIX)
                && let without_prefix = &variable_name[RAIJIN_VARIABLE_NAME_PREFIX.len()..]
...
fn substitute_all_template_variables_in_str<A: AsRef<str>>(
    ...
    let substituted_string = shellexpand::env_with_context(template_str, |var| {
        ...
        } else if variable_name.starts_with(RAIJIN_VARIABLE_NAME_PREFIX) {
            // Unknown RAIJIN variable - use default if available
            if !default.is_empty() {
...
mod tests {
    ...
    fn test_template_variables_resolution() {
        ...
        for i in 0..all_variables.len() {
            ...
            assert!(
                ...
                "If any of the Raijin task variables is not substituted, the task should not be resolved, but got some resolution without the variable {removed_variable:?} (index {i})"
            );
        }
    }
    ...
    #[test]
    fn test_errors_on_missing_raijin_variable() {
        let task = TaskTemplate {
            label: "My task".into(),
            command: "echo".into(),
            args: vec!["$RAIJIN_VARIABLE".into()],
            ..TaskTemplate::default()
        };
        assert!(
    ...
    fn test_variable_default_values() {
        let task_with_defaults = TaskTemplate {
            ...
            args: vec![
                "${RAIJIN_MISSING_VAR:default_value}".to_string(),
                format!("${{{}}}", VariableName::Row.to_string() + ":42"),
            ],
            ..TaskTemplate::default()
        };

        // Test 1: When RAIJIN_FILE exists, should use actual value and ignore default
        ...
        assert_eq!(
            ...
            "echo actual_file.rs",
            "Should use actual RAIJIN_FILE value, not default"
        );
        assert_eq!(
        ...
        // Test 2: When RAIJIN_FILE doesn't exist, should use default value
        ...
        assert_eq!(
            resolved.resolved.command.unwrap(),
            "echo fallback.txt",
            "Should use default value when RAIJIN_FILE is missing"
        );
        assert_eq!(
            resolved.resolved.args,
        ...
        // Test 3: Missing RAIJIN variable without default should fail
        let task_no_default = TaskTemplate {
            label: "test no default".to_string(),
            command: "${RAIJIN_MISSING_NO_DEFAULT}".to_string(),
            ..TaskTemplate::default()
        };
        ...
        assert!(
            task_no_default
                .resolve_task(TEST_ID_BASE, &TaskContext::default())
                .is_none(),
            "Should fail when RAIJIN variable has no default and doesn't exist"
        );
    }

    #[test]
    fn test_unknown_variables() {
        // Variable names starting with `RAIJIN_` that are not valid should be
        // reported.
        let label = "test unknown variables".to_string();
        let command = "$RAIJIN_UNKNOWN".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        assert_eq!(task.unknown_variables(), vec!["RAIJIN_UNKNOWN".to_string()]);

        // Variable names starting with `RAIJIN_CUSTOM_` should never be reported,
        // as those are dynamically provided by extensions.
        let label = "test custom variables".to_string();
        let command = "$RAIJIN_CUSTOM_UNKNOWN".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        let label = "test custom variables".to_string();
        let command = "${RAIJIN_UNKNOWN:default_value}".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        let label = "test custom variables".to_string();
        let command = "$RAIJIN_FILE".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
