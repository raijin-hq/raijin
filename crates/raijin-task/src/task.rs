//! Baseline interface of Tasks in Raijin: all tasks in Raijin are intended to use those for implementing their own logic.
...
pub use debug_format::{
    ...
    Request, TcpArgumentsTemplate, RaijinDebugConfig,
};
...
pub use raijin_actions::RevealTarget;
...
/// Contains all information needed by Raijin to spawn a new terminal tab for the given task.
...
impl SpawnInTerminal {
    pub fn to_proto(&self) -> raijin_proto::SpawnInTerminal {
        raijin_proto::SpawnInTerminal {
    ...
    pub fn from_proto(proto: raijin_proto::SpawnInTerminal) -> Self {
        Self {
...
/// Variables, available for use in [`TaskContext`] when a Raijin's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
impl FromStr for VariableName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let without_prefix = s.strip_prefix(RAIJIN_VARIABLE_NAME_PREFIX).ok_or(())?;
        let value = match without_prefix {
            ...
            _ => {
                if let Some(custom_name) =
                    without_prefix.strip_prefix(RAIJIN_CUSTOM_VARIABLE_NAME_PREFIX)
                {
...
pub const RAIJIN_VARIABLE_NAME_PREFIX: &str = "RAIJIN_";
const RAIJIN_CUSTOM_VARIABLE_NAME_PREFIX: &str = "CUSTOM_";

impl std::fmt::Display for VariableName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::File => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}FILE"),
            Self::Filename => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}FILENAME"),
            Self::RelativeFile => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}RELATIVE_FILE"),
            Self::RelativeDir => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}RELATIVE_DIR"),
            Self::Dirname => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}DIRNAME"),
            Self::Stem => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}STEM"),
            Self::WorktreeRoot => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}WORKTREE_ROOT"),
            Self::Symbol => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}SYMBOL"),
            Self::Row => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}ROW"),
            Self::Column => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}COLUMN"),
            Self::SelectedText => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}SELECTED_TEXT"),
            Self::Language => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}LANGUAGE"),
            Self::RunnableSymbol => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}RUNNABLE_SYMBOL"),
            Self::PickProcessId => write!(f, "{RAIJIN_VARIABLE_NAME_PREFIX}PICK_PID"),
            Self::Custom(s) => write!(
                f,
                "{RAIJIN_VARIABLE_NAME_PREFIX}{RAIJIN_CUSTOM_VARIABLE_NAME_PREFIX}{s}"
            ),
        }
    }
}

/// Container for predefined environment variables that describe state of Raijin at the time the task was spawned.
...
/// Keeps all Raijin-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into Raijin.
...
pub fn shell_from_proto(proto: raijin_proto::Shell) -> anyhow::Result<Shell> {
    let shell_type = proto.shell_type.context("invalid shell type")?;
    let shell = match shell_type {
        raijin_proto::shell::ShellType::System(_) => Shell::System,
        raijin_proto::shell::ShellType::Program(program) => Shell::Program(program),
        raijin_proto::shell::ShellType::WithArguments(program) => Shell::WithArguments {
...
pub fn shell_to_proto(shell: Shell) -> raijin_proto::Shell {
    let shell_type = match shell {
        Shell::System => raijin_proto::shell::ShellType::System(raijin_proto::System {}),
        Shell::Program(program) => raijin_proto::shell::ShellType::Program(program),
        Shell::WithArguments {
            program,
            args,
            title_override: _,
        } => raijin_proto::shell::ShellType::WithArguments(raijin_proto::shell::WithArguments { program, args }),
    };
    raijin_proto::Shell {
...
type RaijinEnvVariable = String;

struct EnvVariableReplacer {
    variables: HashMap<VsCodeEnvVariable, RaijinEnvVariable>,
    commands: HashMap<VsCodeCommand, RaijinEnvVariable>,
}

impl EnvVariableReplacer {
    fn new(variables: HashMap<VsCodeEnvVariable, RaijinEnvVariable>) -> Self {
        Self {
            variables,
    ...
    fn with_commands(
        mut self,
        commands: impl IntoIterator<Item = (VsCodeCommand, RaijinEnvVariable)>,
    ) -> Self {
    ...
    // Replaces occurrences of VsCode-specific environment variables with Raijin equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->Raijin hit, perform a substitution
