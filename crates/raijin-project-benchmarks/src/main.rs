...
use anyhow::anyhow;
use raijin_askpass::EncryptedPassword;
use clap::Parser;
use raijin_client::{Client, UserStore};
use futures::channel::oneshot;
use inazuma::AppContext as _;
use raijin_http_client::FakeHttpClient;
...
use raijin_remote::{ConnectionIdentifier, RemoteClientDelegate, SshConnectionOptions};
use semver::Version;
...
impl RemoteClientDelegate for BenchmarkRemoteClient {
    ...
    fn get_download_url(
        &self,
        _platform: raijin_remote::RemotePlatform,
...
fn main() -> Result<(), anyhow::Error> {
    ...
    inazuma_platform::headless().run(|cx| {
        raijin_release_channel::init_test(semver::Version::new(0, 0, 0), ReleaseChannel::Dev, cx);
        ...
        let fs = Arc::new(RealFs::new(None, cx.background_executor().clone()));
            ...
            cx.spawn(async move |cx| {
                let project = if let Some(ssh_target) = args.ssh {
                    ...
                    let connection_options = raijin_remote::RemoteConnectionOptions::from(ssh_connection_options);
                    let delegate = Arc::new(BenchmarkRemoteClient);
                    let remote_connection = raijin_remote::connect(connection_options.clone(), delegate.clone(), cx).await.unwrap();

                    let (_tx, rx) = oneshot::channel();
                    let remote_client =  cx.update(|cx| raijin_remote::RemoteClient::new(ConnectionIdentifier::setup(), remote_connection, rx, delegate.clone(), cx )).await?.ok_or_else(|| anyhow!("ssh initialization returned None"))?;
