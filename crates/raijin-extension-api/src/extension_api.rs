//! The Raijin Rust Extension API allows you write extensions for Raijin in Rust.
...
pub use wit::{
    ...
    raijin::extension::github::{
        GithubRelease, GithubReleaseAsset, GithubReleaseOptions, github_release_by_tag_name,
        latest_github_release,
    },
    raijin::extension::nodejs::{
        node_binary_path, npm_install_package, npm_package_installed_version,
        npm_package_latest_version,
    },
    raijin::extension::platform::{Architecture, Os, current_platform},
    raijin::extension::slash_command::{
...
pub mod lsp {
    pub use crate::wit::raijin::extension::lsp::{
        Completion, CompletionKind, InsertTextFormat, Symbol, SymbolKind,
    };
}

/// A result returned from a Raijin extension.
...
pub trait Extension: Send + Sync {
    ...
    /// Locators are entities that convert a Raijin task into a debug scenario.
    ///
    /// They can be provided even by extensions that don't provide a debug adapter.
    /// For all tasks applicable to a given buffer, Raijin will query all locators to find one that can turn the task into a debug scenario.
    ...
    /// `dap_locator_create_scenario` if you're able to do that. Make sure to not fill out `build` field in that case, as that will prevent Raijin from running second phase of resolution in such case.
...
/// Registers the provided type as a Raijin extension.
///
...
#[unsafe(link_section = "raijin:api-version")]
#[doc(hidden)]
