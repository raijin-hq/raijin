use raijin_assets::Assets;
use inazuma::{Entity, KeyBinding, StyleRefinement, WindowOptions, prelude::*, rgb};
...
pub fn main() {
    env_logger::init();
    inazuma_platform::application().with_assets(Assets).run(|cx| {
        let store = SettingsStore::test(cx);
        cx.set_global(store);
        cx.bind_keys([KeyBinding::new("cmd-c", raijin_markdown::Copy, None)]);
