use clap::Parser;
use raijin_remote_server::Commands;
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(socket_path) = &cli.askpass {
        raijin_askpass::main(socket_path);
        return Ok(());
    }

    if let Some(socket) = &cli.crash_handler {
        raijin_crashes::crash_server(socket.as_path());
        return Ok(());
    }
    ...
    if let Some(command) = cli.command {
        use raijin_remote_server::ExecuteProxyError;

        let res = raijin_remote_server::run(command);
        if let Err(e) = &res
