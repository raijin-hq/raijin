...
use raijin_paths::{local_settings_file_relative_path, task_file_name};
...
pub const LSP_SETTINGS_SCHEMA_URL_PREFIX: &str = "raijin://schemas/settings/lsp/";
...
impl SettingsStore {
    ...
    pub async fn load_settings(fs: &Arc<dyn Fs>) -> Result<String> {
        match fs.load(raijin_paths::settings_file()).await {
            result @ Ok(_) => result,
            Err(err) => {
            ...
            .unbounded_send(Box::new(move |cx: AsyncApp| {
                async move {
                    let res = async move {
                        ...
                        let settings_path = raijin_paths::settings_file().as_path();
...
impl SettingsStore {
    ...
    pub fn set_default_settings(
        ...
        self.default_settings =
            toml::from_str(default_settings_content).context("failed to parse default settings TOML")?;
        self.recompute_values(None, cx);
        Ok(())
    }
    ...
    pub fn set_user_settings(
        ...
        let (settings, parse_result) = self.parse_settings::<UserSettingsContent>(
    ...
    pub fn set_global_settings(
        ...
        let (settings, parse_result) = self.parse_settings::<SettingsContent>(
    ...
    pub fn set_server_settings(
        ...
        let settings = if server_settings_content.is_empty() {
            ...
            Some(toml::from_str::<SettingsContent>(server_settings_content)
        ...
        match (path.clone(), kind, content) {
            ...
            (
                ...
                let (new_settings, parse_result) = self
                    .parse_settings::<ProjectSettingsContent>(
                        settings_contents,
...
mod tests {
    ...
    fn test_setting_store_assign_json_before_register(cx: &mut App) {
        ...
        store
            .set_user_settings("auto_update = false", cx)
            .unwrap();
        store.register_setting::<AutoUpdateSetting>();
    ...
    fn test_lsp_settings_schema_generation(cx: &mut App) {
        ...
        let schema = SettingsStore::json_schema(&SettingsJsonSchemaParams {
            ...
            font_names: &["Raijin Mono".to_string()],
            theme_names: &["One Dark".into()],
            icon_theme_names: &["Raijin Icons".into()],
            lsp_adapter_names: &[
        ...
        assert_eq!(
            init_options_ref,
            "raijin://schemas/settings/lsp/rust-analyzer/initialization_options"
        );
        ...
        assert_eq!(
            settings_ref,
            "raijin://schemas/settings/lsp/rust-analyzer/settings"
        );
    }
