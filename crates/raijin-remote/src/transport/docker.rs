...
use raijin_release_channel::{AppCommitSha, AppVersion, ReleaseChannel};
...
impl DockerExecConnection {
    ...
    async fn ensure_server_binary(
        ...
        let tmp_path_gz = raijin_paths::remote_server_dir_relative().join(
            RelPath::unix(&format!(
                "{}-download-{}.gz",
                binary_name,
                std::process::id()
    ...
    async fn upload_and_chown(
        ...
        anyhow::bail!(
            "failed to change ownership for raijin_remote_server via chown: {}",
            stderr,
        );
    }
