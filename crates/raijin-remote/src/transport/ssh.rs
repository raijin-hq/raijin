...
use raijin_paths::remote_server_dir_relative;
use raijin_release_channel::{AppVersion, ReleaseChannel};
use raijin_rpc::proto::Envelope;
use semver::Version;
...
struct SshSocket {
    ...
    #[cfg(windows)]
    _proxy: raijin_askpass::PasswordProxy,
}
...
impl SshRemoteConnection {
    pub(crate) async fn new(
        ...
        use raijin_askpass::AskPassResult;
        ...
        let mut askpass =
            raijin_askpass::AskPassSession::new(cx.background_executor().clone(), askpass_delegate)
                .await?;
        ...
        let socket = SshSocket::new(
            ...
            askpass
                .get_password()
                .or_else(|| raijin_askpass::EncryptedPassword::try_from("").ok())
    ...
    async fn ensure_server_binary(
        ...
        let dst_path =
            raijin_paths::remote_server_dir_relative().join(RelPath::unix(&binary_name).unwrap());
        ...
        .await?
        {
            let tmp_path = raijin_paths::remote_server_dir_relative().join(
                RelPath::unix(&format!(
                    "download-{}-{}",
                    std::process::id(),
...
impl SshSocket {
    ...
    async fn new(
        ...
        password: raijin_askpass::EncryptedPassword,
        ...
        let _proxy = raijin_askpass::PasswordProxy::new(Box::new(get_password), executor).await?;
