...
use raijin_syntax::DiagnosticSet;
#[cfg(not(target_family = "wasm"))]
use raijin_syntax::LanguageRegistry;
use crate::input::{
            ...
            let result = cx
                ...
                .spawn(async move {
                    ...
                    let injection_layers = if let Some(data) = injection_data {
                        raijin_syntax::SyntaxHighlighter::compute_injection_layers(
                            data, &new_tree, &text,
                        )
                    } else {
                        Default::default()
...
mod tests {
    ...
    struct InputView {
        ...
        window_handle: inazuma::WindowHandle<AppShell>,
    }
        ...
        pub fn new(cx: &mut TestAppContext) -> Self {
            ...
            let window = cx.update(|cx| {
                cx.open_window(Default::default(), |window, cx| {
                    ...
                    cx.new(|cx| crate::AppShell::new(input.clone().unwrap(), window, cx))
                })
                .unwrap()
            });
    ...
    fn test_highlighting_preserved_after_fold(cx: &mut TestAppContext) {
        use raijin_syntax::HighlightTheme;
