...
impl TextElement {
    ...
    pub(super) fn paint_selections_and_highlights(
        ...
        let secondary_selection = {
            let mut c = cx.theme().colors().element_selection;
            c.c *= 0.1;
            c
        };
        for (path, is_active) in prepaint.search_match_paths.iter() {
            ...
            if *is_active {
                window.paint_path(path.clone(), cx.theme().colors().element_selection);
            }
        }

        if let Some(path) = prepaint.selection_path.take() {
            window.paint_path(path, cx.theme().colors().element_selection);
        }
