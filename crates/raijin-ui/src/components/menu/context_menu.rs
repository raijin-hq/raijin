...
pub trait ContextMenuExt: InteractiveElement + ParentElement + Styled {
        ...
        Self: Sized,
    {
        ...
        WithContextMenu::new(id, self).menu(f)
    }
}

impl<E: InteractiveElement + ParentElement + Styled> ContextMenuExt for E {}

/// An element wrapper that shows a context menu on right-click.
pub struct WithContextMenu<E: ParentElement + Styled + Sized> {
    id: ElementId,
    element: Option<E>,
...
impl<E: ParentElement + Styled> WithContextMenu<E> {
    ...
    fn with_element_state<R>(
        ...
        window: &mut Window,
        cx: &mut App,
        f: impl FnOnce(&mut Self, &mut WithContextMenuState, &mut Window, &mut App) -> R,
    ) -> R {
        window.with_optional_element_state::<WithContextMenuState, _>(
            Some(id),
...
impl<E: ParentElement + Styled> ParentElement for WithContextMenu<E> {
...
impl<E: ParentElement + Styled> Styled for WithContextMenu<E> {
...
impl<E: ParentElement + Styled + IntoElement + 'static> IntoElement for WithContextMenu<E> {
    type Element = Self;
...
struct WithContextMenuSharedState {
...
pub struct WithContextMenuState {
    element: Option<AnyElement>,
    shared_state: Rc<RefCell<WithContextMenuSharedState>>,
}

impl Default for WithContextMenuState {
    fn default() -> Self {
        Self {
            element: None,
            shared_state: Rc::new(RefCell::new(WithContextMenuSharedState {
                menu_view: None,
                open: false,
...
impl<E: ParentElement + Styled + IntoElement + 'static> Element for WithContextMenu<E> {
    type RequestLayoutState = WithContextMenuState;
    ...
    fn request_layout(
        &mut self,
        id: Option<&inazuma::GlobalElementId>,
        ...
        self.with_element_state(
            ...
            window,
            cx,
            |this, state: &mut WithContextMenuState, window, cx| {
                let (position, open) = {
    ...
    fn paint(
        ...
        self.with_element_state(
            ...
            window,
            cx,
            |_view, state: &mut WithContextMenuState, window, _| {
