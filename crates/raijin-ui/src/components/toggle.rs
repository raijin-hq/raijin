use inazuma::{
    AnyElement, AnyView, ClickEvent, Edges, ElementId, Oklch, IntoElement, KeybindingKeystroke,
    Keystroke, Styled, Window, div, oklch, oklcha, prelude::*,
};
...
impl Checkbox {
    fn bg_color(&self, cx: &App) -> Oklch {
        ...
        match (style, self.filled) {
            ...
            (ToggleStyle::ElevationBased(_), false) => inazuma::transparent_black(),
            (ToggleStyle::ElevationBased(elevation), true) => elevation.darker_bg(cx),
            (ToggleStyle::Custom(_), false) => inazuma::transparent_black(),
...
impl SwitchColor {
    fn get_colors(&self, is_on: bool, cx: &App) -> (Oklch, Oklch) {
        if !is_on {
            return (
        ...
        match self {
            SwitchColor::Accent => {
                ...
                (status.info.color.opacity(0.4), colors.text_accent.opacity(0.2))
            }
                ...
                |this, tab_index| {
                    this.tab_index(tab_index)
                        .focus_visible(|mut style| {
                            let c = cx.theme().colors().border_focused;
                            style.border_colors = Some(Edges { top: Some(c), right: Some(c), bottom: Some(c), left: Some(c) });
                            style
                        })
...
/// use raijin_ui::{SwitchField, ToggleState};
///
                        ...
                        vec![
                            ...
                            single_example(
                                ...
                                Switch::new("switch_custom_style", ToggleState::Selected)
                                    .color(SwitchColor::Custom(oklch(0.6716, 0.2099, 327.5925)))
                                    .on_click(|_, _, _cx| {})
                                    .into_any_element(),
                            ),
                        ],
