...
async fn stream_response_archive(
    ...
    match asset_kind {
        ...
        AssetKind::Zip => {
            inazuma_util::archive::extract_zip(destination_path, response).await?;
        }
    };
    Ok(())
}
...
async fn stream_file_archive(
    ...
    match asset_kind {
        ...
        #[cfg(not(windows))]
        AssetKind::Zip => {
            inazuma_util::archive::extract_seekable_zip(destination_path, file_archive).await?;
        }
        #[cfg(windows)]
        AssetKind::Zip => {
            inazuma_util::archive::extract_zip(destination_path, file_archive).await?;
        }
    };
    Ok(())
}
