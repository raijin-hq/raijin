    ...
    T: strum::VariantArray + strum::VariantNames + Copy + PartialEq + Send + Sync + 'static,
{
    ...
    on_change: Rc<dyn Fn(T, &mut raijin_ui::Window, &mut App) + 'static>,
}
    ...
    T: strum::VariantArray + strum::VariantNames + Copy + PartialEq + Send + Sync + 'static,
{
    pub fn new(
        ...
        variants: &'static [T],
        labels: &'static [&'static str],
        on_change: impl Fn(T, &mut raijin_ui::Window, &mut App) + 'static,
    ) -> Self {
        Self {
            id: id.into(),
            current_value,
...
where
    T: strum::VariantArray + strum::VariantNames + Copy + PartialEq + Send + Sync + 'static,
{
    fn render(self, window: &mut raijin_ui::Window, cx: &mut raijin_ui::App) -> impl inazuma::IntoElement {
