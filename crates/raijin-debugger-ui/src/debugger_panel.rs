...
use raijin_dap::{DapRegistry, StartDebuggingRequestArguments};
use raijin_dap::{client::SessionId, debugger_settings::DebuggerSettings};
use raijin_editor::{Editor, MultiBufferOffset, ToPoint};
...
use raijin_project::debugger::session::{Session, SessionQuirks, SessionState, SessionStateEvent};
use raijin_project::{DebugScenarioContext, Fs, ProjectPath, TaskSourceKind, WorktreeId};
use raijin_project::{Project, debugger::session::ThreadStatus};
use raijin_rpc::proto::{self};
use inazuma_settings_framework::Settings;
use std::sync::{Arc, LazyLock};
use raijin_task::{DebugScenario, SharedTaskContext};
...
use raijin_workspace::SplitDirection;
use raijin_workspace::item::SaveOptions;
use raijin_workspace::{
    Item, Pane, Workspace,
    dock::{DockPosition, Panel, PanelEvent},
};
use raijin_actions::debug_panel::ToggleFocus;
    ...
    pub(crate) fn top_controls_strip(
        ...
        let documentation_button = || {
            IconButton::new("debug-open-documentation", IconName::CircleHelp)
                .icon_size(IconSize::Small)
                .on_click(move |_, _, cx| cx.open_url("https://raijin.dev/docs/debugger"))
                ...
                cx.spawn_in(window, async move |workspace, cx| {
                    ...
                    let project_path = workspace.update(cx, |workspace, cx| {
                        workspace
                            ...
                            .context(
                                "Couldn't get project path for .raijin/debug.json in active worktree",
                            )
                    })??;
                    ...
                    let welcome_experience = v_flex()
                        ...
                        .child(
                            Button::new("open-debugger-docs", "Debugger Docs")
                                ...
                                .on_click(|_, _, cx| cx.open_url("https://raijin.dev/docs/debugger")),
                        )
                        .child(
                            Button::new(
