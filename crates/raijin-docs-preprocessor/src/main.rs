...
const FRONT_MATTER_COMMENT: &str = "<!-- RAIJIN_META {} -->";

fn main() -> Result<()> {
    raijin_log::init();
    raijin_log::init_output_stderr();
    ...
    for_each_labeled_code_block_mut(book, errors, &|label, snippet_json| {
        ...
        match label {
            "settings" => {
                ...
                let value =
                    inazuma_settings_framework::parse_json_with_comments::<serde_json::Value>(&snippet_json_fixed)?;
            ...
            "debug" => {
                ...
                inazuma_settings_framework::parse_json_with_comments::<raijin_task::DebugTaskFile>(&snippet_json_fixed)?;
            }
            "tasks" => {
                if !snippet_json_fixed.starts_with('[') || !snippet_json_fixed.ends_with(']') {
                    snippet_json_fixed.insert(0, '[');
                ...
                inazuma_settings_framework::parse_json_with_comments::<raijin_task::TaskTemplates>(&snippet_json_fixed)?;
            }
            "icon-theme" => {
                if !snippet_json_fixed.starts_with('{') || !snippet_json_fixed.ends_with('}') {
                    snippet_json_fixed.insert(0, '{');
                ...
                inazuma_settings_framework::parse_json_with_comments::<raijin_theme::IconThemeFamilyContent>(
                    &snippet_json_fixed,
                )?;
            }
            "semantic_token_rules" => {
                ...
                inazuma_settings_framework::parse_json_with_comments::<inazuma_settings_framework::SemanticTokenRules>(
                    &snippet_json_fixed,
                )?;
            }
...
fn handle_postprocessing() -> Result<()> {
    let logger = raijin_log::scoped!("render");
    ...
    let raijin_html = output.remove("raijin-html").expect("raijin-html output defined");
    let default_description = raijin_html
        .get("default-description")
        .expect("Default description not found")
        .as_str()
        .expect("Default description not a string")
        .to_string();
    let default_title = raijin_html
        .get("default-title")
    ...
    output.insert("html".to_string(), raijin_html);
        ...
        for entry in std::fs::read_dir(&dir).context("failed to read docs dir")? {
                ...
                )
            {
                if ignore_list.contains(&&*entry.file_name().to_string_lossy()) {
                    raijin_log::info!(logger => "Ignoring {}", entry.path().to_string_lossy());
                } else {
    ...
    raijin_log::info!(logger => "Processing {} `.html` files", files.len());
    ...
    for file in files {
        ...
        let contents = meta_regex.replace(&contents, |caps: &regex::Captures| {
            ...
            for (kind, content) in metadata {
                match kind.as_str() {
                    ...
                    _ => {
                        raijin_log::warn!(logger => "Unrecognized frontmatter key: {} in {:?}", kind, pretty_path(&file, &root_dir));
                    }
                }
            }
            String::new()
        ...
        let meta_description = meta_description.as_ref().unwrap_or_else(|| {
            raijin_log::warn!(logger => "No meta description found for {:?}", pretty_path(&file, &root_dir));
            &default_description
        });
        let page_title = extract_title_from_page(&contents, pretty_path(&file, &root_dir));
        let meta_title = meta_title.as_ref().unwrap_or_else(|| {
            raijin_log::debug!(logger => "No meta title found for {:?}", pretty_path(&file, &root_dir));
            &default_title
        });
        let meta_title = format!("{} | {}", page_title, meta_title);
        raijin_log::trace!(logger => "Updating {:?}", pretty_path(&file, &root_dir));
