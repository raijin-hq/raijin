...
use raijin_acp_thread::{AcpThread, AgentThreadEntry, ToolCall, ToolCallStatus};
...
pub fn get_raijin_path() -> PathBuf {
    let mut raijin_path = std::env::current_exe().unwrap();
    ...
    raijin_path.push("raijin");

    if !raijin_path.exists() {
