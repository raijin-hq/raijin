...
use inazuma::{AsyncApp, ScreenCaptureStream};
use livekit::track;
use livekit::webrtc::{
    prelude::NV12Buffer,
...
pub struct WaylandScreenCaptureStream {
    ...
    _capture_task: inazuma::Task<()>,
}

impl WaylandScreenCaptureStream {
    pub fn new(stop_flag: Arc<AtomicBool>, capture_task: inazuma::Task<()>) -> Self {
        Self {
...
impl ScreenCaptureStream for WaylandScreenCaptureStream {
    fn metadata(&self) -> Result<inazuma::SourceMetadata> {
        Ok(inazuma::SourceMetadata {
            id: self.id,
            label: None,
            is_main: None,
            resolution: inazuma::size(inazuma::DevicePixels(1), inazuma::DevicePixels(1)),
        })
    }
}
