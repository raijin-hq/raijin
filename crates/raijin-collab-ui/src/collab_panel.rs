...
use raijin_channel::{Channel, ChannelEvent, ChannelStore};
use raijin_client::{ChannelId, Client, Contact, User, UserStore};
...
pub fn init(cx: &mut App) {
    cx.observe_new(|workspace: &mut Workspace, _, _| {
        ...
        workspace.register_action(|workspace, _: &CopyRoomId, window, cx| {
            use raijin_workspace::notifications::{NotificationId, NotifyTaskExt as _};
                    ...
                    window.handler_for(&this, move |_, window, cx| {
                        ActiveCall::global(cx)
                            ...
                            .detach_and_prompt_err("Failed to grant write access", window, cx, |e, _, _| {
                                match e.error_code() {
                                    ErrorCode::NeedsCla => Some("This user has not yet signed the CLA at https://raijin.dev/cla.".into()),
                                    _ => None,
                                }
                            })
                    }),
