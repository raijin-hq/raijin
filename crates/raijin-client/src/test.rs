...
use raijin_cloud_api_client::{
    AuthenticatedUser, GetAuthenticatedUserResponse, KnownOrUnknown, Plan, PlanInfo,
};
use raijin_cloud_llm_client::{CurrentUsage, UsageData, UsageLimit};
use futures::{StreamExt, stream::BoxStream};
use inazuma::{AppContext as _, Entity, TestAppContext};
use raijin_http_client::{AsyncBody, Method, Request, http};
use parking_lot::Mutex;
use raijin_rpc::{ConnectionId, Peer, Receipt, TypedEnvelope, proto};
                ...
                async move {
                    match (req.method(), req.uri().path()) {
                        (&Method::GET, "/client/users/me") => {
                            ...
                            {
                                return Ok(raijin_http_client::Response::builder()
                                    .status(401)
