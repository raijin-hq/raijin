...
use raijin_http_client::{self, AsyncBody, HttpClient, HttpClientWithUrl, Method, Request};
...
use raijin_telemetry_events::{AssistantEventData, AssistantPhase, Event, EventRequestBody, EventWrapper};
...
use raijin_worktree::{UpdatedEntriesSet, WorktreeId};
...
static RAIJIN_CLIENT_CHECKSUM_SEED: LazyLock<Option<Vec<u8>>> = LazyLock::new(|| {
    option_env!("RAIJIN_CLIENT_CHECKSUM_SEED")
        .map(|s| s.as_bytes().into())
        .or_else(|| {
            env::var("RAIJIN_CLIENT_CHECKSUM_SEED")
                .ok()
                .map(|s| s.as_bytes().into())
        })
});
...
pub static MINIDUMP_ENDPOINT: LazyLock<Option<String>> = LazyLock::new(|| {
    option_env!("RAIJIN_MINIDUMP_ENDPOINT")
        .map(str::to_string)
        .or_else(|| env::var("RAIJIN_MINIDUMP_ENDPOINT").ok())
});
...
impl Telemetry {
    pub fn new(
        ...
        let state = Arc::new(Mutex::new(TelemetryState {
            ...
            os_version: None,
            os_name: os_name(),
            app_version: raijin_release_channel::AppVersion::global(cx).to_string(),
            subscribers: Vec::new(),
        }));
    ...
    fn shutdown_telemetry(self: &Arc<Self>) -> impl Future<Output = ()> + use<> {
        raijin_telemetry::event!("App Closed");
        // TODO: close final edit period and make sure it's sent
        Task::ready(())
    }

    pub fn log_file_path() -> PathBuf {
        raijin_paths::logs_dir().join("telemetry.log")
    }
    ...
    pub fn has_checksum_seed(&self) -> bool {
        RAIJIN_CLIENT_CHECKSUM_SEED.is_some()
    }
    ...
    pub fn start(
        ...
        state.app_version = raijin_release_channel::AppVersion::global(cx).to_string();
    ...
    pub fn report_discovered_project_type_events(
        ...
        for project_type in project_types {
            raijin_telemetry::event!("Project Opened", project_type = project_type);
        }
    }
    ...
    fn report_event(self: &Arc<Self>, mut event: Event) {
        ...
        match &mut event {
            Event::Flexible(event) => event
                .event_properties
                .insert("event_source".into(), "raijin".into()),
        };
    ...
    fn build_request(
        ...
        Ok(Request::builder()
            ...
            .uri(
                self.http_client
                    .build_raijin_api_url("/telemetry/events", &[])?
                    .as_ref(),
            )
            .header("Content-Type", "application/json")
            .header("x-raijin-checksum", checksum)
...
pub fn calculate_json_checksum(json: &impl AsRef<[u8]>) -> Option<String> {
    let checksum_seed = RAIJIN_CLIENT_CHECKSUM_SEED.as_ref()?;
