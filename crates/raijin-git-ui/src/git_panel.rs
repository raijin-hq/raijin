...
use raijin_askpass::AskPassDelegate;
...
use raijin_editor::{EditorStyle, RewrapOptions};
use raijin_feature_flags::{FeatureFlagAppExt as _, GitGraphFeatureFlag};
use raijin_file_icons::FileIcons;
use futures::StreamExt as _;
use raijin_git::commit::ParsedCommitMessage;
use raijin_git::repository::{
...
use raijin_git::stash::GitStash;
use raijin_git::status::{DiffStat, StageStatus};
use raijin_git::{Amend, Signoff, ToggleStaged, repository::RepoPath, status::FileStatus};
use raijin_git::{
...
use raijin_language::{Buffer, File};
...
use raijin_prompt_store::{BuiltInPrompt, PromptId, PromptStore, RULES_FILE_NAMES};
use raijin_proto::RpcError;
...
use raijin_workspace::SERIALIZATION_THROTTLE_TIME;
    ...
    fn preview(_window: &mut Window, _cx: &mut App) -> Option<AnyElement> {
        ...
        fn custom(branch_name: &str, upstream: Option<UpstreamTracking>) -> Branch {
            Branch {
                ...
                upstream: upstream.map(|tracking| Upstream {
                    ref_name: format!("raijin/{}", branch_name).into(),
                    tracking,
                }),
                most_recent_commit: Some(CommitSummary {
                    sha: "abc123".into(),
                        ...
                        vec![
                            ...
                            single_example(
                                ...
                                div()
                                    ...
                                    .overflow_hidden()
                                    .child(PanelRepoFooter::new_preview(
                                        SharedString::from("raijin-industries-community-examples"),
...
mod tests {
    ...
    async fn test_entry_worktree_paths(cx: &mut TestAppContext) {
        ...
        let project =
            Project::test(fs.clone(), [path!("/root/raijin/crates/gpui").as_ref()], cx).await;
        let window_handle =
