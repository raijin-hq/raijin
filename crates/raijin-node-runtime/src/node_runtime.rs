...
use raijin_http_client::{Host, HttpClient, Url};
use log::Level;
use semver::Version;
...
struct NodeRuntimeState {
    ...
    options: raijin_watch::Receiver<Option<NodeBinaryOptions>>,
...
impl NodeRuntime {
    ...
    pub fn unavailable() -> Self {
        NodeRuntime(Arc::new(Mutex::new(NodeRuntimeState {
            http: Arc::new(raijin_http_client::BlockedHttpClient),
            instance: None,
            last_options: None,
            options: raijin_watch::channel(Some(NodeBinaryOptions::default())).1,
        ...
        let instance = if options.allow_binary_download {
            ...
            match ManagedNodeRuntime::install_if_needed(&state.http).await {
                Ok(instance) => {
                    log::log!(
                        log_level,
                        "using Raijin managed Node.js at {} since {}",
                ...
                Err(err) => {
                    ...
                    Box::new(UnavailableNodeRuntime {
                        error_message: format!(
                            "failure while downloading and/or installing Raijin managed Node.js, \
                            restart Raijin to retry: {}",
                            err
                        )
                        .into(),
                    }) as Box<dyn NodeRuntimeTrait>
...
impl ManagedNodeRuntime {
    ...
    async fn install_if_needed(http: &Arc<dyn HttpClient>) -> Result<Self> {
        ...
        let node_containing_dir = raijin_paths::data_dir().join("node");
            ...
            match result {
                Ok(output) => {
                    if output.status.success() {
                        ...
                        log::warn!(
                            "Raijin managed Node.js binary at {} failed check with output: {:?}",
                            node_binary.display(),
                            output
                        );
                        false
                ...
                Err(err) => {
                    log::warn!(
                        "Raijin managed Node.js binary at {} failed check, so re-downloading it. \
                        Error: {}",
                        node_binary.display(),
                        err
                    );
...
impl SystemNodeRuntime {
    ...
    async fn new(node: PathBuf, npm: PathBuf) -> Result<Self> {
        ...
        let scratch_dir = raijin_paths::data_dir().join("node");
