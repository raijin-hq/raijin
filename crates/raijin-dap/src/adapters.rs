...
pub use raijin_http_client::{HttpClient, github::latest_github_release};
use raijin_language::{LanguageName, LanguageToolchainStore};
...
use raijin_task::{DebugScenario, TcpArgumentsTemplate, RaijinDebugConfig};
...
impl TcpArguments {
    pub fn from_proto(proto: raijin_proto::TcpHost) -> anyhow::Result<Self> {
    ...
    pub fn to_proto(&self) -> raijin_proto::TcpHost {
...
impl DebugTaskDefinition {
    ...
    pub fn to_proto(&self) -> raijin_proto::DebugTaskDefinition {
        raijin_proto::DebugTaskDefinition {
    ...
    pub fn from_proto(proto: raijin_proto::DebugTaskDefinition) -> Result<Self> {
        Ok(Self {
...
impl DebugAdapterBinary {
    pub fn from_proto(binary: raijin_proto::DebugAdapterBinary) -> anyhow::Result<Self> {
        let request = match binary.launch_type() {
            raijin_proto::debug_adapter_binary::LaunchType::Launch => {
                StartDebuggingRequestArgumentsRequest::Launch
            }
            raijin_proto::debug_adapter_binary::LaunchType::Attach => {
    ...
    pub fn to_proto(&self) -> raijin_proto::DebugAdapterBinary {
        raijin_proto::DebugAdapterBinary {
            ...
            launch_type: match self.request_args.request {
                StartDebuggingRequestArgumentsRequest::Launch => {
                    raijin_proto::debug_adapter_binary::LaunchType::Launch.into()
                }
                StartDebuggingRequestArgumentsRequest::Attach => {
                    raijin_proto::debug_adapter_binary::LaunchType::Attach.into()
                }
            },
...
pub async fn download_adapter_from_github(
    ...
    let adapter_path = raijin_paths::debug_adapters_dir().join(&adapter_name.as_ref());
...
impl DebugAdapter for FakeAdapter {
    ...
    async fn config_from_raijin_format(&self, raijin_scenario: RaijinDebugConfig) -> Result<DebugScenario> {
        let config = serde_json::to_value(raijin_scenario.request).unwrap();
