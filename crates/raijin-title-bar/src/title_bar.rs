...
pub use raijin_platform_title_bar::{
    self, DraggedWindowTab, MergeAllWindows, MoveTabToNewWindow, PlatformTitleBar,
    ShowNextWindowTab, ShowPreviousWindowTab,
};
use raijin_project::linked_worktree_short_name;
...
use raijin_project::{Project, git_store::GitStoreEvent, trusted_worktrees::TrustedWorktrees};
...
pub fn init(cx: &mut App) {
    raijin_platform_title_bar::PlatformTitleBar::init(cx);
...
impl TitleBar {
    pub fn new(
        ...
        subscriptions.push(
            cx.subscribe(&project, |this, _, event: &raijin_project::Event, cx| {
                if let raijin_project::Event::BufferEdited = event {
        ...
        let banner = cx.new(|cx| {
            OnboardingBanner::new(
                ...
                raijin_actions::agent::OpenClaudeAgentOnboardingModal.boxed_clone(),
                cx,
            )
            // When updating this to a non-AI feature release, remove this line.
            .visible_when(|cx| !raijin_project::DisableAiSettings::get_global(cx).disable_ai)
        });
    ...
    pub fn effective_active_worktree(&self, cx: &App) -> Option<Entity<raijin_project::Worktree>> {
    ...
    fn get_repository_for_worktree(
        &self,
        worktree: &Entity<raijin_project::Worktree>,
        cx: &App,
    ) -> Option<Entity<raijin_project::git_store::Repository>> {
        let project = self.project.read(cx);
    ...
    fn render_remote_project_connection(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let (indicator_color, meta) = match self.project.read(cx).remote_connection_state(cx)? {
            raijin_remote::ConnectionState::Connecting => (Color::Info, format!("Connecting to: {host}")),
            raijin_remote::ConnectionState::Connected => (Color::Success, format!("Connected to: {host}")),
            raijin_remote::ConnectionState::HeartbeatMissed => (
                Color::Warning,
                format!("Connection attempt to {host} missed. Retrying..."),
            ),
            raijin_remote::ConnectionState::Reconnecting => (
                Color::Warning,
                format!("Lost connection to {host}. Reconnecting..."),
            ),
            raijin_remote::ConnectionState::Disconnected => {
                (Color::Error, format!("Disconnected from {host}"))
            }
        };

        let icon_color = match self.project.read(cx).remote_connection_state(cx)? {
            raijin_remote::ConnectionState::Connecting => Color::Info,
            raijin_remote::ConnectionState::Connected => Color::Default,
            raijin_remote::ConnectionState::HeartbeatMissed => Color::Warning,
            raijin_remote::ConnectionState::Reconnecting => Color::Warning,
            raijin_remote::ConnectionState::Disconnected => Color::Error,
        };
        ...
        Some(
            PopoverMenu::new("remote-project-menu")
                .menu(move |window, cx| {
                    ...
                    Some(raijin_recent_projects::RemoteServerProjects::popover(
                        fs,
                        workspace.clone(),
                        false,
                        window,
    ...
    fn render_project_name(
        ...
        let display_name = if let Some(ref name) = name {
            inazuma_util::truncate_and_trailoff(name, MAX_PROJECT_NAME_LENGTH)
        } else {
        ...
        PopoverMenu::new("recent-projects-menu")
            .menu(move |window, cx| {
                Some(raijin_recent_projects::RecentProjects::popover(
                    workspace.clone(),
    ...
    fn render_project_branch(
        &self,
        repository: Entity<raijin_project::git_store::Repository>,
        ...
        let (branch_name, icon_info) = {
            ...
            let branch_name = repo
                ...
                .map(|name| inazuma_util::truncate_and_trailoff(name, MAX_BRANCH_NAME_LENGTH))
                .or_else(|| {
        ...
        Some(
            PopoverMenu::new("branch-menu")
                .menu(move |window, cx| {
                    Some(raijin_git_ui::git_picker::popover(
                        workspace.downgrade(),
                        effective_repository.clone(),
                        raijin_git_ui::git_picker::GitPickerTab::Branches,
                        inazuma::rems(34.),
                        window,
                        cx,
                    ))
                ...
                .trigger_with_tooltip(
                    ...
                    move |_window, cx| {
                        Tooltip::with_meta(
                            "Git Switcher",
                            Some(&raijin_actions::git::Branch),
    ...
    fn render_connection_status(
        ...
        match status {
            raijin_client::Status::ConnectionError
            | raijin_client::Status::ConnectionLost
            | raijin_client::Status::Reauthenticating
            | raijin_client::Status::Reconnecting
            | raijin_client::Status::ReconnectionError { .. } => Some(
                div()
                    .id("disconnected")
            ...
            raijin_client::Status::UpgradeRequired => {
                let auto_updater = raijin_auto_update::AutoUpdater::get(cx);
                let label = match auto_updater.map(|auto_update| auto_update.read(cx).status()) {
                    Some(AutoUpdateStatus::Updated { .. }) => "Please restart Raijin to Collaborate",
                    Some(AutoUpdateStatus::Installing { .. })
                    | Some(AutoUpdateStatus::Downloading { .. })
                    | Some(AutoUpdateStatus::Checking) => "Updating...",
                    Some(AutoUpdateStatus::Idle)
                    | Some(AutoUpdateStatus::Errored { .. })
                    | None => "Please update Raijin to Collaborate",
                };
        ...
        PopoverMenu::new("user-menu")
            ...
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, |menu, _, _cx| {
                        ...
                        this.custom_entry(
                            move |_window, _cx| {
                                h_flex()
                                    ...
                                    .gap_1()
                                    .justify_between()
                                    .child(Label::new("Restart to update Raijin").color(Color::Accent))
                                    .child(
                    ...
                    .action("Settings", raijin_actions::OpenSettings.boxed_clone())
                    .action("Keymap", Box::new(raijin_actions::OpenKeymap))
                    .action(
                        "Themes…",
                        raijin_actions::theme_selector::Toggle::default().boxed_clone(),
                    )
                    .action(
                        "Icon Themes…",
                        raijin_actions::icon_theme_selector::Toggle::default().boxed_clone(),
                    )
                    .action(
                        "Extensions",
                        raijin_actions::Extensions::default().boxed_clone(),
                    )
                    .when(is_signed_in, |this| {
                        this.separator()
                            .action("Sign Out", raijin_client::SignOut.boxed_clone())
                    })
                })
                .into()
            })
