...
use raijin_editor::scroll::ScrollOffset;
use raijin_editor::{Anchor, AnchorRangeExt, Editor, scroll::Autoscroll};
use raijin_editor::{MultiBufferOffset, RowHighlightOptions, SelectionEffects};
...
use raijin_language::{Outline, OutlineItem};
...
use raijin_workspace::{DismissDecision, ModalView};

pub fn init(cx: &mut App) {
    cx.observe_new(OutlineView::register).detach();
    raijin_actions::outline::TOGGLE_OUTLINE
        .set(|view, window, cx| {
...
pub fn toggle(
    ...
    _: &raijin_actions::outline::ToggleOutline,
    window: &mut Window,
    cx: &mut App,
) {
...
mod tests {
    ...
    use indoc::indoc;
    use raijin_language::FakeLspAdapter;
    ...
    fn open_outline_view(
        ...
        cx.dispatch_action(raijin_actions::outline::ToggleOutline);
    ...
    fn set_single_caret_at_row(
        ...
        editor.update_in(cx, |editor, window, cx| {
            editor.change_selections(SelectionEffects::no_scroll(), window, cx, |s| {
                s.select_ranges([inazuma_rope::Point::new(buffer_row, 0)..inazuma_rope::Point::new(buffer_row, 0)])
            });
        });
    }
