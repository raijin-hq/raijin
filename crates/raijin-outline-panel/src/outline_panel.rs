...
use raijin_language::language_settings::LanguageSettings;
use raijin_language::{Anchor, BufferId, BufferSnapshot, OffsetRangeExt, OutlineItem};
...
use raijin_project::{File, Fs, GitEntry, GitTraversal, Project, ProjectItem};
use raijin_search::{BufferSearchBar, ProjectSearchView};
...
use raijin_worktree::{Entry, ProjectEntryId, WorktreeId};
...
type Outline = OutlineItem<raijin_language::Anchor>;
...
struct SearchState {
    kind: SearchKind,
    query: String,
    matches: Vec<(Range<raijin_editor::Anchor>, Arc<OnceLock<SearchData>>)>,
...
struct HighlightArguments {
    ...
    match_range: Range<raijin_editor::Anchor>,
...
impl SearchState {
    fn new(
        kind: SearchKind,
        query: String,
        previous_matches: HashMap<Range<raijin_editor::Anchor>, Arc<OnceLock<SearchData>>>,
        new_matches: Vec<Range<raijin_editor::Anchor>>,
...
#[derive(Debug)]
struct Excerpt {
    range: ExcerptRange<raijin_language::Anchor>,
...
#[derive(Clone, Debug)]
struct SearchData {
    context_range: Range<raijin_editor::Anchor>,
    context_text: String,
    truncated_left: bool,
...
impl OutlinePanel {
        ...
        cx.new(|cx| {
            ...
            let workspace_subscription = cx.subscribe_in(
                ...
                move |outline_panel, workspace, event, window, cx| {
                    if let raijin_workspace::Event::ActiveItemChanged = event {
    ...
    fn open_excerpts(
        &mut self,
        action: &raijin_editor::actions::OpenExcerpts,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if self.filter_editor.focus_handle(cx).is_focused(window) {
    ...
    fn open_excerpts_split(
        &mut self,
        action: &raijin_editor::actions::OpenExcerptsSplit,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if self.filter_editor.focus_handle(cx).is_focused(window) {
    ...
    fn deploy_context_menu(
        ...
        let context_menu = ContextMenu::build(window, cx, |menu, _, _| {
            menu.context(self.focus_handle.clone())
                .action(
                    raijin_ui::utils::reveal_in_file_manager_label(false),
    ...
    fn render_outline(
        ...
        let label_element = raijin_outline::render_item(
            &outline.outline,
            string_match
    ...
    fn entry_element(
        ...
        div()
            ...
            .on_click({
                ...
                cx.listener(move |outline_panel, event: &inazuma::ClickEvent, window, cx| {
    ...
    fn location_for_editor_selection(
        ...
        let selection = editor.update(cx, |editor, cx| {
            editor
                .selections
                .newest::<raijin_language::Point>(&editor.display_snapshot(cx))
                .head()
        });
    ...
    fn outline_location(
        ...
        buffer_id: BufferId,
        excerpt_id: ExcerptId,
        multi_buffer_snapshot: raijin_editor::MultiBufferSnapshot,
        editor_snapshot: raijin_editor::EditorSnapshot,
    ...
    fn excerpt_fetch_ranges(
        ...
        (
            BufferSnapshot,
            HashMap<ExcerptId, ExcerptRange<raijin_language::Anchor>>,
        ),
    > {
        self.fs_entries
            .iter()
            ...
            v_flex()
                ...
                .child(h_flex().justify_center().child({
                    let keystroke = match self.position(window, cx) {
                        DockPosition::Left => window.keystroke_text_for(&raijin_workspace::ToggleLeftDock),
                        DockPosition::Bottom => {
                            window.keystroke_text_for(&raijin_workspace::ToggleBottomDock)
                        }
                        DockPosition::Right => {
                            window.keystroke_text_for(&raijin_workspace::ToggleRightDock)
                        }
                    };
            ...
            let list_contents = {
                ...
                .when(show_indent_guides, |list| {
                    list.with_decoration(
                        raijin_ui::indent_guides(px(indent_size), IndentGuideColors::panel(cx))
...
mod tests {
    ...
    use raijin_language::{self, FakeLspAdapter, rust_lang};
    use pretty_assertions::assert_eq;
    use raijin_project::FakeFs;
    use raijin_search::{
        buffer_search,
  ...
  crates/
    ...
    rust-analyzer/src/
      ...
      config.rs
        ...
        outline_panel.update_in(cx, |outline_panel, window, cx| {
            outline_panel.open_excerpts(&raijin_editor::actions::OpenExcerpts, window, cx);
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
        cx.run_until_parked();
      ...
      ErrorBoundary.tsx
        ...
        outline_panel.update_in(cx, |outline_panel, window, cx| {
            outline_panel
                ...
                .update(cx, |editor, cx| {
                    editor.toggle_fold(&raijin_editor::actions::ToggleFold, window, cx)
                });
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
  ...
  src/
    ...
    fn init_test(cx: &mut TestAppContext) {
        cx.update(|cx| {
            ...
            raijin_search::project_search::init(cx);
            raijin_search::buffer_search::init(cx);
            super::init(cx);
        });
    }
...
// ... snip
    ...
    fn selected_row_text(editor: &Entity<Editor>, cx: &mut App) -> String {
        editor.update(cx, |editor, cx| {
            let selections = editor.selections.all::<raijin_language::Point>(&editor.display_snapshot(cx));
            ...
            let line_start = raijin_language::Point::new(selection.start.row, 0);
            let line_end = multi_buffer_snapshot.clip_point(raijin_language::Point::new(selection.end.row, u32::MAX), raijin_language::Bias::Right);
            ...
            FakeLspAdapter {
                ...
                initializer: Some(Box::new(|fake_language_server| {
                    fake_language_server
                        .set_request_handler::<raijin_lsp::request::DocumentSymbolRequest, _, _>(
                            move |_, _| async move {
                                #[allow(deprecated)]
                                Ok(Some(raijin_lsp::DocumentSymbolResponse::Nested(vec![
                                    raijin_lsp::DocumentSymbol {
                                        name: "Foo".to_string(),
                                        detail: None,
                                        kind: raijin_lsp::SymbolKind::STRUCT,
                                        tags: None,
                                        deprecated: None,
                                        range: raijin_lsp::Range::new(
                                            raijin_lsp::Position::new(0, 0),
                                            raijin_lsp::Position::new(3, 1),
                                        ),
                                        selection_range: raijin_lsp::Range::new(
                                            raijin_lsp::Position::new(0, 7),
                                            raijin_lsp::Position::new(0, 10),
                                        ),
                                        children: Some(vec![
                                            raijin_lsp::DocumentSymbol {
                                                name: "bar".to_string(),
                                                detail: None,
                                                kind: raijin_lsp::SymbolKind::FIELD,
                                                tags: None,
                                                deprecated: None,
                                                range: raijin_lsp::Range::new(
                                            ...
                                            raijin_lsp::DocumentSymbol {
                                                ...
                                                range: raijin_lsp::Range::new(
                                                    ...
                                                    raijin_lsp::Position::new(2, 15),
                                                ),
                                                selection_range: raijin_lsp::Range::new(
                                                    raijin_lsp::Position::new(2, 4),
                                                    raijin_lsp::Position::new(2, 7),
                                                ),
                                                children: None,
                                            },
                                        ]),
