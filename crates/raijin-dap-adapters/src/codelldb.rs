...
use raijin_dap::adapters::{DebugTaskDefinition, latest_github_release};
use futures::StreamExt;
use inazuma::AsyncApp;
use serde_json::Value;
use raijin_task::{DebugRequest, DebugScenario, RaijinDebugConfig};
...
impl CodeLldbDebugAdapter {
    ...
    async fn request_args(
        ...
        label: &str,
    ) -> Result<raijin_dap::StartDebuggingRequestArguments> {
        ...
        Ok(raijin_dap::StartDebuggingRequestArguments {
            request,
            configuration,
        })
    }
...
impl DebugAdapter for CodeLldbDebugAdapter {
    ...
    async fn config_from_raijin_format(&self, raijin_scenario: RaijinDebugConfig) -> Result<DebugScenario> {
        ...
        map.insert(
            "name".into(),
            Value::String(String::from(raijin_scenario.label.as_ref())),
        );
    ...
    async fn get_binary(
        ...
        if command.is_none() {
            ...
            let adapter_path = raijin_paths::debug_adapters_dir().join(&Self::ADAPTER_NAME);
