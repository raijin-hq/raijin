...
impl GoDebugAdapter {
    ...
    async fn install_shim(&self, delegate: &Arc<dyn DapDelegate>) -> anyhow::Result<PathBuf> {
        ...
        let adapter_dir = raijin_paths::debug_adapters_dir().join("delve-shim-dap");
...
impl DebugAdapter for GoDebugAdapter {
    ...
    async fn config_from_raijin_format(&self, raijin_scenario: RaijinDebugConfig) -> Result<DebugScenario> {
        let mut args = match &raijin_scenario.request {
            raijin_dap::DebugRequest::Attach(attach_config) => {
                json!({
                    "request": "attach",
                    "mode": "local",
            ...
            raijin_dap::DebugRequest::Launch(launch_config) => {
