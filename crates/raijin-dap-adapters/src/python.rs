...
use raijin_dap::{DebugRequest, StartDebuggingRequestArguments, adapters::DebugTaskDefinition};
use raijin_fs::RemoveOptions;
use futures::{StreamExt, TryStreamExt};
use inazuma::http_client::AsyncBody;
use inazuma::{AsyncApp, SharedString};
use json_dotpath::DotPaths;
use raijin_language::{LanguageName, Toolchain};
use raijin_paths::debug_adapters_dir;
...
impl PythonDebugAdapter {
    ...
    async fn generate_debugpy_arguments<'a>(
        ...
        let mut args = if let Some(user_installed_path) = user_installed_path {
            ...
            let adapter_path = raijin_paths::debug_adapters_dir().join(Self::DEBUG_ADAPTER_NAME.as_ref());
    ...
    async fn request_args(
        ...
        if let Ok(console) = configuration.dot_get_mut("console") {
            // Use built-in Raijin terminal if user did not explicitly provide a setting for console.
            if console.is_null() {
    ...
    async fn base_venv_path(
        ...
        let result = self.base_venv_path
            .get_or_init(|| async {
                ...
                let debug_adapter_path = raijin_paths::debug_adapters_dir().join(Self::DEBUG_ADAPTER_NAME.as_ref());
                let output = inazuma_util::command::new_command(&base_python)
                    .args(["-m", "venv", "raijin_base_venv"])
                    .current_dir(
                        &debug_adapter_path,
                    )
                    .spawn()
                ...
                Ok(Arc::from(
                    raijin_paths::debug_adapters_dir()
                        .join(Self::DEBUG_ADAPTER_NAME.as_ref())
                        .join("raijin_base_venv")
                        .join(PYTHON_PATH)
                        .as_ref(),
                ))
            })
...
impl DebugAdapter for PythonDebugAdapter {
    ...
    async fn config_from_raijin_format(&self, raijin_scenario: RaijinDebugConfig) -> Result<DebugScenario> {
        let mut args = json!({
            "request": match raijin_scenario.request {
                ...
                },
                {
                    ...
                    "then": {
                        "properties": {
                            ...
                            "envFile": {
                                "default": "${RAIJIN_WORKTREE_ROOT}/.env",
    ...
    async fn get_binary(
        ...
        let base_paths = ["cwd", "program", "module"]
            ...
            .chain(
                ...
                // (Which should always be the case because Raijin defaults to the cwd worktree root)
        ...
        for base_path in base_paths {
            if let Some(found_toolchain) = delegate
                ...
                .active_toolchain(
                    ...
                    base_path.into_arc(),
                    raijin_language::LanguageName::new_static(Self::LANGUAGE_NAME),
                    cx,
                )
                .await
            {
...
mod tests {
    ...
    use super::*;
    use raijin_task::TcpArgumentsTemplate;
