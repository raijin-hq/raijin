...
use raijin_language::{Buffer, Language};
...
impl ProjectSearch {
    ...
    fn subscribe_to_excerpts(
        ...
        cx.subscribe(excerpts, |this, _, event, cx| {
            if matches!(event, raijin_multi_buffer::Event::FileHandleChanged) {
                ...
                let (buffers_with_ranges, has_reached_limit) = cx
                    ...
                    .spawn(async move {
                        ...
                        for result in results {
                            match result {
                                raijin_project::search::SearchResult::Buffer { buffer, ranges } => {
                                    buffers_with_ranges.push((buffer, ranges));
                                }
                                raijin_project::search::SearchResult::LimitReached => {
                                    limit_reached = true;
                                }
                            }
                        }
...
pub enum ViewEvent {
    UpdateTab,
    Activate,
    EditorEvent(raijin_editor::EditorEvent),
    Dismiss,
}
...
impl ProjectSearchView {
    ...
    pub fn new(
        ...
        subscriptions.push(
            cx.subscribe(&results_editor, |this, _, event: &EditorEvent, cx| {
                if matches!(event, raijin_editor::EditorEvent::SelectionsChanged { .. }) {
    ...
    pub fn deploy_search(
        workspace: &mut Workspace,
        action: &raijin_workspace::DeploySearch,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) {
            ...
            let skip_save_on_close = this
                .read_with(cx, |this, cx| {
                    this.workspace.read_with(cx, |workspace, cx| {
                        raijin_workspace::Pane::skip_save_on_close(&this.results_editor, workspace, cx)
                    })
                })?
                .unwrap_or(false);
...
pub mod tests {
    ...
    use super::*;
    use raijin_editor::{DisplayPoint, display_map::DisplayRow};
    use inazuma::{Action, TestAppContext, VisualTestContext, WindowHandle};
    use raijin_language::{FakeLspAdapter, rust_lang};
    use pretty_assertions::assert_eq;
    use raijin_project::{FakeFs, Fs};
    use serde_json::json;
    ...
    async fn test_deploy_project_search_focus(cx: &mut TestAppContext) {
        ...
        workspace.update_in(cx, move |workspace, window, cx| {
            ...
            ProjectSearchView::deploy_search(
                workspace,
                &raijin_workspace::DeploySearch::find(),
                window,
                cx,
            )
        });
    ...
    async fn test_new_project_search_focus(cx: &mut TestAppContext) {
        ...
        workspace.update_in(cx, move |workspace, window, cx| {
            ...
            ProjectSearchView::new_search(workspace, &raijin_workspace::NewSearch, window, cx)
        });
        ...
        assert!(
            ...
            "New search view should be open after `raijin_workspace::NewSearch` event"
        );
