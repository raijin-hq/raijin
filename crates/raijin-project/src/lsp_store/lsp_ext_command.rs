...
use raijin_lsp::{AdapterServerCapabilities, LanguageServer, LanguageServerId};
use raijin_rpc::proto::{self, PeerId};
use serde::{Deserialize, Serialize};
use std::{
    path::{Path, PathBuf},
    sync::Arc,
};
use raijin_task::TaskTemplate;
use inazuma_text::{BufferId, PointUtf16, ToPointUtf16};

pub enum LspExtExpandMacro {}

impl raijin_lsp::request::Request for LspExtExpandMacro {
...
pub struct ExpandMacroParams {
    pub text_document: raijin_lsp::TextDocumentIdentifier,
    pub position: raijin_lsp::Position,
}
...
impl raijin_lsp::request::Request for LspOpenDocs {
...
impl LspCommand for OpenDocs {
    ...
    fn to_lsp(
        ...
        let uri = raijin_lsp::Uri::from_file_path(path)
            .map_err(|()| anyhow::anyhow!("{path:?} is not a valid URI"))?;
        Ok(OpenDocsParams {
            text_document: raijin_lsp::TextDocumentIdentifier { uri },
            position: point_to_lsp(self.position),
        })
    }
...
impl raijin_lsp::request::Request for LspSwitchSourceHeader {
...
#[derive(Serialize, Deserialize, Debug)]
#[serde(rename_all = "camelCase")]
pub struct SwitchSourceHeaderParams(raijin_lsp::TextDocumentIdentifier);
...
impl raijin_lsp::request::Request for LspGoToParentModule {
    type Params = raijin_lsp::TextDocumentPositionParams;
    type Result = Option<Vec<raijin_lsp::LocationLink>>;
...
impl LspCommand for GoToParentModule {
    ...
    async fn response_from_lsp(
        self,
        links: Option<Vec<raijin_lsp::LocationLink>>,
        ...
        location_links_from_lsp(
            links.map(raijin_lsp::GotoDefinitionResponse::Link),
            lsp_store,
            buffer,
            server_id,
            cx,
...
impl raijin_lsp::request::Request for Runnables {
...
pub struct RunnablesParams {
    pub text_document: raijin_lsp::TextDocumentIdentifier,
    #[serde(default)]
    pub position: Option<raijin_lsp::Position>,
}
...
pub struct Runnable {
    ...
    pub location: Option<raijin_lsp::LocationLink>,
...
impl LspCommand for GetLspRunnables {
    ...
    fn to_lsp(
        ...
        Ok(RunnablesParams {
            text_document: raijin_lsp::TextDocumentIdentifier::new(url),
            position: self
                .position
...
impl raijin_lsp::notification::Notification for LspExtCancelFlycheck {
    type Params = ();
    const METHOD: &'static str = "rust-analyzer/cancelFlycheck";
}

impl raijin_lsp::notification::Notification for LspExtRunFlycheck {
...
pub struct RunFlycheckParams {
    pub text_document: Option<raijin_lsp::TextDocumentIdentifier>,
}

impl raijin_lsp::notification::Notification for LspExtClearFlycheck {
    type Params = ();
