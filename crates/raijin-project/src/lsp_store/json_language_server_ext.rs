...
use raijin_lsp::LanguageServer;

use crate::LspStore;

const LOGGER: raijin_log::Logger = raijin_log::scoped!("json-schema");
...
/// In our case, we provide the uris for actions on server startup under the `raijin://schemas/action/{normalize_action_name}` scheme.
...
impl raijin_lsp::request::Request for SchemaContentRequest {
...
impl raijin_lsp::notification::Notification for SchemaContentsChanged {
    const METHOD: &'static str = "json/schemaContent";
    type Params = String;
}

pub fn notify_schemas_changed(lsp_store: Entity<LspStore>, uris: &[String], cx: &App) {
    raijin_log::trace!(LOGGER => "Notifying schema changes for URIs: {:?}", uris);
    ...
    for server in servers {
        for uri in uris {
            raijin_log::trace!(LOGGER => "Notifying server {NAME} (id {ID:?}) of schema change for URI: {uri:?}",
                NAME = server.name(),
                ID = server.server_id()
            );
...
pub fn register_requests(lsp_store: WeakEntity<LspStore>, language_server: &LanguageServer) {
    language_server
        .on_request::<SchemaContentRequest, _, _>(move |params, cx| {
            ...
            async move {
                raijin_log::trace!(LOGGER => "Handling schema request for {:?}", &params);
                let result = resolution.await;
                match &result {
                    Ok(content) => {raijin_log::trace!(LOGGER => "Schema request resolved with {}B schema", content.len());},
                    Err(err) => {raijin_log::warn!(LOGGER => "Schema request failed: {}", err);},
                }
                result
            }
        })
