...
use raijin_askpass::{AskPassDelegate, EncryptedPassword, IKnowWhatIAmDoingAndIHaveReadTheDocs};
use raijin_buffer_diff::{BufferDiff, BufferDiffEvent};
use raijin_client::ProjectId;
...
struct BufferGitState {
    ...
    oid_diffs: HashMap<Option<raijin_git::Oid>, WeakEntity<BufferDiff>>,
    ...
    oid_texts: HashMap<raijin_git::Oid, Arc<str>>,
    head_changed: bool,
    index_changed: bool,
...
enum DiffKind {
    Unstaged,
    Uncommitted,
    SinceOid(Option<raijin_git::Oid>),
}
...
impl GitStore {
    ...
    pub fn file_history(
        ...
        path: RepoPath,
        cx: &mut App,
    ) -> Task<Result<raijin_git::repository::FileHistory>> {
        ...
        for update in updated_git_repositories.iter() {
            ...
            } = update
            {
                let original_repo_abs_path: Arc<Path> =
                    raijin_git::repository::original_repo_path_from_common_dir(common_dir_abs_path).into();
    ...
    async fn handle_push(
        ...
        let options = envelope
            ...
            .map(|_| match envelope.payload.options() {
                proto::push::PushOptions::SetUpstream => raijin_git::repository::PushOptions::SetUpstream,
                proto::push::PushOptions::Force => raijin_git::repository::PushOptions::Force,
            });
    ...
    async fn handle_get_blob_content(
        ...
        let oid = raijin_git::Oid::from_str(&request.payload.oid)?;
        let repository_id = RepositoryId(request.payload.repository_id);
        let content = this
            .update(&mut cx, |this, cx| {
                let repository = this.repositories().get(&repository_id)?;
...
impl BufferGitState {
    ...
    fn oid_diff(&self, oid: Option<raijin_git::Oid>) -> Option<Entity<BufferDiff>> {
    ...
    fn recalculate_diffs(&mut self, buffer: inazuma_text::BufferSnapshot, cx: &mut Context<Self>) {
        *self.recalculating_tx.borrow_mut() = true;
        ...
        let oid_diffs: Vec<(Option<raijin_git::Oid>, Entity<BufferDiff>, Option<Arc<str>>)> = self
            .oid_diffs
            .iter()
            .filter_map(|(oid, weak)| {
...
impl RepositorySnapshot {
    ...
    /// created from.
    ///
    /// For example, if you had both `~/code/raijin` and `~/code/worktrees/raijin-2`,
    /// then `~/code/raijin` is the main worktree and `~/code/worktrees/raijin-2` is a linked worktree.
...
impl Repository {
    ...
    pub fn file_history(
        &mut self,
        path: RepoPath,
    ) -> oneshot::Receiver<Result<raijin_git::repository::FileHistory>> {
            ...
            match git_repo {
                ...
                RepositoryState::Remote(RemoteRepositoryState { client, project_id }) => {
                    let response = client
                        ...
                        .await?;
                    Ok(raijin_git::repository::FileHistory {
                        entries: response
                            .entries
                            .into_iter()
                            .map(|entry| raijin_git::repository::FileHistoryEntry {
                                sha: entry.sha.into(),
    ...
    fn spawn_local_git_worker(
        ...
        cx.spawn(async move |_, cx| {
            ...
            {
                raijin_git_hosting_providers::register_additional_providers(
...
pub async fn resolve_git_worktree_to_main_repo(fs: &dyn Fs, path: &Path) -> Option<PathBuf> {
    ...
    let common_dir = fs
        ...
        .await
        .ok()?;
    Some(raijin_git::repository::original_repo_path_from_common_dir(
        &common_dir,
    ))
}
...
fn serialize_blame_buffer_response(blame: Option<raijin_git::blame::Blame>) -> proto::BlameBufferResponse {
...
fn deserialize_blame_buffer_response(
...
) -> Option<raijin_git::blame::Blame> {
    let response = response.blame_response?;
    let entries = response
        .entries
        .into_iter()
        .filter_map(|entry| {
            Some(raijin_git::blame::BlameEntry {
                sha: raijin_git::Oid::from_bytes(&entry.sha).ok()?,
    ...
    let messages = response
        .messages
        .into_iter()
        .filter_map(|message| Some((raijin_git::Oid::from_bytes(&message.oid).ok()?, message.message)))
        .collect::<HashMap<_, _>>();

    Some(Blame { entries, messages })
}

fn branch_to_proto(branch: &raijin_git::repository::Branch) -> proto::Branch {
    proto::Branch {
...
fn worktree_to_proto(worktree: &raijin_git::repository::Worktree) -> proto::Worktree {
    proto::Worktree {
...
fn proto_to_worktree(proto: &proto::Worktree) -> raijin_git::repository::Worktree {
    raijin_git::repository::Worktree {
...
fn proto_to_branch(proto: &proto::Branch) -> raijin_git::repository::Branch {
    raijin_git::repository::Branch {
        is_head: proto.is_head,
        ref_name: proto.ref_name.clone().into(),
        upstream: proto
            .upstream
            .as_ref()
            .map(|upstream| raijin_git::repository::Upstream {
                ref_name: upstream.ref_name.to_string().into(),
                tracking: upstream
                    .tracking
                    .as_ref()
                    .map(|tracking| {
                        raijin_git::repository::UpstreamTracking::Tracked(UpstreamTrackingStatus {
                            ahead: tracking.ahead as u32,
                            behind: tracking.behind as u32,
                        })
                    })
                    .unwrap_or(raijin_git::repository::UpstreamTracking::Gone),
            }),
        most_recent_commit: proto.most_recent_commit.as_ref().map(|commit| {
            raijin_git::repository::CommitSummary {
