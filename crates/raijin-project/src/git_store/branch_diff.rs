...
impl BranchDiff {
    ...
    #[instrument(skip_all)]
    fn load_buffer(
        branch_diff: Option<raijin_git::status::TreeDiffStatus>,
        ...
        let task = cx.spawn(async move |project, cx| {
            ...
            let changes = if let Some(entry) = branch_diff {
                let oid = match entry {
                    raijin_git::status::TreeDiffStatus::Added { .. } => None,
                    raijin_git::status::TreeDiffStatus::Modified { old, .. }
                    | raijin_git::status::TreeDiffStatus::Deleted { old } => Some(old),
                };
                project
...
fn diff_status_to_file_status(branch_diff: &raijin_git::status::TreeDiffStatus) -> FileStatus {
    let file_status = match branch_diff {
        raijin_git::status::TreeDiffStatus::Added { .. } => FileStatus::Tracked(TrackedStatus {
            index_status: StatusCode::Added,
            worktree_status: StatusCode::Added,
        }),
        raijin_git::status::TreeDiffStatus::Modified { .. } => FileStatus::Tracked(TrackedStatus {
            index_status: StatusCode::Modified,
            worktree_status: StatusCode::Modified,
        }),
        raijin_git::status::TreeDiffStatus::Deleted { .. } => FileStatus::Tracked(TrackedStatus {
