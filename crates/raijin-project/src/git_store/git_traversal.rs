...
use raijin_git::{repository::RepoPath, status::GitSummary};
use std::{collections::BTreeMap, ops::Deref, path::Path};
use inazuma_sum_tree::Cursor;
use inazuma_text::Bias;
use inazuma_util::rel_path::RelPath;
use raijin_worktree::{Entry, PathProgress, PathTarget, Traversal};
...
impl<'a> ChildEntriesGitIter<'a> {
    pub fn new(
        repo_snapshots: &'a HashMap<RepositoryId, RepositorySnapshot>,
        worktree_snapshot: &'a raijin_worktree::Snapshot,
        parent_path: &'a RelPath,
    ) -> Self {
