...
use raijin_git::repository::get_git_committer;
use git_store::{Repository, RepositoryId};
pub mod search_history;
pub mod yarn;

use raijin_dap::inline_value::{InlineValueLocation, VariableLookupKind, VariableScope};
...
use raijin_dap::client::DebugAdapterClient;
...
use ::raijin_git::{blame::Blame, status::FileStatus};
use inazuma::{
    App, AppContext, AsyncApp, BorrowAppContext, Context, Entity, EventEmitter, Oklch, SharedString,
...
use raijin_remote::wsl_path_to_windows_path;
use raijin_remote::{RemoteClient, RemoteConnectionOptions};
use raijin_rpc::{
...
use inazuma_settings_framework::{InvalidSettingsError, RegisterSetting, Settings, SettingsLocation, SettingsStore};
use raijin_snippet::Snippet;
pub use raijin_snippet_provider;
use raijin_snippet_provider::SnippetProvider;
use std::{
    borrow::Cow,
...
use raijin_worktree::{CreatedEntry, Snapshot, Traversal};
...
pub struct Project {
    ...
    collab_client: Arc<raijin_client::Client>,
...
pub enum Event {
    ...
    // [`raijin_lsp::notification::DidOpenTextDocument`] was sent to this server using the buffer data.
    // Raijin's buffer-related data is updated accordingly.
    ...
    SnippetEdit(BufferId, Vec<(raijin_lsp::Range, Snippet)>),
...
impl ProjectPath {
    pub fn from_file(value: &dyn raijin_language::File, cx: &App) -> Self {
        ProjectPath {
...
pub struct InlayHint {
    pub position: raijin_language::Anchor,
...
pub enum CompletionSource {
    Lsp {
        ...
        lsp_completion: Box<raijin_lsp::CompletionItem>,
        /// A set of defaults for this completion item.
        lsp_defaults: Option<Arc<raijin_lsp::CompletionListItemDefaults>>,
...
impl CompletionSource {
    ...
    pub fn lsp_completion(&self, apply_defaults: bool) -> Option<Cow<'_, raijin_lsp::CompletionItem>> {
        if let Self::Lsp {
            lsp_completion,
            lsp_defaults,
            ..
                    ...
                    || default_insert_text_mode.is_some()
                {
                    ...
                    if completion_with_defaults.text_edit.is_none() {
                        match default_edit_range {
                            Some(raijin_lsp::CompletionListItemDefaultsEditRange::Range(range)) => {
                            ...
                            Some(raijin_lsp::CompletionListItemDefaultsEditRange::InsertAndReplace {
                                insert,
                                replace,
                            }) => {
                                completion_with_defaults.text_edit =
...
impl LspAction {
    ...
    pub fn action_kind(&self) -> Option<raijin_lsp::CodeActionKind> {
        match self {
            Self::Action(action) => action.kind.clone(),
            Self::Command(_) => Some(raijin_lsp::CodeActionKind::new("command")),
            Self::CodeLens(_) => Some(raijin_lsp::CodeActionKind::new("code lens")),
        }
    }

    fn edit(&self) -> Option<&raijin_lsp::WorkspaceEdit> {
        match self {
    ...
    fn command(&self) -> Option<&raijin_lsp::Command> {
        match self {
...
pub enum ResolveState {
    Resolved,
    CanResolve(LanguageServerId, Option<raijin_lsp::LSPAny>),
    Resolving,
}
impl InlayHint {
...
pub struct InlayHintLabelPart {
    ...
    pub location: Option<(LanguageServerId, raijin_lsp::Location)>,
}
...
pub struct Hover {
    ...
    pub range: Option<Range<raijin_language::Anchor>>,
...
pub struct DocumentColor {
    pub lsp_range: raijin_lsp::Range,
    pub color: raijin_lsp::Color,
    pub resolved: bool,
...
pub struct ColorPresentation {
    ...
    pub text_edit: Option<raijin_lsp::TextEdit>,
    pub additional_text_edits: Vec<raijin_lsp::TextEdit>,
}
...
pub const DEFAULT_COMPLETION_CONTEXT: CompletionContext = CompletionContext {
    trigger_kind: raijin_lsp::CompletionTriggerKind::INVOKED,
...
/// Whether to disable all AI features in Raijin.
///
/// Default: false
...
impl DisableAiSettings {
    ...
    /// allowing `disable_ai` to be configured per-project in `.raijin/settings.json`.
    ...
    pub fn is_ai_disabled_for_file(file: Option<&Arc<dyn raijin_language::File>>, cx: &App) -> bool {
...
impl Project {
        ...
        cx.new(|cx: &mut Context<Self>| {
            ...
            let context_server_store = cx.new(|cx| {
                ContextServerStore::remote(
                    raijin_rpc::proto::REMOTE_SERVER_PROJECT_ID,
                    remote.clone(),
                    worktree_store.clone(),
                    Some(weak_self.clone()),
                    cx,
    ...
    #[cfg(feature = "test-support")]
    pub fn client_subscriptions(&self) -> &Vec<raijin_client::Subscription> {
    ...
    pub async fn example(
        ...
        let client = cx.update(|cx| raijin_client::Client::new(clock, http_client.clone(), cx));
    ...
    #[inline]
    pub fn remote_connection_state(&self, cx: &App) -> Option<raijin_remote::ConnectionState> {
        self.remote_client
            .as_ref()
    ...
    pub fn default_path_list(&self, cx: &App) -> PathList {
        ...
        if worktree_roots.is_empty() {
            PathList::new(&[raijin_paths::home_dir().as_path()])
        } else {
                        ...
                        self.lsp_store.update(cx, |lsp_store, _| {
                                ...
                                .get_mut(language_server_id)
                            {
                                ...
                                language_server_status.workspace_folders = update
                                    .workspace_folders
                                    .iter()
                                    .filter_map(|uri_str| raijin_lsp::Uri::from_str(uri_str).ok())
                                    .collect();
                            }
                        });
                    }
    ...
    fn on_remote_client_event(
        ...
        event: &raijin_remote::RemoteClientEvent,
        cx: &mut Context<Self>,
    ) {
        match event {
            &raijin_remote::RemoteClientEvent::Disconnected { server_not_running } => {
                self.worktree_store.update(cx, |store, cx| {
                    store.disconnected_from_host(cx);
                });
                self.buffer_store.update(cx, |buffer_store, cx| {
                ...
                Err(InvalidSettingsError::Tasks { message, path }) => {
                    ...
                    cx.emit(Event::Toast {
                        ...
                        link: Some(ToastLink {
                            ...
                            url: "https://raijin.dev/docs/tasks",
                        }),
                        message,
                    });
                }
            ...
            let send_updates_for_buffers = this.update(cx, |this, cx| {
                response
                    ...
                    .map(|buffer| {
                        ...
                        let remote_version = raijin_language::proto::deserialize_version(&buffer.version);
            ...
            location
                ...
                .update(cx, |buffer, cx| {
                    buffer.set_agent_selections(
                        Arc::from([raijin_language::Selection {
                            id: 0,
                            start: location.position,
                            end: location.position,
                            reversed: false,
                            goal: raijin_language::SelectionGoal::None,
                        }]),
                        false,
                        CursorShape::Hollow,
                        cx,
...
impl Completion {
    pub fn kind(&self) -> Option<CompletionItemKind> {
        self.source
            // `raijin_lsp::CompletionListItemDefaults` has no `kind` field
    ...
    pub fn sort_key(&self) -> (usize, &str) {
        ...
        let kind_key = self
            ...
            .and_then(|lsp_completion_kind| match lsp_completion_kind {
                raijin_lsp::CompletionItemKind::KEYWORD => Some(0),
                raijin_lsp::CompletionItemKind::VARIABLE => Some(1),
                raijin_lsp::CompletionItemKind::CONSTANT => Some(2),
                raijin_lsp::CompletionItemKind::PROPERTY => Some(3),
                _ => None,
            })
    ...
    pub fn is_snippet(&self) -> bool {
        self.source
            // `raijin_lsp::CompletionListItemDefaults` has `insert_text_format` field
            .lsp_completion(true)
            .is_some_and(|lsp_completion| {
                lsp_completion.insert_text_format == Some(raijin_lsp::InsertTextFormat::SNIPPET)
            })
    }
...
fn provide_inline_values(
    captures: impl Iterator<Item = (Range<usize>, raijin_language::DebuggerTextObject)>,
    ...
    for (capture_range, capture_kind) in captures {
        match capture_kind {
            raijin_language::DebuggerTextObject::Variable => {
            ...
            raijin_language::DebuggerTextObject::Scope => {
