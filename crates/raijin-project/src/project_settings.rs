...
use raijin_context_server::ContextServerCommand;
use raijin_dap::adapters::DebugAdapterName;
use raijin_fs::Fs;
use futures::StreamExt as _;
use raijin_git::repository::DEFAULT_WORKTREE_DIRECTORY;
...
impl DiagnosticSeverity {
    pub fn into_lsp(self) -> Option<raijin_lsp::DiagnosticSeverity> {
        match self {
            DiagnosticSeverity::Off => None,
            DiagnosticSeverity::Error => Some(raijin_lsp::DiagnosticSeverity::ERROR),
            DiagnosticSeverity::Warning => Some(raijin_lsp::DiagnosticSeverity::WARNING),
            DiagnosticSeverity::Info => Some(raijin_lsp::DiagnosticSeverity::INFORMATION),
            DiagnosticSeverity::Hint => Some(raijin_lsp::DiagnosticSeverity::HINT),
        }
    }
}
...
impl From<inazuma_settings_framework::DiagnosticSeverityContent> for DiagnosticSeverity {
    fn from(severity: inazuma_settings_framework::DiagnosticSeverityContent) -> Self {
        match severity {
            ...
            inazuma_settings_framework::DiagnosticSeverityContent::Error => DiagnosticSeverity::Error,
            inazuma_settings_framework::DiagnosticSeverityContent::Warning => DiagnosticSeverity::Warning,
...
impl From<raijin_lsp::DiagnosticSeverity> for GoToDiagnosticSeverity {
    fn from(severity: raijin_lsp::DiagnosticSeverity) -> Self {
        match severity {
            raijin_lsp::DiagnosticSeverity::ERROR => Self::Error,
            raijin_lsp::DiagnosticSeverity::WARNING => Self::Warning,
            raijin_lsp::DiagnosticSeverity::INFORMATION => Self::Information,
            raijin_lsp::DiagnosticSeverity::HINT => Self::Hint,
            _ => Self::Error,
        }
    }
}
...
impl GoToDiagnosticSeverityFilter {
    pub fn matches(&self, severity: raijin_lsp::DiagnosticSeverity) -> bool {
...
pub struct DiagnosticsSettings {
    ...
    /// Settings for using LSP pull diagnostics mechanism in Raijin.
...
/// SettingsObserver observers changes to .raijin/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/raijin/{settings, task}.json and sends the content
/// upstream.
        ...
        Self {
            ...
            _global_debug_config_watcher: if watch_global_configs {
                Self::subscribe_to_global_debug_scenarios_changes(
                    fs.clone(),
                    raijin_paths::debug_scenarios_file().clone(),
                    cx,
                )
            } else {
                Task::ready(())
