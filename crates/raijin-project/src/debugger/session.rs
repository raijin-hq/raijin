...
use raijin_dap::adapters::{DebugAdapterBinary, DebugAdapterName};
use raijin_dap::messages::Response;
use raijin_dap::requests::{Request, RunInTerminal, StartDebugging};
use raijin_dap::transport::TcpTransport;
use raijin_dap::{
...
use raijin_task::SharedTaskContext;
...
pub struct StackFrame {
    pub dap: raijin_dap::StackFrame,
    pub scopes: Vec<raijin_dap::Scope>,
}

impl From<raijin_dap::StackFrame> for StackFrame {
    fn from(stack_frame: raijin_dap::StackFrame) -> Self {
        Self {
            scopes: vec![],
            dap: stack_frame,
        }
...
#[derive(Debug, Clone)]
pub struct Thread {
    dap: raijin_dap::Thread,
    stack_frames: Vec<StackFrame>,
    stack_frames_error: Option<SharedString>,
    _has_stopped: bool,
}

impl From<raijin_dap::Thread> for Thread {
    fn from(dap: raijin_dap::Thread) -> Self {
        Self {
            dap,
...
pub struct DataBreakpointState {
    pub dap: raijin_dap::DataBreakpoint,
    pub is_enabled: bool,
...
fn client_source(abs_path: &Path) -> raijin_dap::Source {
    raijin_dap::Source {
        name: abs_path
            .file_name()
...
impl RunningMode {
    ...
    ) -> Task<Result<Vec<raijin_dap::Breakpoint>>> {
    ...
    fn initialize_sequence(
        ...
        let launch = match raw.request {
            raijin_dap::StartDebuggingRequestArgumentsRequest::Launch => self.request(Launch {
                raw: raw.configuration,
            }),
            raijin_dap::StartDebuggingRequestArgumentsRequest::Attach => self.request(Attach {
                raw: raw.configuration,
            }),
        };
    ...
    where
        <R::DapRequest as raijin_dap::requests::Request>::Response: 'static,
        <R::DapRequest as raijin_dap::requests::Request>::Arguments: 'static + Send,
    {
...
pub struct SessionSnapshot {
    ...
    variables: HashMap<VariableReference, Vec<raijin_dap::Variable>>,
    stack_frames: IndexMap<StackFrameId, StackFrame>,
    locations: HashMap<u64, raijin_dap::LocationsResponse>,
    modules: Vec<raijin_dap::Module>,
    loaded_sources: Vec<raijin_dap::Source>,
}
...
pub struct Session {
    ...
    output: Box<circular_buffer::CircularBuffer<MAX_TRACKED_OUTPUT_EVENTS, raijin_dap::OutputEvent>>,
...
impl Session {
    ...
    pub fn on_app_quit(&mut self, cx: &mut Context<Self>) -> Task<()> {
        ...
        cx.background_spawn(async move {
            if supports_terminate {
                client
                    .request::<raijin_dap::requests::Terminate>(raijin_dap::TerminateArguments {
                        restart: Some(false),
                    })
                    .await
                    .ok();
                ...
                client
                    .request::<raijin_dap::requests::Disconnect>(raijin_dap::DisconnectArguments {
                        restart: Some(false),
        ...
        cx.spawn(async move |this, cx| {
            while let Some(output) = rx.next().await {
                this.update(cx, |this, _| {
                    let event = raijin_dap::OutputEvent {
                        category: None,
                        output,
                        group: None,
    ...
    fn handle_start_debugging_request(
        &mut self,
        request: raijin_dap::messages::Request,
        cx: &mut Context<Self>,
    ) -> Task<Result<()>> {
    ...
    fn handle_run_in_terminal_request(
        ...
        let request_args = match serde_json::from_value::<RunInTerminalRequestArguments>(
            ...
            Err(error) => {
                return cx.spawn(async move |session, cx| {
                    let error = serde_json::to_value(raijin_dap::ErrorResponse {
        ...
        cx.spawn(async move |session, cx| {
            ...
            let (success, body) = match result {
                Ok(pid) => (
                    true,
                    serde_json::to_value(raijin_dap::RunInTerminalResponse {
                        process_id: None,
                ...
                Err(error) => (
                    false,
                    serde_json::to_value(raijin_dap::ErrorResponse {
                        error: Some(raijin_dap::Message {
                            id: seq,
                            format: error.to_string(),
                            variables: None,
    ...
    pub fn output(
        &self,
        since: OutputToken,
    ) -> (impl Iterator<Item = &raijin_dap::OutputEvent>, OutputToken) {
    ...
    pub(crate) fn handle_dap_event(&mut self, event: Box<Events>, cx: &mut Context<Self>) {
        match *event {
            ...
            Events::Thread(event) => {
                ...
                match event.reason {
                    raijin_dap::ThreadEventReason::Started => {
                        self.active_snapshot
                            .thread_states
                            .continue_thread(thread_id);
                    }
                    raijin_dap::ThreadEventReason::Exited => {
            ...
            Events::Module(event) => {
                match event.reason {
                    raijin_dap::ModuleEventReason::New => {
                        self.active_snapshot.modules.push(event.module);
                    }
                    raijin_dap::ModuleEventReason::Changed => {
                    ...
                    raijin_dap::ModuleEventReason::Removed => {
                        self.active_snapshot
                            .modules
    ...
    pub fn threads(&mut self, cx: &mut Context<Self>) -> Vec<(raijin_dap::Thread, ThreadStatus)> {
        self.fetch(
    ...
    pub fn completions(
        ...
        cx: &mut Context<Self>,
    ) -> Task<Result<Vec<raijin_dap::CompletionItem>>> {
    ...
    pub fn scopes(&mut self, stack_frame_id: u64, cx: &mut Context<Self>) -> &[raijin_dap::Scope] {
    ...
    pub fn data_breakpoint_info(
        ...
        mode: Option<String>,
        cx: &mut Context<Self>,
    ) -> Task<Option<raijin_dap::DataBreakpointInfoResponse>> {
    ...
    pub fn location(
        ...
        reference: u64,
        cx: &mut Context<Self>,
    ) -> Option<raijin_dap::LocationsResponse> {
        self.fetch(
...
async fn get_or_install_companion(node: NodeRuntime, cx: &mut AsyncApp) -> Result<PathBuf> {
    ...
    let dir = raijin_paths::debug_adapters_dir().join("js-debug-companion");
