...
use raijin_fs::{Fs, RemoveOptions};
use futures::{
...
use raijin_language::{Buffer, LanguageToolchainStore};
...
use raijin_task::{DebugScenario, SharedTaskContext, SpawnInTerminal, TaskTemplate};
...
impl DapStore {
                ...
                cx.spawn(async move |_, cx| {
                    ...
                    if let Some(c) = binary.connection {
                        ...
                        if remote.read_with(cx, |remote, _cx| remote.shares_network_interface()) {
                            ...
                            port = raijin_dap::transport::TcpTransport::unused_port(host).await?;
    ...
    pub fn resolve_inline_value_locations(
        ...
        inline_value_locations: Vec<raijin_dap::inline_value::InlineValueLocation>,
        cx: &mut Context<Self>,
        ...
        cx.spawn(async move |_, cx| {
            ...
            for inline_value_location in inline_value_locations.iter() {
                let point = snapshot.point_to_point_utf16(raijin_language::Point::new(
                    ...
                    VariableLookupKind::Variable => {
                        let variable_search =
                            if inline_value_location.scope
                                == raijin_dap::inline_value::VariableScope::Local
                            {
...
#[async_trait]
impl raijin_dap::adapters::DapDelegate for DapAdapterDelegate {
