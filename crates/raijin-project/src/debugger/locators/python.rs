...
use raijin_dap::{DapLocator, DebugRequest, adapters::DebugAdapterName};
use inazuma::{BackgroundExecutor, SharedString};

use raijin_task::{DebugScenario, SpawnInTerminal, TaskTemplate, VariableName};
...
impl DapLocator for PythonLocator {
    ...
    async fn create_scenario(
        ...
        let program_position = mod_name
            .is_none()
            .then(|| {
                let raijin_file = VariableName::File.template_value_with_whitespace();
                build_config.args.iter().position(|arg| *arg == raijin_file)
            })
            .flatten();
