...
use raijin_dap::{StackFrameId, client::SessionId};
use inazuma::{
    App, AppContext, AsyncApp, Context, Entity, EntityId, EventEmitter, Subscription, Task,
};
use itertools::Itertools;
use raijin_language::{Buffer, BufferSnapshot, proto::serialize_anchor as serialize_text_anchor};
use raijin_rpc::{
...
impl BreakpointStore {
        ...
        this.update(&mut cx, move |this, cx| {
            ...
            bps.breakpoints = message
                ...
                .filter_map(|breakpoint| {
                    let position =
                        raijin_language::proto::deserialize_anchor(breakpoint.position.clone()?)?;
    ...
    async fn handle_toggle_breakpoint(
        ...
        let position = raijin_language::proto::deserialize_anchor(
            breakpoint
                .position
                .clone()
    ...
    pub(crate) fn update_session_breakpoint(
        ...
        _: raijin_dap::BreakpointEventReason,
        breakpoint: raijin_dap::Breakpoint,
    ) {
        maybe!({
    ...
    pub fn abs_path_from_buffer(buffer: &Entity<Buffer>, cx: &App) -> Option<Arc<Path>> {
        raijin_worktree::File::from_dyn(buffer.read(cx).file())
...
impl Breakpoint {
    ...
    fn from_proto(breakpoint: raijin_client::proto::Breakpoint) -> Option<Self> {
        Some(Self {
...
impl From<SourceBreakpoint> for raijin_dap::SourceBreakpoint {
