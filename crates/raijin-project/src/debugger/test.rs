...
fn register_default_handlers(session: &Session, client: &Arc<DebugAdapterClient>, cx: &mut App) {
    client.on_request::<raijin_dap::requests::Initialize, _>(move |_, _| Ok(Default::default()));
    let paths = session.breakpoint_store.read(cx).breakpoint_paths();

    client.on_request::<raijin_dap::requests::SetBreakpoints, _>(move |_, args| {
    ...
    client.on_request::<raijin_dap::requests::Launch, _>(move |_, _| Ok(()));

    client.on_request::<raijin_dap::requests::SetExceptionBreakpoints, _>(move |_, _| {
        Ok(raijin_dap::SetExceptionBreakpointsResponse { breakpoints: None })
    });

    client.on_request::<raijin_dap::requests::Disconnect, _>(move |_, _| Ok(()));
