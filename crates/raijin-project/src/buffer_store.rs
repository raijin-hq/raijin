...
use raijin_worktree::{File, PathChange, ProjectEntryId, Worktree, WorktreeId, WorktreeSettings};
...
pub enum BufferStoreEvent {
    ...
    BufferChangedFilePath {
        buffer: Entity<Buffer>,
        old_file: Option<Arc<dyn raijin_language::File>>,
    },
}

#[derive(Default, Debug, Clone)]
pub struct ProjectTransaction(pub HashMap<Entity<Buffer>, raijin_language::Transaction>);
            ...
            proto::create_buffer_for_peer::Variant::State(mut state) => {
                ...
                let buffer_result = maybe!({
                    ...
                    if let Some(file) = state.file.take() {
                        let worktree_id = raijin_worktree::WorktreeId::from_proto(file.worktree_id);
                        let worktree = self
                            .worktree_store
                            .read(cx)
...
impl LocalBufferStore {
                    ...
                    if has_changed_file {
                        downstream_client
                            .send(proto::UpdateBufferFile {
                                ...
                                file: Some(raijin_language::File::to_proto(&*new_file, cx)),
                            })
                            .log_err();
                    }
                    downstream_client
    ...
    fn subscribe_to_worktree(
        ...
        cx.subscribe(worktree, |this, worktree, event, cx| {
            if worktree.read(cx).is_local()
                && let raijin_worktree::Event::UpdatedEntries(changes) = event
            {
    ...
    fn local_worktree_entry_changed(
        ...
        path: &Arc<RelPath>,
        worktree: &Entity<raijin_worktree::Worktree>,
        snapshot: &raijin_worktree::Snapshot,
        cx: &mut Context<BufferStore>,
    ) -> Option<()> {
    ...
    fn create_buffer(
        ...
        cx.spawn(async move |buffer_store, cx| {
            let buffer = cx.new(|cx| {
                Buffer::local("", cx)
                    .with_language(language.unwrap_or_else(|| raijin_language::PLAIN_TEXT.clone()), cx)
            });
...
impl BufferStore {
        ...
        let buffers = self
            ...
            .map(|buffer| {
                ...
                proto::BufferVersion {
                    ...
                    version: raijin_language::proto::serialize_version(&buffer.version),
                }
            })
            .collect();
            ...
            if let Some(buffer) = self.get(buffer_id) {
                ...
                client
                    .send(proto::BufferReloaded {
                        project_id,
                        buffer_id: buffer_id.into(),
                        version: raijin_language::proto::serialize_version(buffer.saved_version()),
                        mtime: buffer.saved_mtime().map(|time| time.into()),
                        line_ending: raijin_language::proto::serialize_line_ending(buffer.line_ending())
                            as i32,
                    })
                    .log_err();
    ...
    pub fn serialize_project_transaction_for_peer(
        ...
        for (buffer, transaction) in project_transaction.0 {
            ...
            serialized_transaction
                .transactions
                .push(raijin_language::proto::serialize_transaction(&transaction));
        }
        serialized_transaction
    }
