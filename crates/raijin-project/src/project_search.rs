...
use raijin_language::{Buffer, BufferSnapshot};
...
use raijin_worktree::{Entry, ProjectEntryId, Snapshot, Worktree, WorktreeSettings};
...
impl Search {
    ...
    async fn grab_buffer_snapshots(
        ...
        find_all_matches_tx: Sender<(
            Entity<Buffer>,
            BufferSnapshot,
            oneshot::Sender<(Entity<Buffer>, Vec<Range<raijin_language::Anchor>>)>,
        )>,
        results: Sender<oneshot::Receiver<(Entity<Buffer>, Vec<Range<raijin_language::Anchor>>)>>,
        mut cx: AsyncApp,
    ) {
        _ = maybe!(async move {
    ...
    async fn ensure_matched_ranges_are_reported_in_order(
        rx: Receiver<oneshot::Receiver<(Entity<Buffer>, Vec<Range<raijin_language::Anchor>>)>>,
