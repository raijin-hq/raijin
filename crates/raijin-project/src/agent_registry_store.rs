...
impl AgentRegistryStore {
    ...
    pub fn init_test_global(cx: &mut App, agents: Vec<RegistryAgent>) -> Entity<Self> {
        let fs: Arc<dyn Fs> = raijin_fs::FakeFs::new(cx.background_executor().clone());
        let store = cx.new(|_cx| Self {
            fs,
            http_client: raijin_http_client::FakeHttpClient::with_404_response(),
            agents,
            is_fetching: false,
            fetch_error: None,
...
fn registry_cache_dir() -> PathBuf {
    raijin_paths::external_agents_dir().join("registry")
}
