...
use raijin_client::{TypedEnvelope, proto};
...
impl ProgressToken {
    fn from_lsp(value: raijin_lsp::NumberOrString) -> Self {
        match value {
            raijin_lsp::NumberOrString::Number(number) => Self::Number(number),
            raijin_lsp::NumberOrString::String(string) => Self::String(SharedString::new(string)),
        }
    }

    fn to_lsp(&self) -> raijin_lsp::NumberOrString {
        match self {
            Self::Number(number) => raijin_lsp::NumberOrString::Number(*number),
            Self::String(string) => raijin_lsp::NumberOrString::String(string.to_string()),
        }
    }
...
impl LocalLspStore {
        ...
        let startup = {
            ...
            cx.spawn(async move |cx| {
                let result = async {
                    ...
                    let did_change_configuration_params = raijin_lsp::DidChangeConfigurationParams {
                    ...
                    language_server.notify::<raijin_lsp::notification::DidChangeConfiguration>(
    ...
    fn setup_lsp_messages(
        ...
        language_server
            .on_notification::<raijin_lsp::notification::PublishDiagnostics, _>({
                let adapter = adapter.clone();
            ...
            .detach();
        language_server
            .on_request::<raijin_lsp::request::WorkspaceConfiguration, _, _>({
        ...
        language_server
            .on_request::<raijin_lsp::request::WorkspaceFoldersRequest, _, _>({
                let this = lsp_store.clone();
                move |_, cx| {
                    let this = this.clone();
                    let cx = cx.clone();
        ...
        language_server
            .on_request::<raijin_lsp::request::WorkDoneProgressCreate, _, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    let this = this.clone();
                    let mut cx = cx.clone();
        ...
        language_server
            .on_request::<raijin_lsp::request::RegisterCapability, _, _>({
        ...
        language_server
            .on_request::<raijin_lsp::request::UnregisterCapability, _, _>({
                let lsp_store = lsp_store.clone();
                move |params, cx| {
                    let lsp_store = lsp_store.clone();
                    let mut cx = cx.clone();
        ...
        language_server
            .on_request::<raijin_lsp::request::ApplyWorkspaceEdit, _, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    let mut cx = cx.clone();
                    let this = this.clone();
        ...
        language_server
            .on_request::<raijin_lsp::request::InlayHintRefreshRequest, _, _>({
                let lsp_store = lsp_store.clone();
        ...
        language_server
            .on_request::<raijin_lsp::request::CodeLensRefresh, _, _>({
                let this = lsp_store.clone();
                move |(), cx| {
                    let this = this.clone();
                    let mut cx = cx.clone();
        ...
        language_server
            .on_request::<raijin_lsp::request::SemanticTokensRefresh, _, _>({
                let lsp_store = lsp_store.clone();
                let request_id = Arc::new(AtomicUsize::new(0));
                move |(), cx| {
                    let lsp_store = lsp_store.clone();
        ...
        language_server
            .on_request::<raijin_lsp::request::WorkspaceDiagnosticRefresh, _, _>({
                let this = lsp_store.clone();
                move |(), cx| {
                    let this = this.clone();
                    let mut cx = cx.clone();
        ...
        language_server
            .on_request::<raijin_lsp::request::ShowMessageRequest, _, _>({
                let this = lsp_store.clone();
                ...
                move |params, cx| {
                    ...
                    async move {
                        ...
                        let level = match params.typ {
                            raijin_lsp::MessageType::ERROR => PromptLevel::Critical,
                            raijin_lsp::MessageType::WARNING => PromptLevel::Warning,
                            _ => PromptLevel::Info,
                        };
                        ...
                        if did_update {
                            ...
                            if let Some(ref selected_action) = response {
                                let context = raijin_language::PromptResponseContext {
                                    message,
            ...
            .detach();
        language_server
            .on_notification::<raijin_lsp::notification::ShowMessage, _>({
                let this = lsp_store.clone();
                let name = name.to_string();
                move |params, cx| {
                    let this = this.clone();
        ...
        language_server
            .on_notification::<raijin_lsp::notification::Progress, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    if let Some(this) = this.upgrade() {
                        this.update(cx, |this, cx| {
        ...
        language_server
            .on_notification::<raijin_lsp::notification::LogMessage, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    if let Some(this) = this.upgrade() {
                        this.update(cx, |_, cx| {
        ...
        language_server
            .on_notification::<raijin_lsp::notification::LogTrace, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    let mut cx = cx.clone();
                    if let Some(this) = this.upgrade() {
        ...
        for buffer in &buffers {
            ...
            buffer.handle.update(cx, |buffer, cx| {
                let Some(formatting_transaction) =
                    ...
                    raijin_log::warn!(logger => "no formatting transaction");
                    return;
                };
                if formatting_transaction.edit_ids.is_empty() {
                    raijin_log::debug!(logger => "no changes made while formatting");
                    buffer.forget_transaction(formatting_transaction_id);
                    return;
                }
                if !push_to_history {
                    raijin_log::trace!(logger => "forgetting format transaction");
    ...
    async fn format_buffer_locally(
        ...
        if settings.remove_trailing_whitespace_on_save {
            raijin_log::trace!(logger => "removing trailing whitespace");
            let diff = buffer
                .handle
        ...
        if settings.ensure_final_newline_on_save {
            raijin_log::trace!(logger => "ensuring final newline");
            extend_formatting_transaction(buffer, formatting_transaction_id, cx, |buffer, cx| {
        ...
        if should_run_code_actions_on_format {
            ...
            if have_code_actions_to_run_on_format {
                raijin_log::trace!(logger => "going to run code actions on format");
        ...
        for formatter in formatters {
            let formatter = if formatter == &Formatter::Auto {
                if settings.prettier.allowed {
                    raijin_log::trace!(logger => "Formatter set to auto: defaulting to prettier");
                    &Formatter::Prettier
                } else {
                    raijin_log::trace!(logger => "Formatter set to auto: defaulting to primary language server");
                    &Formatter::LanguageServer(inazuma_settings_framework::LanguageServerFormatterSpecifier::Current)
                }
            } else {
                formatter
            };
            ...
            .await
            {
                raijin_log::error!(logger => "Formatter failed, skipping: {err:#}");
            }
        }
    ...
    async fn apply_formatter(
        ...
        match formatter {
            Formatter::None => {
                raijin_log::trace!(logger => "skipping formatter 'none'");
                return Ok(());
            }
            Formatter::Auto => {
            ...
            Formatter::Prettier => {
                let logger = raijin_log::scoped!(logger => "prettier");
                raijin_log::trace!(logger => "formatting");
                let _timer = raijin_log::time!(logger => "Formatting buffer via prettier");
                ...
                let Some(diff) = diff else {
                    raijin_log::trace!(logger => "No changes");
                    return Ok(());
                };
            ...
            Formatter::External { command, arguments } => {
                let logger = raijin_log::scoped!(logger => "command");
                raijin_log::trace!(logger => "formatting");
                let _timer = raijin_log::time!(logger => "Formatting buffer via external command");
            ...
            Formatter::LanguageServer(specifier) => {
                let logger = raijin_log::scoped!(logger => "language-server");
                raijin_log::trace!(logger => "formatting");
                let _timer = raijin_log::time!(logger => "Formatting buffer using language server");

                let Some(buffer_path_abs) = buffer.abs_path.as_ref() else {
                    raijin_log::warn!(logger => "Cannot format buffer that is not backed by a file on disk using language servers. Skipping");
                    return Ok(());
                };

                let language_server = match specifier {
                    inazuma_settings_framework::LanguageServerFormatterSpecifier::Specific { name } => {
                    ...
                    inazuma_settings_framework::LanguageServerFormatterSpecifier::Current => adapters_and_servers
                        .iter()
                ...
                let edits = if let Some(ranges) = buffer.ranges.as_ref() {
                    raijin_log::trace!(logger => "formatting ranges");
                    ...
                    raijin_log::trace!(logger => "formatting full");
                    Self::format_via_lsp(
                        &lsp_store,
                        &buffer.handle,
                        buffer_path_abs,
            ...
            Formatter::CodeAction(code_action_name) => {
                let logger = raijin_log::scoped!(logger => "code-actions");
                raijin_log::trace!(logger => "formatting");
                let _timer = raijin_log::time!(logger => "Formatting buffer using code actions");

                let Some(buffer_path_abs) = buffer.abs_path.as_ref() else {
                    raijin_log::warn!(logger => "Cannot format buffer that is not backed by a file on disk using code actions. Skipping");
                    return Ok(());
                };

                let code_action_kind: CodeActionKind = code_action_name.clone().into();
                raijin_log::trace!(logger => "Attempting to resolve code actions {:?}", &code_action_kind);
                ...
                if actions_and_servers.is_empty() {
                    raijin_log::warn!(logger => "No code actions were resolved, continuing");
                    return Ok(());
                }
                ...
                'actions: for (mut action, server_index) in actions_and_servers {
                    ...
                    raijin_log::trace!(logger => "Executing {}", describe_code_action(&action));
                    ...
                    if let Some(edit) = action.lsp_action.edit().cloned() {
                        ...
                        if let Some(document_changes) = edit.document_changes {
                            match document_changes {
                                raijin_lsp::DocumentChanges::Edits(edits) => operations.extend(
                                    edits.into_iter().map(raijin_lsp::DocumentChangeOperation::Edit),
                                ),
                                raijin_lsp::DocumentChanges::Operations(ops) => operations = ops,
                            }
                        } else if let Some(changes) = edit.changes {
                            operations.extend(changes.into_iter().map(|(uri, edits)| {
                                raijin_lsp::DocumentChangeOperation::Edit(raijin_lsp::TextDocumentEdit {
                                    text_document: raijin_lsp::OptionalVersionedTextDocumentIdentifier {
                                        uri,
                                        version: None,
                                    },
                        ...
                        for operation in operations {
                            let op = match operation {
                                raijin_lsp::DocumentChangeOperation::Edit(op) => op,
                                raijin_lsp::DocumentChangeOperation::Op(_) => {
                                    raijin_log::warn!(
                                        logger =>
                        ...
                        if edits.is_empty() {
                            raijin_log::warn!(logger => "No edits resolved from LSP");
                            continue;
                        }
                        ...
                        extend_formatting_transaction(
                            ...
                            |buffer, cx| {
                                ...
                                buffer.edit(edits, None, cx);
                                raijin_log::info!("Applied edits. New Content: {:?}", buffer.text());
                            },
                        )?;
                    }
                    ...
                    raijin_log::info!(logger => "Executing command {}", &command.command);
                    ...
                    let execute_command_result = server
                        .request::<raijin_lsp::request::ExecuteCommand>(
    ...
    pub async fn format_ranges_via_lsp(
        ...
        let text_document = raijin_lsp::TextDocumentIdentifier::new(uri);
        ...
        let lsp_edits = {
            ...
            for range in lsp_ranges {
                if let Some(mut edit) = language_server
                    .request::<raijin_lsp::request::RangeFormatting>(
                        raijin_lsp::DocumentRangeFormattingParams {
    ...
    async fn format_via_lsp(
        ...
        let logger = raijin_log::scoped!("lsp_format");
        raijin_log::debug!(logger => "Formatting via LSP");
        ...
        let lsp_edits = if matches!(formatting_provider, Some(p) if *p != OneOf::Left(false)) {
            let _timer = raijin_log::time!(logger => "format-full");
            language_server
                .request::<raijin_lsp::request::Formatting>(
                    raijin_lsp::DocumentFormattingParams {
                        text_document,
                        options: lsp_command::lsp_formatting_options(settings),
                        work_done_progress_params: Default::default(),
                    },
            ...
            let _timer = raijin_log::time!(logger => "format-range");
            let buffer_start = raijin_lsp::Position::new(0, 0);
            let buffer_end = buffer.read_with(cx, |b, _| point_to_lsp(b.max_point_utf16()));
            language_server
                .request::<raijin_lsp::request::RangeFormatting>(
                    raijin_lsp::DocumentRangeFormattingParams {
                        text_document: text_document.clone(),
                        range: raijin_lsp::Range::new(buffer_start, buffer_end),
                        options: lsp_command::lsp_formatting_options(settings),
                        work_done_progress_params: Default::default(),
                    },
                    request_timeout,
        ...
        match &mut action.lsp_action {
            LspAction::Action(lsp_action) => {
                    ...
                    && (lsp_action.command.is_none() || lsp_action.edit.is_none())
                {
                    **lsp_action = lang_server
                        .request::<raijin_lsp::request::CodeActionResolveRequest>(
                            *lsp_action.clone(),
                            request_timeout,
                        )
                        .await
            ...
            LspAction::CodeLens(lens) => {
                if !action.resolved && GetCodeLens::can_resolve_lens(&lang_server.capabilities()) {
                    *lens = lang_server
                        .request::<raijin_lsp::request::CodeLensResolve>(lens.clone(), request_timeout)
                        .await
                        .into_response()?;
                }
            }
    ...
    pub(crate) fn unregister_old_buffer_from_language_servers(
        ...
        let Ok(file_url) = raijin_lsp::Uri::from_file_path(old_path.as_path()) else {
    ...
    async fn get_server_code_actions_from_action_kinds(
        lsp_store: &WeakEntity<LspStore>,
        language_server_id: LanguageServerId,
        code_action_kinds: Vec<raijin_lsp::CodeActionKind>,
        buffer: &Entity<Buffer>,
        cx: &mut AsyncApp,
    ...
    pub fn edits_from_lsp(
        &mut self,
        buffer: &Entity<Buffer>,
        lsp_edits: impl 'static + Send + IntoIterator<Item = raijin_lsp::TextEdit>,
        server_id: LanguageServerId,
        version: Option<i32>,
        cx: &mut Context<LspStore>,
        ...
        cx.background_spawn(async move {
            ...
            while let Some((range, mut new_text)) = lsp_edits.next() {
                ...
                if range.end.row > range.start.row {
                    ...
                    let range_edits = raijin_language::text_diff(old_text.as_str(), &new_text);
    ...
    pub(crate) async fn deserialize_workspace_edit(
        ...
        if let Some(document_changes) = edit.document_changes {
            match document_changes {
                raijin_lsp::DocumentChanges::Edits(edits) => {
                    operations.extend(edits.into_iter().map(raijin_lsp::DocumentChangeOperation::Edit))
                }
                raijin_lsp::DocumentChanges::Operations(ops) => operations = ops,
            }
        } else if let Some(changes) = edit.changes {
        ...
        for operation in operations {
            match operation {
                raijin_lsp::DocumentChangeOperation::Op(raijin_lsp::ResourceOp::Create(op)) => {
                    let abs_path = op
                        .uri
                        .to_file_path()
                    ...
                    if abs_path.ends_with("/") {
                        ...
                        fs.create_file(
                            &abs_path,
                            op.options
                                .map(|options| raijin_fs::CreateOptions {
                ...
                raijin_lsp::DocumentChangeOperation::Op(raijin_lsp::ResourceOp::Rename(op)) => {
                    ...
                    let options = raijin_fs::RenameOptions {
                        overwrite: op
                            .options
                            .as_ref()
                ...
                raijin_lsp::DocumentChangeOperation::Op(raijin_lsp::ResourceOp::Delete(op)) => {
                    let abs_path = op
                        .uri
                        .to_file_path()
                        .map_err(|()| anyhow!("can't convert URI to path"))?;
                    let options = op
                        .options
                        .map(|options| raijin_fs::RemoveOptions {
                ...
                raijin_lsp::DocumentChangeOperation::Edit(op) => {
    ...
    async fn on_lsp_workspace_edit(
        this: WeakEntity<LspStore>,
        params: raijin_lsp::ApplyWorkspaceEditParams,
        server_id: LanguageServerId,
        cx: &mut AsyncApp,
    ) -> Result<raijin_lsp::ApplyWorkspaceEditResponse> {
        ...
        Ok(raijin_lsp::ApplyWorkspaceEditResponse {
            applied: true,
            failed_change: None,
            failure_reason: None,
        })
        ...
        for watcher in watchers {
                ...
                Self::worktree_and_path_for_file_watcher(&worktrees, watcher, cx)
            {
                ...
                let (path, pattern) = match &watcher.glob_pattern {
                    raijin_lsp::GlobPattern::String(s) => {
                    ...
                    raijin_lsp::GlobPattern::Relative(rp) => {
                        let Ok(mut base_uri) = match &rp.base_uri {
                            raijin_lsp::OneOf::Left(workspace_folder) => &workspace_folder.uri,
                            raijin_lsp::OneOf::Right(base_uri) => base_uri,
                        }
                        .to_file_path() else {
                            continue;
                        };
...
pub struct LspStore {
    ...
    pub lsp_server_capabilities: HashMap<LanguageServerId, raijin_lsp::ServerCapabilities>,
...
struct CoreSymbol {
    ...
    pub name: String,
    pub kind: raijin_lsp::SymbolKind,
...
impl LspStore {
    ...
    fn on_buffer_event(
        ...
        match event {
            ...
            raijin_language::BufferEvent::Reloaded => {
                    ...
                    this.update(cx, |this, cx| {
                        ...
                        for handle in this.buffer_store.read(cx).buffers() {
                            ...
                            if buffer.language().is_none()
                                || buffer.language() == Some(&*raijin_language::PLAIN_TEXT)
                            {
    ...
    fn detect_language_for_buffer(
        ...
        buffer_handle: &Entity<Buffer>,
        cx: &mut Context<Self>,
    ) -> Option<raijin_language::AvailableLanguage> {
    ...
    where
        F: FnMut(&raijin_lsp::ServerCapabilities) -> bool,
    {
    ...
    fn all_capable_for_proto_request<F>(
        ...
        mut check: F,
        cx: &App,
    ) -> Vec<(raijin_lsp::LanguageServerId, raijin_lsp::LanguageServerName)>
    where
        F: FnMut(&raijin_lsp::LanguageServerName, &raijin_lsp::ServerCapabilities) -> bool,
    {
        let Some(language) = buffer.read(cx).language().cloned() else {
            return Vec::default();
        };
    ...
    where
        R: LspCommand,
        <R::LspRequest as raijin_lsp::request::Request>::Result: Send,
        <R::LspRequest as raijin_lsp::request::Request>::Params: Send,
    {
        ...
        if let Some((upstream_client, project_id)) = self.upstream_client() {
            ...
            cx.spawn(async move |this, cx| {
                ...
                let _result = lang_server
                    .request::<raijin_lsp::request::ExecuteCommand>(raijin_lsp::ExecuteCommandParams {
                        command: command.command.clone(),
                        arguments: command.arguments.clone().unwrap_or_default(),
                        ..raijin_lsp::ExecuteCommandParams::default()
                    }, request_timeout)
                    .await.into_response()
                    .context("execute command")?;
        ...
        if let Some((client, project_id)) = self.upstream_client() {
            ...
            cx.background_spawn(async move {
                client
                    ...
                    .await?
                    .transaction
                    .map(raijin_language::proto::deserialize_transaction)
                    .transpose()
            })
    ...
    async fn resolve_completion_local(
        server: Arc<raijin_lsp::LanguageServer>,
        completions: Rc<RefCell<Box<[Completion]>>>,
        ...
        let request = {
            ...
            match &completion.source {
                CompletionSource::Lsp {
                    ...
                    server.request::<raijin_lsp::request::ResolveCompletionItem>(
    ...
    async fn regenerate_completion_labels(
        ...
        let mut new_label = match completion_item {
            Some(completion_item) => {
                ...
                // the resolvable properties Raijin advertises. Regenerate labels here to handle this.
    ...
    pub fn apply_additional_edits_for_completion(
        ...
        completion_index: usize,
        push_to_history: bool,
        all_commit_ranges: Vec<Range<raijin_language::Anchor>>,
        cx: &mut Context<Self>,
    ) -> Task<Result<Option<Transaction>>> {
        if let Some((client, project_id)) = self.upstream_client() {
            let buffer = buffer_handle.read(cx);
            ...
            cx.spawn(async move |_, cx| {
                let request = {
                    ...
                    proto::ApplyCompletionAdditionalEdits {
                        ...
                        all_commit_ranges: all_commit_ranges
                            .iter()
                            .cloned()
                            .map(raijin_language::proto::serialize_anchor_range)
                            .collect(),
                    }
                };
                ...
                let transaction = raijin_language::proto::deserialize_transaction(transaction)?;
                buffer_handle
                    .update(cx, |buffer, _| {
                ...
                if let Some(edits) = additional_text_edits {
                    ...
                    buffer_handle.update(cx, |buffer, cx| {
                        ...
                        for (range, text) in edits {
                            ...
                            // https://github.com/raijin-industries/raijin/issues/26136
                            ...
                            //https://github.com/raijin-industries/raijin/pull/1871
                            if !has_overlap {
                                buffer.edit([(range, text)], None, cx);
                            }
                        }
                        ...
                        |mut acc, (server_id, uri, diagnostics, new_registration_id)| {
                            ...
                            acc.entry(server_id)
                                ...
                                .push(DocumentDiagnosticsUpdate {
                                    server_id,
                                    diagnostics: raijin_lsp::PublishDiagnosticsParams {
                                        uri,
                                        diagnostics,
                                        version: None,
                                    },
    ...
    pub fn symbols(&self, query: &str, cx: &mut Context<Self>) -> Task<Result<Vec<Symbol>>> {
        ...
        if let Some((upstream_client, project_id)) = self.upstream_client().as_ref() {
            ...
            struct WorkspaceSymbolsResult {
                ...
                lsp_symbols: Vec<(String, SymbolKind, raijin_lsp::Location, Option<String>)>,
            }
            ...
            for (seed, state) in local.language_server_ids.iter() {
                ...
                requests.push(
                    server
                        .request::<raijin_lsp::request::WorkspaceSymbolRequest>(
                            raijin_lsp::WorkspaceSymbolParams {
                                query: query.to_string(),
                                ..Default::default()
                            },
                            request_timeout,
                        ...
                        .map(move |response| {
                            let lsp_symbols = response
                                ...
                                .map(|symbol_response| match symbol_response {
                                    raijin_lsp::WorkspaceSymbolResponse::Flat(flat_responses) => {
                                        flat_responses
                                            .into_iter()
                                            .map(|lsp_symbol| {
                                                (
                                    ...
                                    raijin_lsp::WorkspaceSymbolResponse::Nested(nested_responses) => {
                                        nested_responses
                                            .into_iter()
    ...
    pub fn on_buffer_edited(
        ...
        let uri = raijin_lsp::Uri::from_file_path(&abs_path)
            .ok()
        ...
        for language_server in language_servers {
            ...
            let document_sync_kind = language_server
                ...
                .and_then(|sync| match sync {
                    raijin_lsp::TextDocumentSyncCapability::Kind(kind) => Some(*kind),
                    raijin_lsp::TextDocumentSyncCapability::Options(options) => options.change,
                });

            let content_changes: Vec<_> = match document_sync_kind {
                Some(raijin_lsp::TextDocumentSyncKind::FULL) => {
                    vec![raijin_lsp::TextDocumentContentChangeEvent {
                        range: None,
                        range_length: None,
                        text: next_snapshot.text(),
                    }]
                }
                Some(raijin_lsp::TextDocumentSyncKind::INCREMENTAL) => build_incremental_change(),
                _ => {
                    #[cfg(any(test, feature = "test-support"))]
                    {
    ...
    pub fn on_buffer_saved(
        ...
        let worktree_id = file.worktree_id(cx);
        let abs_path = file.as_local()?.abs_path(cx);
        let text_document = raijin_lsp::TextDocumentIdentifier {
                                ...
                                LanguageServerState::Running {
                                    ...
                                    Some(cx.spawn(async move |_, cx| {
                                        let settings =
                                            ...
                                            .ok()?;
                                        server
                                            .notify::<raijin_lsp::notification::DidChangeConfiguration>(
                                                raijin_lsp::DidChangeConfigurationParams { settings },
                                            )
                                            .ok()?;
                                        Some(())
                                    }))
    ...
    pub fn merge_diagnostic_entries<'a>(
        ...
        merge: impl Fn(&raijin_lsp::Uri, &Diagnostic, &App) -> bool + Clone,
        cx: &mut Context<Self>,
    ) -> anyhow::Result<()> {
        ...
        for mut update in diagnostic_updates {
            ...
            let document_uri = raijin_lsp::Uri::from_file_path(abs_path)
    ...
    pub fn open_buffer_for_symbol(
        ...
        if let Some((client, project_id)) = self.upstream_client() {
            ...
            let symbol_uri = if let Ok(uri) = raijin_lsp::Uri::from_file_path(symbol_abs_path) {
                uri
            } else {
    ...
    where
        <T::LspRequest as raijin_lsp::request::Request>::Params: Send,
        <T::LspRequest as raijin_lsp::request::Request>::Result: Send,
    {
        let sender_id = envelope.original_sender_id().unwrap_or_default();
    ...
    async fn handle_start_language_server(
        ...
        let server_capabilities =
            serde_json::from_str::<raijin_lsp::ServerCapabilities>(&envelope.payload.capabilities)
                .with_context(|| {
                    format!(
    ...
    pub(super) fn did_rename_entry(
        ...
        maybe!({
            ...
            let old_uri = raijin_lsp::Uri::from_file_path(old_path)
                .ok()
                .map(|uri| uri.to_string())?;
            let new_uri = raijin_lsp::Uri::from_file_path(new_path)
                .ok()
                .map(|uri| uri.to_string())?;
    ...
    fn lsp_notify_abs_paths_changed(
        ...
        maybe!({
            ...
            let changes = changes
                ...
                .filter_map(|event| {
                    let typ = match event.kind? {
                        PathEventKind::Created => raijin_lsp::FileChangeType::CREATED,
                        PathEventKind::Removed => raijin_lsp::FileChangeType::DELETED,
                        PathEventKind::Changed | PathEventKind::Rescan => {
                            raijin_lsp::FileChangeType::CHANGED
                        }
                    };
                    Some(raijin_lsp::FileEvent {
            ...
            if !changes.is_empty() {
                server
                    .notify::<raijin_lsp::notification::DidChangeWatchedFiles>(
                        raijin_lsp::DidChangeWatchedFilesParams { changes },
                    )
                    .ok();
            }
            Some(())
    ...
    fn on_lsp_progress(
        &mut self,
        progress_params: raijin_lsp::ProgressParams,
        language_server_id: LanguageServerId,
        disk_based_diagnostics_progress_token: Option<String>,
        cx: &mut Context<Self>,
    ) {
        match progress_params.value {
            raijin_lsp::ProgressParamsValue::WorkDone(progress) => {
            ...
            raijin_lsp::ProgressParamsValue::WorkspaceDiagnostic(report) => {
                let registration_id = match progress_params.token {
                    raijin_lsp::NumberOrString::Number(_) => None,
                    raijin_lsp::NumberOrString::String(token) => token
    ...
    fn handle_work_done_progress(
        &mut self,
        progress: raijin_lsp::WorkDoneProgress,
        language_server_id: LanguageServerId,
        disk_based_diagnostics_progress_token: Option<String>,
        token: ProgressToken,
        cx: &mut Context<Self>,
        ...
        match progress {
            raijin_lsp::WorkDoneProgress::Begin(report) => {
            ...
            raijin_lsp::WorkDoneProgress::Report(report) => self.on_lsp_work_progress(
                language_server_id,
                token,
                LanguageServerProgress {
                    title: None,
            ...
            raijin_lsp::WorkDoneProgress::End(_) => {
    ...
    pub async fn handle_resolve_completion_documentation(
        ...
        let completion = this
            .read_with(&cx, |this, cx| {
                ...
                anyhow::Ok(cx.background_spawn(async move {
                    ...
                    if can_resolve {
                        server
                            .request::<raijin_lsp::request::ResolveCompletionItem>(
                                lsp_completion,
                                request_timeout,
                            )
                            .await
        ...
        let documentation = match completion.documentation {
            Some(raijin_lsp::Documentation::String(text)) => text,

            Some(raijin_lsp::Documentation::MarkupContent(raijin_lsp::MarkupContent { kind, value })) => {
                documentation_is_markdown = kind == raijin_lsp::MarkupKind::Markdown;
                value
            }
    ...
    async fn handle_on_type_formatting(
        ...
        let transaction = on_type_formatting
            .await?
            .as_ref()
            .map(raijin_language::proto::serialize_transaction);
    ...
    async fn handle_open_buffer_for_symbol(
        ...
        this.update(&mut cx, |this, cx| {
            let is_private = buffer
                ...
                .unwrap_or_default();
            if is_private {
                Err(anyhow!(raijin_rpc::ErrorCode::UnsharedItem))
            } else {
                this.buffer_store
                    .update(cx, |buffer_store, cx| {
    ...
    async fn handle_apply_additional_edits_for_completion(
        ...
        let (buffer, completion, all_commit_ranges) = this.update(&mut cx, |this, cx| {
            ...
            let all_commit_ranges = envelope
                ...
                .all_commit_ranges
                .into_iter()
                .map(raijin_language::proto::deserialize_anchor_range)
    ...
    pub fn format(
        ...
        let logger = raijin_log::scoped!("format");
        if self.as_local().is_some() {
            raijin_log::trace!(logger => "Formatting locally");
            let logger = raijin_log::scoped!(logger => "local");
            let buffers = buffers
                .into_iter()
                .map(|buffer_handle| {
                    let buffer = buffer_handle.read(cx);
            ...
            cx.spawn(async move |lsp_store, cx| {
                ...
                raijin_log::trace!(logger => "Formatting {:?} buffers", formattable_buffers.len());

                let format_timer = raijin_log::time!(logger => "Formatting buffers");
                ...
                raijin_log::trace!(logger => "Formatting completed with result {:?}", result.as_ref().map(|_| "<project-transaction>"));
            ...
            raijin_log::trace!(logger => "Formatting remotely");
            let logger = raijin_log::scoped!(logger => "remote");
            ...
            let buffer_store = self.buffer_store();
            cx.spawn(async move |lsp_store, cx| {
                raijin_log::trace!(logger => "Sending remote format request");
                let request_timer = raijin_log::time!(logger => "remote format request");
                let result = client
                ...
                raijin_log::trace!(logger => "Remote format request resolved to {:?}", result.as_ref().map(|_| "<project_transaction>"));
                ...
                let transaction_response = result?;
                let _timer = raijin_log::time!(logger => "deserializing project transaction");
                buffer_store
                    .update(cx, |buffer_store, cx| {
                        buffer_store.deserialize_project_transaction(
                            transaction_response,
            ...
            raijin_log::trace!(logger => "Not formatting");
            Task::ready(Ok(ProjectTransaction::default()))
        }
    }
    ...
    pub fn merge_lsp_diagnostics(
        &mut self,
        source_kind: DiagnosticSourceKind,
        lsp_diagnostics: Vec<DocumentDiagnosticsUpdate<raijin_lsp::PublishDiagnosticsParams>>,
        merge: impl Fn(&raijin_lsp::Uri, &Diagnostic, &App) -> bool + Clone,
        cx: &mut Context<Self>,
    ) -> Result<()> {
    ...
    fn lsp_to_document_diagnostics(
        ...
        source_kind: DiagnosticSourceKind,
        server_id: LanguageServerId,
        mut lsp_diagnostics: raijin_lsp::PublishDiagnosticsParams,
        disk_based_sources: &[String],
        registration_id: Option<SharedString>,
    ) -> DocumentDiagnostics {
    ...
    fn insert_newly_running_language_server(
        ...
        self.buffer_store.clone().update(cx, |buffer_store, cx| {
            ...
            for buffer_handle in buffer_store.buffers() {
                ...
                if local.registered_buffers.contains_key(&buffer_id) {
                    let abs_path = file.abs_path(cx);
                    let uri = match raijin_lsp::Uri::from_file_path(&abs_path) {
                        Ok(uri) => uri,
                        Err(()) => {
            ...
            if let Some((LanguageServerState::Running { server, .. }, status)) = server.zip(status)
            {
                for (token, progress) in &status.pending_work {
                    ...
                    if progress.is_cancellable {
                        server
                            .notify::<raijin_lsp::notification::WorkDoneProgressCancel>(
    ...
    pub(super) fn update_local_worktree_language_servers(
        ...
        for server_id in &language_server_ids {
            ...
            {
                let params = raijin_lsp::DidChangeWatchedFilesParams {
                    changes: changes
                        .iter()
                        .filter_map(|(path, _, change)| {
                            ...
                            let typ = match change {
                                ...
                                PathChange::Added => raijin_lsp::FileChangeType::CREATED,
                                PathChange::Removed => raijin_lsp::FileChangeType::DELETED,
                                PathChange::Updated => raijin_lsp::FileChangeType::CHANGED,
                                PathChange::AddedOrUpdated => raijin_lsp::FileChangeType::CHANGED,
                            };
                            let uri = raijin_lsp::Uri::from_file_path(
                                worktree_handle.read(cx).absolutize(&path),
                            )
                            .ok()?;
                            Some(raijin_lsp::FileEvent { uri, typ })
                        })
                        .collect(),
                };
                if !params.changes.is_empty() {
                    server
                        .notify::<raijin_lsp::notification::DidChangeWatchedFiles>(params)
                        .ok();
                }
            }
        }
    ...
    fn deserialize_symbol(serialized_symbol: proto::Symbol) -> Result<CoreSymbol> {
        ...
        let kind = unsafe { mem::transmute::<i32, raijin_lsp::SymbolKind>(serialized_symbol.kind) };
    ...
    fn register_server_capabilities(
        &mut self,
        server_id: LanguageServerId,
        params: raijin_lsp::RegistrationParams,
        cx: &mut Context<Self>,
    ) -> anyhow::Result<()> {
        let server = self
        ...
        for reg in params.registrations {
            match reg.method.as_str() {
                ...
                "workspace/didChangeWorkspaceFolders" => {
                    ...
                    let caps = raijin_lsp::WorkspaceFoldersServerCapabilities {
                        supported: Some(true),
                ...
                "textDocument/codeAction" => {
                    ...
                    let provider = match options {
                        OneOf::Left(value) => raijin_lsp::CodeActionProviderCapability::Simple(value),
                ...
                "textDocument/hover" => {
                    let options = parse_register_capabilities(reg)?;
                    let provider = match options {
                        OneOf::Left(value) => raijin_lsp::HoverProviderCapability::Simple(value),
                        OneOf::Right(caps) => caps,
                    };
                    server.update_capabilities(|capabilities| {
                ...
                "textDocument/didChange" => {
                    if let Some(sync_kind) = reg
                        ...
                        .map(serde_json::from_value::<raijin_lsp::TextDocumentSyncKind>)
                        .transpose()?
                    {
                        server.update_capabilities(|capabilities| {
                            ...
                            capabilities.text_document_sync =
                                Some(raijin_lsp::TextDocumentSyncCapability::Options(sync_options));
                        });
                        notify_server_capabilities_updated(&server, cx);
                    }
                }
                        ...
                        .transpose()?
                    {
                        server.update_capabilities(|capabilities| {
                            let mut sync_options =
                                Self::take_text_document_sync_options(capabilities);
                            sync_options.save =
                                Some(TextDocumentSyncSaveOptions::SaveOptions(raijin_lsp::SaveOptions {
                                    include_text,
                                }));
                            capabilities.text_document_sync =
                                Some(raijin_lsp::TextDocumentSyncCapability::Options(sync_options));
                ...
                "textDocument/documentColor" => {
                    let options = parse_register_capabilities(reg)?;
                    let provider = match options {
                        OneOf::Left(value) => raijin_lsp::ColorProviderCapability::Simple(value),
                        OneOf::Right(caps) => caps,
                    };
                    server.update_capabilities(|capabilities| {
                ...
                "textDocument/foldingRange" => {
                    let options = parse_register_capabilities(reg)?;
                    let provider = match options {
                        OneOf::Left(value) => raijin_lsp::FoldingRangeProviderCapability::Simple(value),
                        OneOf::Right(caps) => caps,
                    };
                    server.update_capabilities(|capabilities| {
    ...
    fn unregister_server_capabilities(
        &mut self,
        server_id: LanguageServerId,
        params: raijin_lsp::UnregistrationParams,
        cx: &mut Context<Self>,
    ) -> anyhow::Result<()> {
        let server = self
            .language_server_for_id(server_id)
            ...
            match unreg.method.as_str() {
                ...
                "workspace/didChangeWorkspaceFolders" => {
                    server.update_capabilities(|capabilities| {
                        capabilities
                            .workspace
                            .get_or_insert_with(|| raijin_lsp::WorkspaceServerCapabilities {
    ...
    fn take_text_document_sync_options(
        capabilities: &mut raijin_lsp::ServerCapabilities,
    ) -> raijin_lsp::TextDocumentSyncOptions {
        match capabilities.text_document_sync.take() {
            Some(raijin_lsp::TextDocumentSyncCapability::Options(sync_options)) => sync_options,
            Some(raijin_lsp::TextDocumentSyncCapability::Kind(sync_kind)) => {
                let mut sync_options = raijin_lsp::TextDocumentSyncOptions::default();
                sync_options.change = Some(sync_kind);
                sync_options
            }
            None => raijin_lsp::TextDocumentSyncOptions::default(),
        }
    }
        ...
        loop {
            ...
            'request: loop {
                ...
                let Ok(previous_result_ids) = lsp_store.update(cx, |lsp_store, _| {
                    lsp_store
                        ...
                        .filter_map(|(abs_path, result_id)| {
                            ...
                            Some(raijin_lsp::PreviousResultId {
                                uri,
                ...
                let response_result = server
                    .request_with_timer::<raijin_lsp::WorkspaceDiagnosticRequest, _>(
                        raijin_lsp::WorkspaceDiagnosticParams {
                            previous_result_ids,
                            identifier: identifier.clone(),
                            work_done_progress_params: Default::default(),
                            partial_result_params: raijin_lsp::PartialResultParams {
                                partial_result_token: Some(raijin_lsp::ProgressToken::String(token)),
                            },
                        },
...
fn buffer_diagnostic_identifier(options: &DiagnosticServerCapabilities) -> Option<SharedString> {
    match &options {
        raijin_lsp::DiagnosticServerCapabilities::Options(diagnostic_options) => diagnostic_options
            .identifier
            .as_deref()
            .map(SharedString::new),
        raijin_lsp::DiagnosticServerCapabilities::RegistrationOptions(registration_options) => {
...
fn workspace_diagnostic_identifier(
    ...
    match &options {
        raijin_lsp::DiagnosticServerCapabilities::Options(diagnostic_options) => {
...
impl From<raijin_lsp::Documentation> for CompletionDocumentation {
    fn from(docs: raijin_lsp::Documentation) -> Self {
        match docs {
            raijin_lsp::Documentation::String(text) => {
            ...
            raijin_lsp::Documentation::MarkupContent(raijin_lsp::MarkupContent { kind, value }) => match kind {
...
impl LspAdapterDelegate for LocalLspAdapterDelegate {
    ...
    fn update_status(&self, server_name: LanguageServerName, status: raijin_language::BinaryStatus) {
...
async fn populate_labels_for_symbols(
    ...
    for (language, mut symbols) in symbols_by_language {
        label_params.clear();
        label_params.extend(symbols.iter_mut().map(|symbol| raijin_language::Symbol {
...
fn include_text(server: &raijin_lsp::LanguageServer) -> Option<bool> {
    match server.capabilities().text_document_sync.as_ref()? {
        raijin_lsp::TextDocumentSyncCapability::Options(opts) => match opts.save.as_ref()? {
            // Server wants didSave but didn't specify includeText.
            raijin_lsp::TextDocumentSyncSaveOptions::Supported(true) => Some(false),
            // Server doesn't want didSave at all.
            raijin_lsp::TextDocumentSyncSaveOptions::Supported(false) => None,
            // Server provided SaveOptions.
            raijin_lsp::TextDocumentSyncSaveOptions::SaveOptions(save_options) => {
                Some(save_options.include_text.unwrap_or(false))
            }
        },
        // We do not have any save info. Kind affects didChange only.
        raijin_lsp::TextDocumentSyncCapability::Kind(_) => None,
    }
}
