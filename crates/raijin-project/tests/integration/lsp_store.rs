...
use raijin_language::{CodeLabel, HighlightId};

use raijin_project::lsp_store::*;
...
fn test_trailing_newline_in_completion_documentation() {
    let doc =
        raijin_lsp::Documentation::String("Inappropriate argument value (of correct type).\n".to_string());
    ...
    let doc = raijin_lsp::Documentation::String("  some value  \n".to_string());
    let completion_doc: CompletionDocumentation = doc.into();
    assert!(matches!(
        completion_doc,
