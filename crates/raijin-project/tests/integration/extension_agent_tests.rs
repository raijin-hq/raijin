...
use raijin_project::{agent_server_store::*, worktree_store::WorktreeIdCounter};
...
fn archive_launcher_constructs_with_all_fields() {
    use raijin_extension::AgentServerManifestEntry;
...
async fn archive_agent_uses_extension_and_agent_id_for_cache_key(cx: &mut TestAppContext) {
    ...
    let agent = LocalExtensionArchiveAgent {
        fs,
        http_client,
        node_runtime: raijin_node_runtime::NodeRuntime::unavailable(),
        project_environment,
