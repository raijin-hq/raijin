use anyhow::Result;
use raijin_context_server::test::create_fake_transport;
use raijin_context_server::{ContextServer, ContextServerId};
use inazuma::{AppContext, AsyncApp, Entity, Subscription, Task, TestAppContext, UpdateGlobal as _};
use raijin_http_client::{FakeHttpClient, Response};
use raijin_project::context_server_store::registry::ContextServerDescriptorRegistry;
use raijin_project::context_server_store::*;
use raijin_project::project_settings::ContextServerSettings;
use raijin_project::worktree_store::WorktreeStore;
use raijin_project::{
...
fn set_context_server_configuration(
    context_servers: Vec<(Arc<str>, inazuma_settings_framework::ContextServerSettingsContent)>,
...
async fn test_remote_context_server(cx: &mut TestAppContext) {
    ...
    let client = FakeHttpClient::create(|_| async move {
        use raijin_http_client::AsyncBody;
