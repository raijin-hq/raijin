...
use raijin_lsp::{DiagnosticSeverity, DiagnosticTag};
use raijin_project::lsp_command::*;
use raijin_rpc::proto::{self};
use serde_json::json;

#[test]
fn test_serialize_lsp_diagnostic() {
    let lsp_diagnostic = raijin_lsp::Diagnostic {
        range: raijin_lsp::Range {
            start: raijin_lsp::Position::new(0, 1),
            end: raijin_lsp::Position::new(2, 3),
        },
        severity: Some(DiagnosticSeverity::ERROR),
        code: Some(raijin_lsp::NumberOrString::String("E001".to_string())),
...
fn test_deserialize_lsp_diagnostic() {
    ...
    assert_eq!(
        lsp_diagnostic.code,
        Some(raijin_lsp::NumberOrString::String("ERR".to_string()))
    );
...
fn test_related_information() {
    let related_info = raijin_lsp::DiagnosticRelatedInformation {
        location: raijin_lsp::Location {
            uri: raijin_lsp::Uri::from_str("file:///test.rs").unwrap(),
            range: raijin_lsp::Range {
                start: raijin_lsp::Position::new(1, 1),
                end: raijin_lsp::Position::new(1, 5),
            },
        },
    ...
    let lsp_diagnostic = raijin_lsp::Diagnostic {
        range: raijin_lsp::Range {
            start: raijin_lsp::Position::new(0, 0),
            end: raijin_lsp::Position::new(0, 1),
        },
