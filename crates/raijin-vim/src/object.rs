...
use raijin_language::{BufferSnapshot, CharKind, Point, Selection, TextObject, TreeSitterOptions};
use raijin_multi_buffer::MultiBufferRow;
use schemars::JsonSchema;
use serde::Deserialize;
use raijin_ui::Context;
...
#[cfg(test)]
mod test {
    use raijin_editor::{Editor, EditorMode, MultiBuffer, test::editor_test_context::EditorTestContext};
    ...
    async fn test_miniquotes_object(cx: &mut inazuma::TestAppContext) {
        ...
        const TEST_CASES: &[(&str, &str, &str, Mode)] = &[
            ...
            // Bug reference: https://github.com/raijin-industries/raijin/issues/23889
        ...
        let (editor, cx) = cx.add_window_view(|window, cx| {
            ...
            if let Some(buffer) = multi_buffer.read(cx).buffer(buffer_ids[1]) {
                buffer.update(cx, |buffer, cx| {
                    buffer.set_language(Some(raijin_language::rust_lang()), cx);
                })
            };
