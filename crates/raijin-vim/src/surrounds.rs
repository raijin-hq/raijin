...
use raijin_editor::{Bias, MultiBufferOffset, movement};
use inazuma::{Context, Window};
use raijin_language::BracketPair;
...
mod test {
    ...
    async fn test_change_surrounds_any_brackets(cx: &mut inazuma::TestAppContext) {
        ...
        cx.set_state(indoc! {"(< name: ˇ'Raijin' >)"}, Mode::Normal);
        cx.simulate_keystrokes("c s b }");
        cx.assert_state(indoc! {"(ˇ{ name: 'Raijin' })"}, Mode::Normal);
