...
fn main() {
    ...
    inazuma_platform::application()
        ...
        .run(move |cx| {
            ...
            let http_client = ReqwestClient::user_agent("raijin_storybook").unwrap();
            cx.set_http_client(Arc::new(http_client));

            inazuma_settings_framework::init(cx);
            raijin_theme_settings::init(raijin_theme::LoadThemes::All(Box::new(Assets)), cx);

            let selector = story_selector;

            let mut theme_settings = ThemeSettings::get_global(cx).clone();
            theme_settings.theme =
                raijin_theme_settings::ThemeSelection::Static(inazuma_settings_framework::ThemeName(theme_name.into()));
