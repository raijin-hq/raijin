...
use raijin_client::{Collaborator, ParticipantIndex};
...
use raijin_feature_flags::{DebuggerFeatureFlag, FeatureFlagAppExt};
use futures::{
    FutureExt,
...
use raijin_language::{BufferRow, CharClassifier, Runnable, RunnableRange, point_to_lsp};
...
use raijin_task::{ResolvedTask, RunnableTag, TaskTemplate, TaskVariables};

pub use raijin_lsp::CompletionContext;
use raijin_lsp::{
...
use raijin_language::BufferSnapshot;
pub use lsp_ext::lsp_tasks;
use movement::TextLayoutDetails;
pub use raijin_multi_buffer::{
...
impl Editor {
    ...
    fn do_completion(
        ...
        use raijin_language::ToOffset as _;
        ...
        let result = h_flex()
            ...
            .when(!has_keybind, |el| {
                ...
                el.bg(status_colors.error_background)
                    .border_color(status_colors.error.opacity(0.6))
                    .pl_2()
                    .child(Icon::new(IconName::RaijinPredictError).color(Color::Error))
                    .cursor_default()
                    .hoverable_tooltip(move |_window, cx| {
                        cx.new(|_| MissingEditPredictionKeybindingTooltip).into()
                    })
                        ...
                        h_flex()
                            ...
                            .child(div().px_1p5().child(match &prediction.completion {
                                InlineCompletion::Move { target, snapshot } => {
                                    ...
                                    {
                                        Icon::new(IconName::RaijinPredictDown)
                                    } else {
                                        Icon::new(IconName::RaijinPredictUp)
                                    }
                                }
                                InlineCompletion::Edit { .. } => Icon::new(IconName::RaijinPredict),
                            }))
                            .child(
                                h_flex()
                                    .gap_1()
        ...
        match &completion.completion {
            ...
            InlineCompletion::Edit {
                ...
                let left = if first_edit_row != cursor_point.row {
                    ...
                    Icon::new(IconName::RaijinPredict).into_any_element()
                };
                ...
                .child(
                    h_flex()
                        ...
                        .child(Button::new("open-keymap", "Assign Keybinding").size(ButtonSize::Compact).on_click(|_ev, window, cx| {
                            window.dispatch_action(raijin_actions::OpenKeymap.boxed_clone(), cx)
                        }))
