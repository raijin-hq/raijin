...
use raijin_language::language_settings::{self, FormatOnSave};
use raijin_language::{Buffer, LanguageRegistry};
use raijin_language_model::LanguageModelToolResultContent;
use raijin_project::lsp_store::{FormatTrigger, LspFormatTarget};
use raijin_project::{AgentLocation, Project, ProjectPath};
...
";
    ...
    async fn test_streaming_authorize(cx: &mut TestAppContext) {
        ...
        let _auth = cx.update(|cx| {
            tool.authorize(
                &PathBuf::from(".raijin/settings.json"),
                "test 1",
                &stream_tx,
                cx,
            )
    ...
    async fn test_streaming_needs_confirmation_with_different_modes(cx: &mut TestAppContext) {
        ...
        for _mode in modes {
            ...
            let _auth = cx.update(|cx| {
                tool.authorize(
                    &PathBuf::from("project/.raijin/settings.json"),
                    "Edit settings",
                    &stream_tx,
                    cx,
                )
