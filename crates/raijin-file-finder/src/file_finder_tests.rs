...
use raijin_project::{FS_WATCH_LATENCY, RemoveOptions};
use serde_json::json;
use inazuma_settings_framework::SettingsStore;
use inazuma_util::{path, rel_path::rel_path};
use raijin_workspace::{
    AppState, CloseActiveItem, OpenOptions, ToggleFileFinder, Workspace, open_paths,
};
...
async fn test_nonexistent_history_items_not_shown(cx: &mut inazuma::TestAppContext) {
    ...
    let project = Project::test(app_state.fs.clone(), [path!("/src").as_ref()], cx).await;
    let (workspace, cx) =
        cx.add_window_view(|window, cx| Workspace::test_new(project, window, cx)); // generate some history to select from
    open_close_queried_buffer("fir", 1, "first.rs", &workspace, cx).await;
...
#[inazuma::test]
async fn test_search_results_refreshed_on_standalone_file_creation(cx: &mut inazuma::TestAppContext) {
    let app_state = init_test(cx);
...
#[inazuma::test]
async fn test_selected_match_stays_selected_after_matches_refreshed(cx: &mut inazuma::TestAppContext) {
    let app_state = init_test(cx);
