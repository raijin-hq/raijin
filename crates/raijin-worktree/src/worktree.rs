...
use inazuma_sum_tree::{Bias, Dimensions, Edit, KeyedItem, SeekTarget, SumTree, Summary, TreeMap, TreeSet};
...
pub struct LocalSnapshot {
    ...
    root_file_handle: Option<Arc<dyn raijin_fs::FileHandle>>,
}
...
impl File {
    ...
    pub fn from_dyn(file: Option<&Arc<dyn raijin_language::File>>) -> Option<&Self> {
        file.and_then(|f| {
            let f: &dyn raijin_language::File = f.borrow();
            let f: &dyn Any = f;
            f.downcast_ref()
        })
    }
...
impl<'a>
    inazuma_sum_tree::SeekTarget<'a, PathSummary<GitSummary>, Dimensions<TraversalProgress<'a>, GitSummary>>
    for PathTarget<'_>
{
    fn cmp(
        &self,
...
impl BackgroundScanner {
    async fn run(&mut self, mut fs_events_rx: Pin<Box<dyn Send + Stream<Item = Vec<PathEvent>>>>) {
        ...
        let global_gitignore_file = raijin_paths::global_gitignore_path();
        ...
        if let Some(abs_path) = containing_git_repository {
            self.process_events(vec![PathEvent {
                path: abs_path,
                kind: Some(raijin_fs::PathEventKind::Changed),
            }])
            .await;
        }
    ...
    async fn process_events(&self, mut events: Vec<PathEvent>) {
        ...
        events.dedup_by(|left, right| {
            if left.path == right.path {
                if matches!(left.kind, Some(raijin_fs::PathEventKind::Rescan)) {
        ...
        });
        {
            ...
            for (ix, event) in events.iter().enumerate() {
                ...
                relative_paths.push(EventRoot {
                    ...
                    was_rescanned: matches!(event.kind, Some(raijin_fs::PathEventKind::Rescan)),
                });
            }
...
async fn discover_ancestor_git_repo(
    ...
    for (index, ancestor) in root_abs_path.as_path().ancestors().enumerate() {
        if index != 0 {
            if ancestor == raijin_paths::home_dir() {
...
impl raijin_fs::Watcher for NullWatcher {
