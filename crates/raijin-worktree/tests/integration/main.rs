...
use anyhow::Result;
use encoding_rs;
use raijin_fs::{FakeFs, Fs, RealFs, RemoveOptions};
use raijin_git::{DOT_GIT, GITIGNORE, REPO_EXCLUDE};
use inazuma::{AppContext as _, BackgroundExecutor, BorrowAppContext, Context, Task, TestAppContext};
use parking_lot::Mutex;
use postage::stream::Stream;
use pretty_assertions::assert_eq;
use rand::prelude::*;
use raijin_worktree::{Entry, EntryKind, Event, PathChange, Worktree, WorktreeModelHandle};
...
async fn test_root_rescan_reconciles_stale_state(cx: &mut TestAppContext) {
    ...
    fs.emit_fs_event("/root", Some(raijin_fs::PathEventKind::Rescan));
...
async fn test_subtree_rescan_reports_unchanged_descendants_as_updated(cx: &mut TestAppContext) {
    ...
    fs.emit_fs_event("/root/dir", Some(raijin_fs::PathEventKind::Rescan));
    tree.flush_fs_events(cx).await;
...
async fn test_write_file(cx: &mut TestAppContext) {
    ...
    raijin_fs::fs_watcher::global(|_| {}).unwrap();
...
async fn test_fs_event_for_gitignored_dir_does_not_lose_contents(cx: &mut TestAppContext) {
    ...
    fs.emit_fs_event("/root/ignored_dir", Some(raijin_fs::PathEventKind::Changed));
    tree.flush_fs_events(cx).await;
