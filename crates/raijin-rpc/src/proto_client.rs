...
type RequestIds = Arc<
    Mutex<
        HashMap<
            ...
            oneshot::Sender<
                Result<
                    Option<TypedEnvelope<Vec<raijin_proto::ProtoLspResponse<Box<dyn AnyTypedEnvelope>>>>>,
                >,
            >,
        >,
    >,
...
impl AnyProtoClient {
    ...
    pub fn request_lsp<T>(
        ...
        Output = Result<Option<TypedEnvelope<Vec<raijin_proto::ProtoLspResponse<T::Response>>>>>,
    > + use<T>
    where
        T: LspRequestMessage,
    {
        ...
        let query = raijin_proto::LspQuery {
            project_id,
            server_id,
    ...
    pub fn send_lsp_response<T: LspRequestMessage>(
        ...
        self.send(raijin_proto::LspQueryResponse {
            project_id,
            lsp_request_id: lsp_request_id.0,
            responses: server_responses
                .into_iter()
                .map(|(server_id, response)| raijin_proto::LspResponse {
                    server_id,
    ...
    pub fn handle_lsp_response(&self, mut envelope: TypedEnvelope<raijin_proto::LspQueryResponse>) {
        ...
        if let Some(tx) = response_senders.remove(&request_id) {
            ...
            tx.send(Ok(Some(raijin_proto::TypedEnvelope {
                ...
                payload: responses
                    .into_iter()
                    .filter_map(|response| {
                        use raijin_proto::lsp_response::Response;
                        ...
                        Some(raijin_proto::ProtoLspResponse {
                            server_id,
                            response,
                        })
                    })
...
fn to_any_envelope<T: EnvelopedMessage>(
    envelope: &TypedEnvelope<raijin_proto::LspQueryResponse>,
    response: T,
) -> Box<dyn AnyTypedEnvelope> {
    Box::new(raijin_proto::TypedEnvelope {
        sender_id: envelope.sender_id,
        original_sender_id: envelope.original_sender_id,
        message_id: envelope.message_id,
        received_at: envelope.received_at,
